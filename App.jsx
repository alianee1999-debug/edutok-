...
const INIT_VIDEOS = [
  ...
  {id:3,title:"الحركة والقوة",teacher:"أ. سارة",subject:"العلوم",stage:"الابتدائية",grade:"الرابع",type:"teacher",duration:"3:10",price:3000,locked:true,thumbnail:"⚡",videoUrl:null},
  ...
  {id:5,title:"قواعد النحو",teacher:"أ. فاطمة",subject:"اللغة العربية",stage:"الابتدائية",grade:"الرابع",type:"teacher",duration:"2:50",price:2000,locked:true,thumbnail:"📝",videoUrl:null},
];
...
// ══════════════════════════════════════════════════════
// MAIN APP (entry point — auth gate only)
// ══════════════════════════════════════════════════════
export default function EduTok() {
  // Always start as null — require login every time app opens
...
// ══════════════════════════════════════════════════════
// APP (all hooks here — no early return before them)
// ══════════════════════════════════════════════════════
function App({student, onLogout}) {
  ...
  const [unlocked, setUnlocked] = useState([]); // "subject_stage" keys
  ...
  // Subject prices: "subject_stage" → {price, locked}
  ...
  const [notifications, setNotifications] = useState([]); // {id,title,body,date,read}
  ...
  const isUnlocked = (v) => !v.locked || unlocked.includes(v.subject+"_"+v.stage);

  const onVideoTap = (v) => {
    if (!isUnlocked(v)) setModal({type:"pay",data:v});
  };
  ...
  const sendNotification = (title, body) => {
    ...
    // Save to localStorage so all students see it
    try {
  ...
  const updatePrice = (key, field, val) => {
    setPrices(p=>({...p,[key]:{...p[key],[field]:val}}));
    ...
    <div ref={ref} style={{height:"100%",position:"relative",overflow:"hidden",
      ...
      <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{width:"82%",maxWidth:360,aspectRatio:"9/16",maxHeight:"76vh",
          ...
          {/* Play btn when unlocked and no real video */}
          {unlocked && !v.videoUrl && (
            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <div style={{width:60,height:60,background:"rgba(255,255,255,0.15)",borderRadius:"50%",
                ...
                <span style={{fontSize:22,marginRight:-3}}>▶</span>
              </div>
            </div>
          )}
      ...
      {/* Side buttons */}
      <div style={{position:"absolute",left:14,bottom:110,display:"flex",flexDirection:"column",gap:12}}>
        {[
          ...
          <Btn key={i} onClick={b.action} style={{background:"rgba(0,0,0,0.6)",borderRadius:"50%",width:48,height:48,
            display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,
            backdropFilter:"blur(10px)",boxShadow:b.active?`0 0 14px ${b.color}80`:"none",
            transform:b.active?"scale(1.1)":"scale(1)",transition:"all 0.2s"}}>
            <span style={{fontSize:20,filter:b.active?"none":"grayscale(0.3)"}}>{b.icon}</span>
            <span style={{fontSize:7,color:b.active?b.color:"#aaa",fontWeight:600}}>{b.label}</span>
          </Btn>
        ))}
        {/* More menu btn */}
        <Btn onClick={()=>setMenuOpen(o=>!o)} style={{background:menuOpen?"linear-gradient(135deg,#00d4ff,#a855f7)":"rgba(255,255,255,0.1)",
          borderRadius:"50%",width:48,height:48,display:"flex",flexDirection:"column",
          alignItems:"center",justifyContent:"center",gap:3,backdropFilter:"blur(10px)"}}>
          <span style={{fontSize:18,transform:menuOpen?"rotate(90deg)":"none",transition:"transform 0.3s"}}>⋯</span>
          <span style={{fontSize:7,color:"#ccc",fontWeight:600}}>المزيد</span>
        </Btn>
      </div>
...
function ProfileTab({student,isAdmin,saved,unlocked,onLogout,wallet,adminWallet,onTopUp,showNotif}) {
  ...
  const [tab,setTab] = useState("info"); // info | zain
...
// ── Zain Cash Card (shared between student & admin) ──
function ZainCashCard({wallet,isAdmin,phone,onTopUp,showNotif}) {
  const [amount,setAmount] = useState("");
  const [busy,setBusy] = useState(false);
  const [step,setStep] = useState("main"); // main | topup | done
...
// ══════════════════════════════════════════════════════
// ADMIN NOTIFICATIONS TAB (proper component — no hooks in IIFE)
// ══════════════════════════════════════════════════════
      ...
      {storedNotifs.length > 0 && (
        <div style={{marginTop:18}}>
          <div style={{fontSize:11,color:"#888",marginBottom:8,fontWeight:700}}>
            الإشعارات المرسلة ({storedNotifs.length})
          </div>
          {storedNotifs.slice(0,6).map((n,i)=>(
            <div key={i} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,
              padding:"9px 12px",marginBottom:6,border:"1px solid rgba(255,255,255,0.06)"}}>
...
// ADMIN TAB
// ══════════════════════════════════════════════════════
function AdminTab({videos,setVideos,prices,updatePrice,adminWallet,adminPass,setAdminPass,adminPhone,setAdminPhone,adminTab,setAdminTab,showNotif,sendNotification}) {
  ...
  const tabs = [["videos","🎬 المقاطع"],["students","👥 الطلاب"],["prices","💰 الأسعار"],["notifs","🔔 إشعارات"],["settings","⚙️ الإعدادات"]];
  ...
  return (
    <div style={{height:"100%",overflow:"auto",padding:"78px 14px 14px"}}>
        ...
        {tabs.map(([id,label])=>(
          <Btn key={id} onClick={()=>setAdminTab(id)}
            style={{background:adminTab===id?"linear-gradient(135deg,#f59e0b,#ef4444)":"rgba(255,255,255,0.06)",
              borderRadius:12,padding:"7px 12px",color:"#fff",fontSize:12,fontWeight:600}}>
            {label}
          </Btn>
        ))}
      </div>
        ...
        <div>
          ...
          {showAdd && (
            <div style={{background:"rgba(255,255,255,0.04)",borderRadius:16,padding:16,marginBottom:14,border:"1px solid rgba(0,212,255,0.2)"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                <div>
                  <label style={lbl}>نوع المقطع</label>
                ...
                <div>
                  <label style={lbl}>رقم المقطع</label>
                  <input style={inp} value={form.videoNumber} onChange={e=>setForm(f=>({...f,videoNumber:e.target.value}))} placeholder="01" />
                </div>
              </div>
              <div style={{marginBottom:10}}>
                <label style={lbl}>العنوان</label>
                <input style={inp} value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))} placeholder="عنوان المقطع" />
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                <div>
                  <label style={lbl}>المعلم</label>
                  <input style={inp} value={form.teacher} onChange={e=>setForm(f=>({...f,teacher:e.target.value}))} placeholder="أ. محمد" />
                </div>
                <div>
                  <label style={lbl}>الرمز</label>
                  <input style={{...inp,fontSize:20,textAlign:"center"}} value={form.thumbnail} onChange={e=>setForm(f=>({...f,thumbnail:e.target.value}))} placeholder="📚" />
                </div>
              </div>
              ...
              <div style={{marginBottom:14}}>
                ...
                <div style={{background:"rgba(245,158,11,0.07)",border:"1px solid rgba(245,158,11,0.2)",
                  borderRadius:10,padding:"8px 12px",marginBottom:10,fontSize:11,color:"#f59e0b"}}>
                ...
                {studioMode==="choose" && (
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                    ...
                    <label style={{background:"rgba(0,212,255,0.08)",border:"1.5px solid rgba(0,212,255,0.3)",
                      borderRadius:12,padding:"14px 8px",color:"#00d4ff",fontWeight:700,fontSize:13,
                    ...
                    <Btn onClick={startCam}
                      style={{background:"rgba(168,85,247,0.08)",border:"1.5px solid rgba(168,85,247,0.3)",
                        borderRadius:12,padding:"14px 8px",color:"#a855f7",fontWeight:700,fontSize:13,
                      ...
                      <div style={{display:"flex",gap:8}}>
                        <input
                          ...
                          style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",
                            borderRadius:10,padding:"8px 12px",color:"#fff",fontSize:12,direction:"ltr"}} />
                        <Btn onClick={()=>{
                        ...
                        }} style={{background:"linear-gradient(135deg,#00d4ff,#a855f7)",borderRadius:10,
                          padding:"8px 14px",color:"#fff",fontSize:12,fontWeight:700,whiteSpace:"nowrap"}}>
                          إضافة ▶
                        </Btn>
                      </div>
                    </div>
                ...
                {studioMode==="camera" && (
                  <div style={{background:"#000",borderRadius:12,overflow:"hidden",position:"relative"}}>
                    ...
                    <div style={{position:"absolute",bottom:8,left:0,right:0,display:"flex",justifyContent:"center",gap:12}}>
                      {!recording ? (
                        <Btn onClick={startRec} style={{background:"#ef4444",border:"3px solid #fff",
                          borderRadius:"50%",width:52,height:52,display:"flex",alignItems:"center",justifyContent:"center"}}>
                          <div style={{width:18,height:18,borderRadius:"50%",background:"#fff"}} />
                        </Btn>
                      ) : (
                        <Btn onClick={stopRec} style={{background:"#ef4444",border:"3px solid #fff",
                      ...
                      <Btn onClick={()=>{stopCam();setStudioMode("choose");}}
                        style={{background:"rgba(0,0,0,0.6)",border:"1px solid rgba(255,255,255,0.3)",
                          borderRadius:"50%",width:42,height:42,color:"#fff",fontSize:16}}>✕</Btn>
                    </div>
                    {recording && (
                ...
                {studioMode==="preview" && form.videoUrl && (
                  <div>
                    <video src={form.videoUrl} controls
                      style={{width:"100%",borderRadius:10,maxHeight:180,background:"#000",display:"block",marginBottom:6}}
                      onError={()=>showNotif("تعذّر تشغيل الفيديو — تحقق من الرابط","error")} />
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:11,color:"#22c55e",fontWeight:700}}>✓ {form.videoName}</span>
                      <Btn onClick={()=>{
                        if(form.videoUrl && form.videoUrl.startsWith("blob:")) URL.revokeObjectURL(form.videoUrl);
                        setForm(f=>({...f,videoUrl:null,videoName:""}));
                        setStudioMode("choose");
                      }} style={{background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",
                        borderRadius:8,padding:"4px 10px",color:"#ef4444",fontSize:11}}>حذف</Btn>
                    </div>
                  </div>
                )}
              </div>
          ...
          {videos.map(v=>(
            <div key={v.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:12,padding:12,marginBottom:8,display:"flex",gap:10,alignItems:"center",border:"1px solid rgba(255,255,255,0.06)"}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:"#888"}}>{v.stage} • {v.grade} • {v.subject} • فصل {v.semester}</div>
                <div style={{fontSize:10,color:"#666"}}>{typeLabel(v.type)} • {v.teacher||"—"} • ⏱{v.duration} {v.videoUrl&&<span style={{color:"#22c55e"}}>• 🎬</span>}</div>
              </div>
              <Btn onClick={()=>setVideos(vs=>vs.filter(x=>x.id!==v.id))}
                style={{background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:8,padding:"6px 10px",color:"#ef4444"}}>🗑</Btn>
            </div>
          ))}
        </div>
      )}
          ...
          <div>
            ...
            {list.length===0 ? (
              ...
              <div key={s.id||i} style={{background:"rgba(255,255,255,0.04)",borderRadius:12,padding:12,marginBottom:8,display:"flex",gap:10,alignItems:"center",border:"1px solid rgba(255,255,255,0.06)"}}>
                <div style={{width:40,height:40,background:"linear-gradient(135deg,#00d4ff,#a855f7)",borderRadius:"50%",
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:700,flexShrink:0}}>{s.name?.[0]}</div>
                <div style={{flex:1}}>
      ...
      {adminTab==="prices" && (
        <div>
          <div style={{fontSize:12,color:"#888",marginBottom:14,background:"rgba(0,212,255,0.06)",borderRadius:10,padding:"10px 14px",border:"1px solid rgba(0,212,255,0.15)"}}>
            💡 حدد سعراً لكل مادة حسب المرحلة — يُطبق تلقائياً على مقاطعها
          </div>
          {SUBJECTS.map(subj=>(
            <div key={subj} style={{marginBottom:16}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <div style={{flex:1,height:1,background:"rgba(255,255,255,0.08)"}} />
                <div style={{fontSize:13,fontWeight:800,color:"#00d4ff",whiteSpace:"nowrap"}}>📚 {subj}</div>
                <div style={{flex:1,height:1,background:"rgba(255,255,255,0.08)"}} />
              </div>
              {STAGES.map(stage=>{
                const key = subj+"_"+stage;
                const entry = prices[key]||{price:0,locked:false};
                return (
                  <div key={key} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"9px 12px",
                    marginBottom:6,border:"1px solid rgba(255,255,255,0.06)",display:"flex",alignItems:"center",gap:8}}>
                    <div style={{fontSize:12,fontWeight:700,color:"#aaa",minWidth:54}}>
                      {stage==="الابتدائية"?"ابتدائي":stage==="المتوسطة"?"متوسطة":"إعدادية"}
                    </div>
                    <input type="number" value={entry.price}
                      onChange={e=>updatePrice(key,"price",parseInt(e.target.value)||0)}
                    ...
                    <span style={{fontSize:11,color:"#666"}}>د.ع</span>
                    <Btn onClick={()=>updatePrice(key,"locked",!entry.locked)}
                      style={{background:entry.locked?"rgba(245,158,11,0.15)":"rgba(34,197,94,0.12)",
                        border:`1px solid ${entry.locked?"#f59e0b":"#22c55e"}`,borderRadius:8,padding:"6px 10px",
                        color:entry.locked?"#f59e0b":"#22c55e",fontSize:12,fontWeight:700}}>
                      {entry.locked?"🔒":"🔓"}
                    </Btn>
                  </div>
                );
      ...
      {adminTab==="settings" && (
        <div>
          ...
          <div style={{background:"rgba(255,255,255,0.04)",borderRadius:14,padding:16,marginBottom:12}}>
            ...
            <Btn onClick={()=>{if(newPhone.length<10)return showNotif("رقم غير صحيح","error");
              adminCreds.phone=newPhone;setAdminPhone(newPhone);setNewPhone("");showNotif("تم تغيير رقم الهاتف ✓");}}
          ...
          <div style={{background:"rgba(255,255,255,0.04)",borderRadius:14,padding:16,marginBottom:12}}>
            ...
            <Btn onClick={()=>{if(newPass.length<6)return showNotif("كلمة المرور قصيرة","error");
              adminCreds.password=newPass;setAdminPass(newPass);setNewPass("");showNotif("تم تغيير كلمة المرور ✓");}}
...
function BottomSheet({title,onClose,children}) {
  return (
    <div style={{position:"fixed",inset:0,zIndex:1000,display:"flex",flexDirection:"column",justifyContent:"flex-end"}}
      ...
      <div style={{position:"relative",background:"#111118",borderRadius:"22px 22px 0 0",
        maxHeight:"88vh",overflow:"auto",animation:"slideUp 0.3s ease",direction:"rtl"}}>
        <div style={{padding:"14px 18px",borderBottom:"1px solid rgba(255,255,255,0.08)",
          display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{fontSize:15,fontWeight:700}}>{title}</div>
          <Btn onClick={onClose} style={{background:"rgba(255,255,255,0.08)",borderRadius:"50%",width:30,height:30,
            display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:14}}>✕</Btn>
        </div>
...
function PayModal({v,wallet,onPay,onTopUp,onClose,showNotif}) {
  ...
  const enough = wallet.balance >= v.price;

  const payWallet = () => {
    if (!onPay(v)) return showNotif("رصيدك غير كافٍ","error");
    setDone(true);
    setTimeout(()=>{showNotif("تم الدفع ✓");onClose();},1500);
  };

  const payDirect = async () => {
    if (phone.length<10) return showNotif("رقم الهاتف غير صحيح","error");
    setBusy(true);
  ...
  if (done) return (
    <BottomSheet title="💳 الدفع" onClose={onClose}>
      <div style={{textAlign:"center",padding:24}}>
        <div style={{fontSize:64,marginBottom:12}}>✅</div>
        <div style={{fontSize:18,fontWeight:700,color:"#22c55e"}}>تم الدفع بنجاح!</div>
      </div>
    </BottomSheet>
  );

  return (
    <BottomSheet title="💳 الدفع — زين كاش" onClose={onClose}>
      <div style={{background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.25)",
        borderRadius:14,padding:16,textAlign:"center",marginBottom:16}}>
        <div style={{fontSize:32,marginBottom:6}}>{v.thumbnail}</div>
        <div style={{fontSize:16,fontWeight:700,marginBottom:2}}>{v.subject}</div>
        <div style={{fontSize:12,color:"#888",marginBottom:10}}>{v.stage} — جميع المقاطع</div>
        <div style={{fontSize:28,fontWeight:900,color:"#f59e0b"}}>{v.price.toLocaleString()} <span style={{fontSize:14}}>د.ع</span></div>
      </div>
      <div style={{background:enough?"rgba(34,197,94,0.08)":"rgba(255,255,255,0.04)",
        border:`1px solid ${enough?"#22c55e44":"rgba(255,255,255,0.08)"}`,borderRadius:14,padding:14,marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:enough?10:0}}>
          <div>
            <div style={{fontWeight:700,fontSize:13}}>💰 الدفع من المحفظة</div>
            <div style={{fontSize:11,color:"#888",marginTop:2}}>رصيدك: {wallet.balance.toLocaleString()} د.ع</div>
          </div>
          <span style={{background:enough?"#22c55e22":"#ef444422",color:enough?"#22c55e":"#ef4444",
            borderRadius:8,padding:"3px 8px",fontSize:11,fontWeight:700,alignSelf:"flex-start"}}>
            {enough?"كافٍ ✓":"غير كافٍ"}
          </span>
        </div>
        {enough && (
          <Btn onClick={payWallet} style={{width:"100%",background:"linear-gradient(135deg,#22c55e,#16a34a)",
            borderRadius:12,padding:12,color:"#fff",fontWeight:700,fontSize:14}}>
            ادفع فوراً ⚡
          </Btn>
        )}
      ...
      <div style={{background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:14,padding:14}}>
        <div style={{fontWeight:700,fontSize:13,marginBottom:4}}>📱 الدفع المباشر بزين كاش</div>
        <input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="07XX XXX XXXX" type="tel"
          style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(245,158,11,0.3)",
            borderRadius:10,padding:"10px 14px",color:"#fff",fontSize:15,textAlign:"center",
            letterSpacing:1,marginBottom:10,display:"block"}} />
        <Btn onClick={payDirect} disabled={busy}
          style={{width:"100%",background:busy?"rgba(245,158,11,0.3)":"linear-gradient(135deg,#f59e0b,#d97706)",
            borderRadius:12,padding:12,color:"#fff",fontWeight:700,fontSize:14}}>
          {busy?"⏳ جاري المعالجة...":`ادفع ${v.price.toLocaleString()} د.ع`}
        </Btn>
      </div>
    </BottomSheet>
  );
...
// ══════════════════════════════════════════════════════
// NOTIFICATION MODAL (bell for students)
// ══════════════════════════════════════════════════════
function NotifModal({notifications,setNotifications,onClose}) {
  const markAll = () => setNotifications(n=>n.map(x=>({...x,read:true})));
  const unread = notifications.filter(n=>!n.read).length;
  return (
    <BottomSheet title={`🔔 الإشعارات ${unread>0?`(${unread} جديد)`:""}`} onClose={onClose}>
      {unread>0 && (
        <Btn onClick={markAll} style={{width:"100%",background:"rgba(255,255,255,0.05)",
          border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,padding:"8px 0",
          color:"#888",fontSize:12,marginBottom:12}}>
          تحديد الكل كمقروء
        </Btn>
      )}
      {notifications.length===0 ? (
        <div style={{textAlign:"center",padding:32,color:"#555"}}>
          ...
          <div style={{fontSize:14}}>لا توجد إشعارات</div>
        </div>
      ) : notifications.map((n,i)=>(
        <div key={n.id||i}
          onClick={()=>setNotifications(prev=>prev.map((x,j)=>j===i?{...x,read:true}:x))}
          style={{background:n.read?"rgba(255,255,255,0.03)":"rgba(0,212,255,0.07)",
            border:`1px solid ${n.read?"rgba(255,255,255,0.06)":"rgba(0,212,255,0.2)"}`,
            borderRadius:12,padding:"11px 14px",marginBottom:8,cursor:"pointer",
            display:"flex",gap:10,alignItems:"flex-start"}}>
          <div style={{fontSize:22,flexShrink:0}}>{n.read?"🔔":"🔵"}</div>
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:700,marginBottom:3,color:n.read?"#ccc":"#fff"}}>{n.title}</div>
            <div style={{fontSize:12,color:"#888",lineHeight:1.5}}>{n.body}</div>
            <div style={{fontSize:10,color:"#555",marginTop:4}}>{n.date}</div>
          </div>
          {!n.read && <div style={{width:8,height:8,borderRadius:"50%",background:"#00d4ff",flexShrink:0,marginTop:4}} />}
        </div>
      ))}
    </BottomSheet>
  );
...
function WalletModal({wallet,adminWallet,isAdmin,onTopUp,onClose,showNotif}) {
  ...
  const doTopUp = async () => {
    const a = parseInt(amount);
    if (!a||a<1000) return showNotif("أدخل مبلغاً (1000 على الأقل)","error");
    if (phone.length<10) return showNotif("رقم الهاتف غير صحيح","error");
    setBusy(true);
    await new Promise(r=>setTimeout(r,1800));
    onTopUp(a);
  ...
  const w = view==="admin" ? adminWallet : wallet;

  return (
    <BottomSheet title="💰 المحفظة المالية" onClose={onClose}>
      <div style={{display:"flex",gap:6,marginBottom:16,background:"rgba(255,255,255,0.05)",borderRadius:12,padding:4}}>
        {(isAdmin ? [["user","👤 محفظتي"],["admin","👑 إيرادات"],["topup","➕ شحن"]] : [["user","👤 محفظتي"],["topup","➕ شحن"]]).map(([id,label])=>(
          <Btn key={id} onClick={()=>setView(id)}
            style={{flex:1,background:view===id?"linear-gradient(135deg,#22c55e,#16a34a)":"transparent",
              borderRadius:8,padding:"7px 4px",color:view===id?"#fff":"#888",fontSize:11,fontWeight:700}}>
            {label}
          </Btn>
        ))}
      </div>
      ...
      {(view==="user"||view==="admin") && (
        <div>
          <div style={{background:view==="admin"?"rgba(245,158,11,0.1)":"rgba(34,197,94,0.1)",
            border:`1px solid ${view==="admin"?"#f59e0b44":"#22c55e44"}`,borderRadius:16,
            padding:20,textAlign:"center",marginBottom:14}}>
            <div style={{fontSize:12,color:"#888",marginBottom:4}}>{view==="admin"?"إجمالي الإيرادات":"الرصيد المتاح"}</div>
            <div style={{fontSize:36,fontWeight:900,color:view==="admin"?"#f59e0b":"#22c55e"}}>{w.balance.toLocaleString()}</div>
            <div style={{fontSize:13,color:"#888"}}>دينار عراقي</div>
            {view==="user" && <Btn onClick={()=>setView("topup")} style={{marginTop:12,background:"linear-gradient(135deg,#22c55e,#16a34a)",borderRadius:12,padding:"8px 22px",color:"#fff",fontWeight:700,fontSize:13}}>+ شحن</Btn>}
          </div>
          <div style={{fontSize:12,color:"#888",marginBottom:8,fontWeight:700}}>📋 سجل العمليات ({w.txs.length})</div>
          {w.txs.length===0
            ? <div style={{textAlign:"center",color:"#555",padding:20}}>📭 لا توجد عمليات</div>
            : w.txs.map(tx=>(
              <div key={tx.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"10px 12px",
                marginBottom:6,display:"flex",alignItems:"center",gap:10,
                border:`1px solid ${tx.amount>0?"rgba(34,197,94,0.2)":"rgba(239,68,68,0.2)"}`}}>
                <span style={{fontSize:18}}>{tx.amount>0?"⬆️":"⬇️"}</span>
                <div style={{flex:1}}>
                  <div style={{fontSize:12,fontWeight:700}}>{tx.label}</div>
                  <div style={{fontSize:10,color:"#666"}}>{tx.date}</div>
                </div>
                <div style={{fontWeight:800,fontSize:14,color:tx.amount>0?"#22c55e":"#ef4444"}}>
                  {tx.amount>0?"+":""}{tx.amount.toLocaleString()}
                </div>
              </div>
            ))
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
            {[5000,10000,25000,50000].map(a=>(
              <Btn key={a} onClick={()=>setAmount(String(a))}
                style={{background:amount==a?"linear-gradient(135deg,#22c55e,#16a34a)":"rgba(255,255,255,0.05)",
                  border:`1px solid ${amount==a?"#22c55e":"rgba(255,255,255,0.1)"}`,
                  borderRadius:12,padding:12,color:amount==a?"#fff":"#aaa",fontWeight:700,fontSize:14}}>
                {a.toLocaleString()} د.ع
              </Btn>
            ))}
          </div>
          <input type="number" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="مبلغ مخصص (د.ع)"
            style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",
              borderRadius:12,padding:"10px 14px",color:"#fff",fontSize:15,display:"block",marginBottom:10}} />
          <input type="tel" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="07XX XXX XXXX"
            style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",
              borderRadius:12,padding:"10px 14px",color:"#fff",fontSize:15,textAlign:"center",
              letterSpacing:1,display:"block",marginBottom:14}} />
          <Btn onClick={doTopUp} disabled={busy}
            style={{width:"100%",background:busy?"rgba(34,197,94,0.3)":"linear-gradient(135deg,#22c55e,#16a34a)",
              borderRadius:14,padding:14,color:"#fff",fontWeight:700,fontSize:15}}>
            {busy?"⏳ جاري المعالجة...":"📱 شحن عبر زين كاش"}
          </Btn>
        </div>
      )}
    </BottomSheet>
...
function ClassModal({onClose,videos,onFilter}) {
  ...
  const apply = () => {
    const vids = videos.filter(v=>(!stage||v.stage===stage)&&(!grade||v.grade===grade)&&(!subject||v.subject===subject));
    onFilter([stage,grade,subject].filter(Boolean).join(" • "), vids);
  };

  const Chip = ({label,active,onClick,color="#00d4ff"}) => (
    <Btn onClick={onClick} style={{background:active?color+"22":"rgba(255,255,255,0.04)",
      border:`1px solid ${active?color:"rgba(255,255,255,0.08)"}`,borderRadius:12,
      padding:"10px 6px",color:active?color:"#aaa",fontWeight:600,fontSize:12,width:"100%"}}>
      {label}
    </Btn>
  );

  return (
    <BottomSheet title="📚 اختر صفك" onClose={onClose}>
      <div style={{marginBottom:12}}>
        <div style={{fontSize:11,color:"#888",marginBottom:8,fontWeight:700}}>المرحلة</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          {STAGES.map(s=><Chip key={s} label={s} active={stage===s} onClick={()=>setStage(x=>x===s?null:s)} color="#00d4ff" />)}
        </div>
      </div>
      {stage && (
        <div style={{marginBottom:12}}>
          <div style={{fontSize:11,color:"#888",marginBottom:8,fontWeight:700}}>الصف</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
            {GRADES[stage].map(g=><Chip key={g} label={g} active={grade===g} onClick={()=>setGrade(x=>x===g?null:g)} color="#a855f7" />)}
          </div>
        </div>
      )}
      <div style={{marginBottom:16}}>
        <div style={{fontSize:11,color:"#888",marginBottom:8,fontWeight:700}}>المادة</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {SUBJECTS.map(s=><Chip key={s} label={s} active={subject===s} onClick={()=>setSubject(x=>x===s?null:s)} color="#f59e0b" />)}
        </div>
      </div>
      <Btn onClick={apply} style={{width:"100%",background:"linear-gradient(135deg,#00d4ff,#a855f7)",
        borderRadius:14,padding:14,color:"#fff",fontWeight:700,fontSize:15}}>
        عرض المقاطع ▶
      </Btn>
    </BottomSheet>
...
function AIModal({onClose}) {
  const [msgs,setMsgs] = useState([{role:"assistant",content:"مرحباً! أنا مساعدك الذكي. كيف يمكنني مساعدتك؟ 🤖"}]);
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,
          system:"أنت مساعد تعليمي للطلاب العراقيين. أجب بالعربية بشكل بسيط وواضح.",
          messages:[...msgs.filter((_,i)=>i>0),{role:"user",content:msg}]})
      });
      const data = await res.json();
      setMsgs(m=>[...m,{role:"assistant",content:data.content?.[0]?.text||"حدث خطأ"}]);
    } catch { setMsgs(m=>[...m,{role:"assistant",content:"خطأ في الاتصال"}]); }
    setBusy(false);
  };

  return (
    <BottomSheet title="🤖 المساعد الذكي" onClose={onClose}>
      <div style={{height:300,overflow:"auto",display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{alignSelf:m.role==="user"?"flex-end":"flex-start",
            ...
            borderRadius:12,padding:"9px 13px",maxWidth:"85%",fontSize:14,lineHeight:1.6}}>
            {m.content}
          </div>
        ))}
        {busy && <div style={{alignSelf:"flex-start",color:"#888",fontSize:13}}>جاري التفكير...</div>}
        <div ref={endRef} />
      </div>
      <div style={{display:"flex",gap:8}}>
        <input value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()}
          ...
          style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",
            borderRadius:12,padding:"10px 14px",color:"#fff",fontSize:14,outline:"none",direction:"rtl"}} />
        <Btn onClick={send} style={{background:"linear-gradient(135deg,#a855f7,#7c3aed)",borderRadius:12,
          width:44,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>➤</Btn>
      </div>
    </BottomSheet>
  );
}
...
function ChatModal({onClose}) {
  const [msgs,setMsgs] = useState([
    {from:"علي محمد",text:"السلام عليكم! هل فهمت درس الكسور؟"},
    {from:"me",text:"وعليكم السلام، فهمت بعضه"},
  ]);
  const [inp,setInp] = useState("");
  const send = () => {
    if (!inp.trim()) return;
    setMsgs(m=>[...m,{from:"me",text:inp.trim()}]);
    setInp("");
  };
  return (
    <BottomSheet title="💬 التواصل مع الطلاب" onClose={onClose}>
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
        {["علي محمد","سارة أحمد","فاطمة حسن"].map(s=>(
          <span key={s} style={{background:"rgba(34,197,94,0.1)",border:"1px solid rgba(34,197,94,0.3)",
            borderRadius:16,padding:"3px 12px",fontSize:11,color:"#22c55e"}}>🟢 {s}</span>
        ))}
      </div>
      <div style={{height:220,overflow:"auto",display:"flex",flexDirection:"column",gap:8,marginBottom:10}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{alignSelf:m.from==="me"?"flex-end":"flex-start",
            background:m.from==="me"?"rgba(34,197,94,0.15)":"rgba(255,255,255,0.06)",
            borderRadius:12,padding:"8px 12px",maxWidth:"80%"}}>
            {m.from!=="me"&&<div style={{fontSize:10,color:"#22c55e",marginBottom:2}}>{m.from}</div>}
      ...
      <div style={{display:"flex",gap:8}}>
        <input value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()}
          ...
          style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",
            borderRadius:12,padding:"10px 14px",color:"#fff",fontSize:13,outline:"none",direction:"rtl"}} />
        <Btn onClick={send} style={{background:"#22c55e",borderRadius:12,width:44,
          display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>➤</Btn>
      </div>
    </BottomSheet>
  );
}
  ...
  const solve = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,
          system:"أنت معلم يحل الأسئلة للطلاب العراقيين. اشرح الحل خطوة بخطوة بالعربية.",
          messages:[{role:"user",content:`حل هذا السؤال: ${q}`}]})
      });
      const data = await res.json();
      setAns(data.content?.[0]?.text||"حدث خطأ");
    } catch { setAns("خطأ في الاتصال"); }
    setBusy(false);
  };

  return (
    <BottomSheet title="📷 حل سؤال" onClose={onClose}>
      {!mode && (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[{e:"📸",l:"التقاط صورة"},{e:"⌨️",l:"كتابة السؤال"},{e:"🎬",l:"الاستوديو"},{e:"📁",l:"رفع صورة"}].map((o,i)=>(
            <Btn key={i} onClick={()=>setMode(o.l==="كتابة السؤال"?"type":"other")}
              style={{background:"rgba(168,85,247,0.08)",border:"1.5px solid rgba(168,85,247,0.3)",
                borderRadius:14,padding:"16px 8px",color:"#a855f7",
      ...
      {mode==="other" && (
        <div style={{textAlign:"center"}}>
          <div style={{background:"rgba(255,255,255,0.04)",borderRadius:14,padding:36,marginBottom:14,
            border:"2px dashed rgba(255,255,255,0.1)"}}>
            <div style={{fontSize:48,marginBottom:8}}>📸</div>
            <div style={{color:"#888",fontSize:13}}>في التطبيق الحقيقي يفتح هنا</div>
          </div>
          <Btn onClick={()=>setMode("type")} style={{background:"rgba(255,255,255,0.06)",
            border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:"10px 20px",color:"#aaa",fontSize:13}}>
            كتابة السؤال يدوياً
          </Btn>
        </div>
      )}
      {mode==="type" && (
        <div>
          <textarea value={q} onChange={e=>setQ(e.target.value)} placeholder="اكتب سؤالك هنا..."
            style={{width:"100%",height:90,background:"rgba(255,255,255,0.06)",
              border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:12,
              color:"#fff",fontSize:14,resize:"none",direction:"rtl",display:"block",marginBottom:10}} />
          <Btn onClick={solve} disabled={busy}
            style={{width:"100%",background:busy?"rgba(168,85,247,0.3)":"linear-gradient(135deg,#a855f7,#7c3aed)",
              borderRadius:12,padding:12,color:"#fff",fontWeight:700,marginBottom:12,fontSize:14}}>
            {busy?"⏳ جاري الحل...":"🤖 حل بالذكاء الاصطناعي"}
          </Btn>
          {ans && (
            <div style={{background:"rgba(168,85,247,0.08)",border:"1px solid rgba(168,85,247,0.2)",
              borderRadius:12,padding:14,fontSize:13,lineHeight:1.8,whiteSpace:"pre-wrap"}}>
              {ans}
            </div>
          )}
        </div>
...
function PDFModal({onClose}) {
  ...
  const pdfs = [
    {name:"رياضيات الصف الرابع",icon:"📊"},{name:"علوم الصف الخامس",icon:"🔬"},
    {name:"قواعد اللغة العربية",icon:"📝"},{name:"الفيزياء للإعدادية",icon:"⚡"},
  ];
  return (
    <BottomSheet title="📄 مكتبة PDF" onClose={onClose}>
      <Btn onClick={()=>{}} style={{width:"100%",background:"linear-gradient(135deg,#ef4444,#dc2626)",
        borderRadius:14,padding:14,color:"#fff",fontWeight:700,marginBottom:14,
        display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontSize:14}}>
        + رفع ملف PDF
      </Btn>
      {pdfs.map((p,i)=>(
        <div key={i} onClick={()=>setSel(p)}
          style={{background:sel===p?"rgba(239,68,68,0.1)":"rgba(255,255,255,0.04)",
            border:`1px solid ${sel===p?"#ef4444":"rgba(255,255,255,0.08)"}`,
          ...
          {sel===p && <span style={{color:"#ef4444",fontSize:18}}>✓</span>}
        </div>
      ))}
      {sel && (
        <Btn onClick={()=>{}} style={{width:"100%",background:"linear-gradient(135deg,#ef4444,#dc2626)",
          borderRadius:14,padding:13,color:"#fff",fontWeight:700,marginTop:8,fontSize:14}}>
          📖 فتح {sel.name}
        </Btn>
      )}
    </BottomSheet>
...
function ShareModal({v,onClose}) {
  return (
    <BottomSheet title="📤 مشاركة" onClose={onClose}>
      <div style={{background:"rgba(255,255,255,0.04)",borderRadius:12,padding:13,
        marginBottom:14,display:"flex",gap:10,alignItems:"center"}}>
        <span style={{fontSize:30}}>{v.thumbnail}</span>
        <div>
          <div style={{fontWeight:700,fontSize:14}}>{v.title}</div>
          <div style={{fontSize:12,color:"#888"}}>{v.subject}</div>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {[{l:"واتساب",e:"💬",c:"#22c55e"},{l:"تيليغرام",e:"📨",c:"#0ea5e9"},
          {l:"فيسبوك",e:"👤",c:"#3b82f6"},{l:"نسخ الرابط",e:"🔗",c:"#888"}].map((p,i)=>(
          <Btn key={i} onClick={()=>{}}
            style={{background:p.c+"15",border:`1px solid ${p.c}44`,borderRadius:12,
              padding:14,color:p.c,display:"flex",alignItems:"center",gap:8,fontWeight:600,fontSize:13}}>
