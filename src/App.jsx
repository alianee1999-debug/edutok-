// ─────────────────────────────────────────
//  EduTok - Complete App with Firebase
//  Firebase Project: edutok-a48f9
// ─────────────────────────────────────────
...
import {
  Bookmark, Share2, Bot, MessageCircle, MoreHorizontal,
  FileText, Camera, Search, ChevronUp, ChevronDown,
  Settings, User, Home, Bell, DollarSign, Users, Layers,
  Film, Sparkles, X, Link, Save, BookOpen,
  GraduationCap, Plus, Play, Upload, Loader
} from "lucide-react";
...
const PRICE_SUBJECTS = ["الرياضيات","العلوم","اللغة العربية","اللغة الإنجليزية","الفيزياء","الكيمياء","الأحياء","التربية الإسلامية"];
...
const C = {
  ...
  warnBanner:{backgroundColor:"rgba(161,117,0,0.12)",border:"1px solid rgba(234,179,8,0.3)",borderRadius:"12px",padding:"10px 12px",fontSize:"12px",color:"#fbbf24",marginBottom:"12px",display:"flex",alignItems:"center",gap:"6px"},
...
const ErrBox = ({msg}) => msg ? (
  <div style={{backgroundColor:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:"10px",padding:"10px",fontSize:"13px",color:"#f87171",marginBottom:"14px",textAlign:"center"}}>⚠️ {msg}</div>
) : null;
...
function ShareModal({onClose,video}) {
  const link="https://edutok-a48f9.web.app/v/"+video.id;
...
function PDFModal({onClose,video}) {
  const [files,setFiles]=useState([]);
  const [loading,setLoading]=useState(true);
  useEffect(()=>{
    getDocs(collection(db,"clips",String(video.id),"files"))
      .then(snap=>{setFiles(snap.docs.map(d=>({id:d.id,...d.data()})));setLoading(false);})
      .catch(()=>{setFiles([]);setLoading(false);});
  },[video.id]);
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(249,115,22,0.2)"}}>
    <MHead icon={<FileText size={20} color="#f97316"/>} title="ملفات الدرس" color="#f97316" onClose={onClose}/>
    {loading?<div style={{textAlign:"center",padding:"20px"}}><Spinner color="#f97316"/></div>
    :files.length===0
      ?<div style={{textAlign:"center",padding:"24px",color:"#52525b"}}><FileText size={40} color="#3f3f46" style={{margin:"0 auto 8px"}}/><div>لا توجد ملفات لهذا الدرس بعد</div></div>
      :files.map(f=>(
        <div key={f.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",...C.card}}>
          <div><div style={{fontSize:"13px",fontWeight:"bold"}}>{f.name}</div><div style={{fontSize:"11px",color:"#71717a",marginTop:"2px"}}>{f.size||""}</div></div>
...
function SolveModal({onClose,video}) {
  const [tab,setTab]=useState("text"); const [q,setQ]=useState(""); const [url,setUrl]=useState("");
  const [ans,setAns]=useState(""); const [loading,setLoading]=useState(false);
  const solve=async()=>{setLoading(true);setAns("");
    try{
      const msgs=tab==="text"
        ?[{role:"user",content:"أنت مساعد تعليمي. الطالب يدرس "+video.subject+". السؤال: "+q+". حله خطوة بخطوة بالعربية."}]
        :[{role:"user",content:[{type:"image",source:{type:"url",url:url.trim()}},{type:"text",text:"أنت مساعد تعليمي. الطالب يدرس "+video.subject+". حل هذا السؤال خطوة بخطوة بالعربية."}]}];
      const r=await callAI(msgs);setAns(r);
    }catch{setAns("حدث خطأ في الاتصال.");}setLoading(false);
  };
  const can=tab==="text"?q.trim():url.trim();
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(52,211,153,0.2)"}}>
    ...
    <div style={{display:"flex",gap:"8px",marginBottom:"14px"}}>
      ...
      <button onClick={()=>setTab("url")} style={{flex:1,padding:"10px",borderRadius:"10px",border:"none",backgroundColor:tab==="url"?"#34d399":"#27272a",color:tab==="url"?"#000":"#a1a1aa",fontWeight:"bold",fontSize:"13px",cursor:"pointer"}}>🔗 رابط صورة</button>
    </div>
    ...
    {tab==="url"&&<><div style={C.warnBanner}>🔗 ارفع الصورة على imgur.com ثم الصق رابطها هنا</div><input type="url" value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://i.imgur.com/example.jpg" style={C.input}/>{url.trim()&&<div style={{marginBottom:"12px",borderRadius:"12px",overflow:"hidden",border:"1px solid rgba(255,255,255,0.08)"}}><img src={url.trim()} alt="معاينة" style={{width:"100%",maxHeight:"180px",objectFit:"contain",backgroundColor:"#09090b"}} onError={e=>e.target.style.display="none"}/></div>}</>}
    ...
    {!ans&&!loading&&<button onClick={solve} disabled={!can} style={{...C.purpleBtn,background:can?"linear-gradient(to right,#059669,#34d399)":"#27272a",opacity:can?1:0.5}}><Bot size={16}/> {tab==="text"?"حل السؤال":"تحليل الصورة وحل السؤال"}</button>}
    {ans&&<div style={{display:"flex",gap:"10px"}}><button onClick={()=>{setAns("");setQ("");setUrl("");}} style={C.cancelBtn}>سؤال جديد</button><button onClick={onClose} style={C.saveBtn}>إغلاق ✓</button></div>}
  </div></div>;
}

function SearchModal({onClose,clips}) {
  const [q,setQ]=useState("");
  const all=[...SAMPLE_VIDEOS,...clips];
...
// ─────────────────────────────────────────
function VideoUpload({onUploadDone, existingUrl, onChange}) {
  ...
  const handleFile = (e) => {
    ...
    if (file.size > 500*1024*1024) { alert("حجم الفيديو كبير جداً (أكثر من 500MB)"); return; }

    setUploading(true);
    const storRef = ref(storage, "videos/" + Date.now() + "_" + file.name);
    const task = uploadBytesResumable(storRef, file);
    task.on("state_changed",
      snap => setProgress(Math.round(snap.bytesTransferred/snap.totalBytes*100)),
      err  => { alert("فشل الرفع: "+err.message); setUploading(false); },
      async () => {
        const url = await getDownloadURL(task.snapshot.ref);
        setLocalUrl(url);
        setUploading(false);
        setProgress(100);
        onUploadDone && onUploadDone(url);
      }
    );
  };
    ...
    <div style={{marginBottom:"14px"}}>
      {localUrl ? (
        <div style={{marginBottom:"10px"}}>
          <video src={localUrl} controls style={{width:"100%",borderRadius:"12px",maxHeight:"200px"}}/>
          <button onClick={()=>{setLocalUrl("");onChange&&onChange("");}} style={{marginTop:"6px",backgroundColor:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:"8px",padding:"6px 12px",color:"#f87171",fontSize:"12px",cursor:"pointer"}}>✕ حذف الفيديو</button>
        </div>
      ) : uploading ? (
        <div style={{backgroundColor:"#09090b",borderRadius:"12px",padding:"20px",textAlign:"center",border:"1px solid rgba(255,255,255,0.08)"}}>
          <Spinner color="#0ea5e9"/>
          <div style={{marginTop:"10px",fontSize:"13px",color:"#38bdf8"}}>جارٍ رفع الفيديو... {progress}%</div>
          <div style={C.progressBar}><div style={{width:progress+"%",height:"100%",backgroundColor:"#0ea5e9",transition:"width 0.3s"}}/></div>
        </div>
      ) : (
        <div>
          <label style={{display:"block",width:"100%",padding:"20px",backgroundColor:"rgba(14,165,233,0.1)",border:"2px dashed rgba(14,165,233,0.4)",borderRadius:"14px",textAlign:"center",cursor:"pointer",boxSizing:"border-box"}}>
            <Upload size={32} color="#0ea5e9" style={{margin:"0 auto 8px"}}/>
            <div style={{fontSize:"13px",color:"#0ea5e9",fontWeight:"bold"}}>اضغط لرفع فيديو من جهازك</div>
            <div style={{fontSize:"11px",color:"#71717a",marginTop:"4px"}}>MP4, MOV — حتى 500MB</div>
            <input ref={fileRef} type="file" accept="video/*" style={{display:"none"}} onChange={handleFile}/>
          </label>
          <div style={{textAlign:"center",margin:"8px 0",color:"#52525b",fontSize:"12px"}}>أو</div>
          <input type="text" value={localUrl} onChange={e=>{setLocalUrl(e.target.value);onChange&&onChange(e.target.value);}} placeholder="الصق رابط فيديو مباشر (YouTube, Drive...)" style={{...C.input,marginBottom:0}}/>
        </div>
      )}
    </div>
  );
...
function SlidesStudio({slidesTheme,setSlidesTheme,onSaveClip}) {
  ...
  const [topic,setTopic]=useState(""); const [imageUrl,setImageUrl]=useState("");
  const [slidesCount,setCount]=useState(6); const [loading,setLoading]=useState(false); const [loadMsg,setLoadMsg]=useState("");
  const [slides,setSlides]=useState([]); const [curSlide,setCurSlide]=useState(0);
  const [clipTitle,setClipTitle]=useState(""); const [clipSubj,setClipSubj]=useState("الرياضيات"); const [clipStage,setClipStage]=useState("الابتدائية");
  ...
  const generate=async(fromImage)=>{
    ...
    try{
      const msgs=fromImage
        ?[{role:"user",content:[{type:"image",source:{type:"url",url:imageUrl.trim()}},{type:"text",text:"اقرأ هذه الورقة وحوّل محتواها إلى "+slidesCount+" شرائح تعليمية."+JSONSUFFIX}]}]
  ...
  const saveToFirestore = async () => {
    setSaving(true);
    try {
      const clipData = {title:clipTitle,subject:clipSubj,stage:clipStage,slides,theme:slidesTheme,type:"شرائح AI",bg:"linear-gradient(135deg,#1e1b4b,#312e81)",createdAt:serverTimestamp()};
      await addDoc(collection(db,"clips"),clipData);
      onSaveClip(clipData);
      alert("✅ تم حفظ الشرائح في Firebase!");
  ...
  if(mode==="menu") return <div>
    ...
    <div style={{...C.twoCol,marginBottom:"16px"}}>
      <div onClick={()=>setMode("image")} style={{backgroundColor:"rgba(88,28,135,0.25)",border:"1px solid rgba(139,92,246,0.35)",borderRadius:"16px",padding:"24px 16px",textAlign:"center",cursor:"pointer"}}>
        ...
        <div style={{fontSize:"13px",fontWeight:"bold",color:"#c4b5fd"}}>رابط صورة ورقة</div>
        <div style={{fontSize:"11px",color:"#71717a",marginTop:"4px"}}>الصق رابط الصورة</div>
      </div>
      <div onClick={()=>setMode("text")} style={{backgroundColor:"rgba(3,105,161,0.25)",border:"1px solid rgba(56,189,248,0.35)",borderRadius:"16px",padding:"24px 16px",textAlign:"center",cursor:"pointer"}}>
        ...
        <div style={{fontSize:"11px",color:"#71717a",marginTop:"4px"}}>اكتب الموضوع</div>
      </div>
    </div>
  ...
  if(mode==="image") return <div>{back}
    <div style={{fontSize:"15px",fontWeight:"bold",marginBottom:"12px"}}>رابط صورة الورقة</div>
    <div style={C.warnBanner}>💡 ارفع الصورة على <strong>imgur.com</strong> ثم الصق الرابط</div>
    <label style={C.label}>🔗 رابط الصورة</label>
    <input type="url" value={imageUrl} onChange={e=>setImageUrl(e.target.value)} placeholder="https://i.imgur.com/example.jpg" style={C.input}/>
    {imageUrl.trim()&&<div style={{marginBottom:"12px",borderRadius:"12px",overflow:"hidden",border:"1px solid rgba(255,255,255,0.08)"}}><img src={imageUrl.trim()} alt="معاينة" style={{width:"100%",maxHeight:"200px",objectFit:"contain",backgroundColor:"#09090b"}} onError={e=>e.target.style.display="none"}/></div>}
    ...
    {loading?<div style={{textAlign:"center",padding:"16px"}}><Spinner color="#a855f7"/><div style={{marginTop:"8px",fontSize:"13px",color:"#a855f7"}}>{loadMsg}</div></div>
      :<button disabled={!imageUrl.trim()} onClick={()=>generate(true)} style={{...C.purpleBtn,opacity:imageUrl.trim()?1:0.5}}>🤖 قراءة الورقة وإنشاء الشرائح</button>}
  </div>;

  if(mode==="text") return <div>{back}
    <div style={{fontSize:"15px",fontWeight:"bold",marginBottom:"12px"}}>كتابة موضوع</div>
  ...
  if(mode==="result"){const sl=slides[curSlide]||{}; return <div>
    ...
    <div style={{...C.card,border:"1px solid rgba(56,189,248,0.15)"}}>
      ...
      <button onClick={saveToFirestore} disabled={saving} style={{...C.primaryBtn,marginBottom:0,opacity:saving?0.7:1}}>
        {saving?<><Spinner size={16}/> جارٍ الحفظ...</>:<><Save size={16}/> حفظ في Firebase</>}
      </button>
    </div>
  </div>;}
  return null;
...
export default function App() {
  ...
  const [regAccount,setRegAccount]=useState(""); const [regPass,setRegPass]=useState(""); const [regErr,setRegErr]=useState("");
  // login
  ...
  useEffect(()=>{
    ...
    // clips
    const unsubClips = onSnapshot(collection(db,"clips"), snap=>{
      setClips(snap.docs.map(d=>({id:d.id,...d.data()})));
    });
    // prices
    getDocs(collection(db,"prices")).then(snap=>{
      const p={};snap.docs.forEach(d=>{p[d.id]=d.data();});setPrices(p);
    }).catch(()=>{});
  ...
  const handleLogoTap=()=>{
    setTapCount(c=>{const n=c+1;if(n>=6){setShowAdminLogin(true);clearTimeout(tapTimer.current);return 0;}clearTimeout(tapTimer.current);tapTimer.current=setTimeout(()=>setTapCount(0),2000);return n;});
  };
  ...
  const doRegister=async()=>{
    ...
    try{
      ...
      setCurrentStudent(s);setRole("student");setScreen("home");
  ...
  const saveClip=async()=>{
    ...
    try{
      await addDoc(collection(db,"clips"),{
        ...
        videoUrl:clipVideoUrl,bg:"linear-gradient(180deg,#0f172a,#1e1b4b)",
        createdAt:serverTimestamp()
      });
      alert("✅ تم حفظ المقطع في Firebase!");
      setShowClipForm(false);
      setClipTitle("");setClipTeacher("");setClipVideoUrl("");setClipNum("01");
    }catch(e){alert("فشل الحفظ: "+e.message);}
    setSavingClip(false);
  };

  const savePrice=async(subject,stage,value)=>{
    try{
      const key=subject+"_"+stage;
      await addDoc(collection(db,"prices"),{subject,stage,value,key});
    }catch{}
  };
  ...
  const sendNotification=async()=>{
    ...
    try{
      ...
      alert("✅ تم حفظ الإشعار وإرساله لـ "+students.length+" طالب!");
  ...
  return (
    <div style={C.app}>
      ...
      {screen==="home"&&(
        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} style={{userSelect:"none"}}>
          <div style={{...C.videoWrap,background:video.bg||"linear-gradient(180deg,#0f172a,#1e1b4b)"}}>
            ...
            {showMore&&<div style={C.moreMenu}>{[[<FileText size={22} color="#fff"/>,"PDF","pdf"],[<Camera size={22} color="#fff"/>,"حل ذكي","solve"],[<Search size={22} color="#fff"/>,"البحث","search"],[<Bookmark size={22} color="#fff"/>,"المحفوظات","saved"]].map(([icon,label,key])=><button key={key} style={C.moreItem} onClick={()=>{setModal(key);setShowMore(false);}}>{icon}<span style={{fontSize:"11px",marginTop:"4px"}}>{label}</span></button>)}</div>}
          </div>
          ...
          <div style={C.section}>
              ...
              <div>
                {clips.map((clip,i)=>(
                  <div key={i} style={{...C.card,border:"1px solid rgba(139,92,246,0.2)"}}>
                    <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"6px"}}><Film size={16} color="#a855f7"/><span style={{fontWeight:"bold",fontSize:"14px",flex:1}}>{clip.title}</span><span style={{backgroundColor:"rgba(139,92,246,0.2)",color:"#a855f7",fontSize:"11px",padding:"2px 8px",borderRadius:"6px"}}>{clip.type||"مقطع"}</span></div>
                    ...
                    {clip.videoUrl&&<div style={{marginTop:"8px",backgroundColor:"rgba(52,211,153,0.1)",borderRadius:"8px",padding:"4px 10px",fontSize:"11px",color:"#34d399",display:"inline-block"}}>✅ فيديو مرفوع</div>}
                  </div>
                ))}
            ...
            {adminTab==="clips"&&showClipForm&&(
              <div>
                ...
                <label style={{...C.label,display:"flex",alignItems:"center",gap:"6px"}}><Film size={14}/> الفيديو</label>
                <VideoUpload existingUrl={clipVideoUrl} onUploadDone={url=>setClipVideoUrl(url)} onChange={url=>setClipVideoUrl(url)}/>
                <div style={C.saveRow}>
                  ...
                  <button disabled={savingClip} style={{...C.saveBtn,display:"flex",alignItems:"center",justifyContent:"center",gap:"6px",opacity:savingClip?0.7:1}} onClick={saveClip}>
                    {savingClip?<><Spinner size={15}/> جارٍ الحفظ...</>:<><Save size={15}/> حفظ في Firebase</>}
                  </button>
                </div>
              </div>
            )}
            ...
            {adminTab==="prices"&&(
              <div>
                <div style={C.infoBanner}>💡 حدد سعراً لكل مادة — يُحفظ في Firebase تلقائياً</div>
                    ...
                    {STAGES.map(stage=>(
                      <div key={stage} style={C.priceRow}>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:"6px",backgroundColor:"#09090b",padding:"6px 10px",borderRadius:"10px",border:"1px solid rgba(255,255,255,0.08)",flex:1,margin:"0 10px"}}>
                          <input type="number" defaultValue={prices[subj+"_"+stage]?.value||""} onBlur={e=>savePrice(subj,stage,e.target.value)} placeholder="•" style={C.priceInput}/>
                        </div>
            ...
            {adminTab==="notifications"&&(
              <div>
                <div style={C.infoBanner}><Bell size={15}/> الإشعارات تُحفظ في Firebase وتُرسل لـ {students.length} طالب.</div>
            ...
            {adminTab==="settings"&&(
              <div>
                ...
                <div style={C.card}><span style={{color:"#38bdf8",fontWeight:"bold",fontSize:"13px",display:"block",marginBottom:"10px"}}>📱 تغيير رقم الهاتف</span><input type="text" placeholder="07XX XXX XXXX" style={C.input}/><button style={C.blueBtn} onClick={()=>alert("في النسخة الكاملة يُحدَّث في Firebase.")}>تغيير رقم الهاتف</button></div>
                <div style={{...C.card,marginTop:"10px"}}><span style={{color:"#ef4444",fontWeight:"bold",fontSize:"13px",display:"block",marginBottom:"10px"}}>🔑 تغيير كلمة المرور</span><input type="password" placeholder="كلمة المرور الجديدة" style={C.input}/><button style={C.redBtn} onClick={()=>alert("في النسخة الكاملة يُحدَّث في Firebase.")}>تغيير كلمة المرور</button></div>
                <div style={{color:"#a855f7",fontWeight:"bold",fontSize:"14px",display:"flex",alignItems:"center",gap:"6px",margin:"16px 0 8px"}}>📊 إحصائيات Firebase</div>
      ...
      {modal==="pdf"&&<PDFModal onClose={closeModal} video={video}/>}
