...
import { Bookmark, Share2, Bot, MessageCircle, MoreHorizontal, FileText, Camera, Search, ChevronUp, ChevronDown, Settings, User, Home, Bell, DollarSign, Users, Layers, Film, Sparkles, X, Save, BookOpen, GraduationCap, Plus, Play, Pause, Loader, Trash2, Edit3, AlertTriangle, Check } from "lucide-react";
...
const ConfirmDelete = ({title, subtitle, onConfirm, onCancel}) => (
  <div style={C.confirmBox}>
    ...
    <div style={{display:"flex",gap:"10px",marginTop:"16px"}}>
      ...
      <button onClick={onConfirm} style={{flex:1,padding:"12px",backgroundColor:"#ef4444",border:"none",borderRadius:"12px",color:"#fff",fontWeight:"bold",fontSize:"14px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}><Trash2 size={15}/> احذف</button>
    </div>
  </div>
);
    ...
    return (
      <div style={{position:"absolute",inset:0,zIndex:2,background:ts.bg,display:"flex",flexDirection:"column",justifyContent:"center",padding:"20px 16px",overflowY:"auto"}} onClick={onClick}>
        ...
        <div style={{display:"flex",gap:"10px",marginTop:"14px"}}>
          ...
          <button disabled={slideIdx===video.slides.length-1} onClick={e=>{e.stopPropagation();setSlideIdx(i=>i+1);}} style={{flex:1,padding:"10px",borderRadius:"12px",border:"none",backgroundColor:slideIdx===video.slides.length-1?"rgba(255,255,255,0.05)":"rgba(255,255,255,0.15)",color:slideIdx===video.slides.length-1?"rgba(255,255,255,0.3)":"#fff",cursor:slideIdx===video.slides.length-1?"not-allowed":"pointer",fontWeight:"bold",fontSize:"13px"}}>التالي</button>
        </div>
...
function AIModal({onClose,video}) {
  ...
  const ask=async()=>{
    ...
    try{const r=await callGemini("أنت مساعد تعليمي. الطالب يشاهد درس: "+video.title+" في مادة "+video.subject+". سؤاله: "+q+". أجب بإيجاز وبالعربية.");setAns(r||"لم أتمكن من الإجابة.");}
...
function SolveModal({onClose,video}) {
  ...
  const solve=async()=>{
    ...
    try{
      let r;
      if(tab==="text"){r=await callGemini("أنت مساعد تعليمي. الطالب يدرس "+video.subject+". السؤال: "+q+". حله خطوة بخطوة بالعربية.");}
      else{if(!imgB64){setAns("يرجى رفع صورة أولاً.");setLoading(false);return;}r=await callGemini("أنت مساعد تعليمي. الطالب يدرس "+video.subject+". انظر للصورة وحل هذا السؤال خطوة بخطوة بالعربية.",imgB64);}
...
function WalletModal({onClose,student,subscriptions}) {
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(34,197,94,0.2)"}}>
    ...
    <div style={{display:"flex",gap:"6px",marginBottom:"14px"}}>{DURATIONS.map(d=><button key={d.days} onClick={()=>setSelDuration(d)} style={{flex:1,padding:"9px 4px",borderRadius:"10px",border:"none",backgroundColor:selDuration.days===d.days?"#4ade80":"#27272a",color:selDuration.days===d.days?"#000":"#a1a1aa",fontWeight:"bold",fontSize:"10px",cursor:"pointer"}}>{d.label}</button>)}</div>
...
function SlidesStudio({slidesTheme,setSlidesTheme,onSaveClip}) {
  ...
  if(editingSlideIdx!==null) return (
    <div>
      ...
      <button onClick={()=>setEditSlidePoints(p=>[...p,""])} style={{width:"100%",padding:"10px",borderRadius:"10px",border:"1px dashed rgba(56,189,248,0.4)",backgroundColor:"rgba(56,189,248,0.06)",color:"#38bdf8",fontSize:"13px",cursor:"pointer",marginBottom:"14px",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}><Plus size={14}/> إضافة نقطة</button>
      <div style={C.saveRow}>
              ...
              <div key={clip.id} style={{...C.card,border:"1px solid rgba(139,92,246,0.2)",marginBottom:"8px"}}>
                {confirmDeleteClipDoc?.id===clip.id
                  ...
                  :<>
                    ...
                    <div style={{display:"flex",gap:"8px"}}>
                      <button onClick={()=>loadClipForEdit(clip)} style={{flex:1,padding:"9px",borderRadius:"10px",border:"1px solid rgba(56,189,248,0.3)",backgroundColor:"rgba(56,189,248,0.1)",color:"#38bdf8",fontSize:"12px",fontWeight:"bold",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"5px"}}><Edit3 size={13}/> تعديل</button>
                      <button onClick={()=>setConfirmDeleteClipDoc(clip)} style={{flex:1,padding:"9px",borderRadius:"10px",border:"1px solid rgba(239,68,68,0.3)",backgroundColor:"rgba(239,68,68,0.1)",color:"#f87171",fontSize:"12px",fontWeight:"bold",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"5px"}}><Trash2 size={13}/> حذف</button>
                    </div>
                  </>
                }
              </div>
  ...
  if(mode==="image") return <div>{back}
    ...
    <div style={{backgroundColor:"rgba(139,92,246,0.1)",border:"1px solid rgba(139,92,246,0.3)",borderRadius:"12px",padding:"10px 14px",marginBottom:"12px",fontSize:"12px",color:"#c4b5fd"}}> صوّر صفحة الكتاب أو الورقة — Groq سيقرأها ويحوّلها لشرائح تعليمية</div>
    <div style={{marginBottom:"12px"}}>
      {imgPreview&&<img src={imgPreview} alt="معاينة" style={{width:"100%",maxHeight:"200px",objectFit:"contain",borderRadius:"12px",marginBottom:"8px",border:"1px solid rgba(255,255,255,0.1)"}}/>}
  ...
  if(mode==="result"){
    ...
    return <div>
      ...
      <div style={{background:ts.bg,borderRadius:"20px",padding:"20px 16px",minHeight:"200px",marginBottom:"10px",border:"1px solid rgba(255,255,255,0.08)",position:"relative"}}>
        <div style={{position:"absolute",top:"10px",left:"12px",backgroundColor:ts.card,borderRadius:"8px",padding:"3px 10px",border:"1px solid "+ts.accent+"44"}}><span style={{color:ts.accent,fontSize:"11px",fontWeight:"bold"}}>شريحة {curSlide+1}</span></div>
        {/* ── NEW: Per-slide action buttons ── */}
        <div style={{position:"absolute",top:"8px",right:"10px",display:"flex",gap:"6px"}}>
          <button onClick={()=>openEditSlide(curSlide)} style={{width:32,height:32,borderRadius:"8px",backgroundColor:"rgba(56,189,248,0.2)",border:"1px solid rgba(56,189,248,0.3)",color:"#38bdf8",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} title="تعديل الشريحة"><Edit3 size={14}/></button>
          <button onClick={()=>setConfirmDeleteSlideIdx(curSlide)} style={{width:32,height:32,borderRadius:"8px",backgroundColor:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",color:"#f87171",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} title="حذف الشريحة" disabled={slides.length<=1}><Trash2 size={14}/></button>
        </div>
      ...
      <button onClick={addSlide} style={{width:"100%",padding:"11px",borderRadius:"12px",border:"1px dashed rgba(139,92,246,0.4)",backgroundColor:"rgba(139,92,246,0.06)",color:"#c4b5fd",fontSize:"13px",cursor:"pointer",marginBottom:"12px",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}><Plus size={14}/> إضافة شريحة جديدة</button>
        ...
        <div>
          ...
          <div style={C.section}>
            ...
            {adminTab==="clips"&&!showClipForm&&(
              <div>
                {/* ── NEW: Inline ConfirmDelete for clips ── */}
                {confirmDeleteClip&&(
                  <ConfirmDelete
                    title={confirmDeleteClip.title}
                    subtitle={confirmDeleteClip.subject+" • "+confirmDeleteClip.stage+" • "+(confirmDeleteClip.type||"معلم")}
                    onConfirm={()=>doDeleteClip(confirmDeleteClip)}
                    onCancel={()=>setConfirmDeleteClip(null)}
                  />
                )}
                {clips.length===0&&(
                  <div style={{textAlign:"center",padding:"24px 0 10px"}}>
                    <Film size={48} color="#3f3f46" style={{margin:"0 auto 12px"}}/>
                    <p style={{color:"#71717a",fontSize:"14px",margin:"0 0 16px"}}>لا توجد مقاطع بعد</p>
                  </div>
                )}
                {clips.map((clip,i)=>(
                  <div key={i} style={{...C.card,border:"1px solid rgba(139,92,246,0.2)"}}>
                    <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"8px"}}>
                      {clip.thumbUrl
                        ?<img src={clip.thumbUrl} alt="" style={{width:44,height:44,borderRadius:"8px",objectFit:"cover"}}/>
                        :<div style={{width:44,height:44,borderRadius:"8px",background:clip.bg||"linear-gradient(135deg,#1e1b4b,#312e81)",display:"flex",alignItems:"center",justifyContent:"center"}}><Film size={20} color="#fff"/></div>
                      }
                      <div style={{flex:1}}>
                        <div style={{fontWeight:"bold",fontSize:"14px"}}>{clip.title}</div>
                        <div style={{fontSize:"12px",color:"#71717a",marginTop:"2px"}}>{clip.subject} • {clip.stage}</div>
                        <div style={{fontSize:"11px",color:"#52525b",marginTop:"1px"}}>{clip.type||"معلم"} {clip.grade?" • الصف "+clip.grade:""} {clip.semester?" • الفصل "+clip.semester:""}</div>
                      </div>
                    </div>
                    {clip.videoUrl&&<div style={{fontSize:"11px",color:"#34d399",marginBottom:"6px",display:"flex",alignItems:"center",gap:"4px"}}><span style={{width:6,height:6,borderRadius:"50%",backgroundColor:"#34d399",display:"inline-block"}}/>{getYoutubeId(clip.videoUrl)?"يوتيوب":"فيديو مباشر"}</div>}
                    {clip.slides&&<div style={{fontSize:"11px",color:"#a855f7",marginBottom:"6px",display:"flex",alignItems:"center",gap:"4px"}}><Layers size={11}/> {clip.slides.length} شريحة</div>}
                    {/* ── NEW: Edit + Delete buttons ── */}
                    <div style={{display:"flex",gap:"8px",marginTop:"4px"}}>
                      <button onClick={()=>openEditClip(clip)} style={{flex:1,padding:"9px",borderRadius:"10px",border:"1px solid rgba(56,189,248,0.35)",backgroundColor:"rgba(56,189,248,0.08)",color:"#38bdf8",fontSize:"12px",fontWeight:"bold",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"5px"}}>
                        <Edit3 size={13}/> تعديل
                      </button>
                      <button onClick={()=>setConfirmDeleteClip(clip)} style={{flex:1,padding:"9px",borderRadius:"10px",border:"1px solid rgba(239,68,68,0.35)",backgroundColor:"rgba(239,68,68,0.08)",color:"#f87171",fontSize:"12px",fontWeight:"bold",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"5px"}}>
                        <Trash2 size={13}/> حذف
                      </button>
                    </div>
                  </div>
                ))}
                <button style={{...C.gradBtn,marginTop:"6px"}} onClick={()=>{setEditingClip(null);setShowClipForm(true);}}><Plus size={18}/> إضافة مقطع جديد</button>
              </div>
            )}
            ...
            {adminTab==="clips"&&showClipForm&&(
              <div>
                <div style={{...C.infoBanner,marginBottom:"12px"}}>
                  {editingClip?<><Edit3 size={15}/> <span style={{fontWeight:"bold"}}>تعديل المقطع: {editingClip.title}</span></>:<><Film size={15}/> <span style={{fontWeight:"bold"}}>مقطع جديد</span></>}
                </div>
                ...
                <div style={C.twoCol}>
                  <div><label style={C.label}>المرحلة</label><select style={C.select} v
