...
export default function App() {
  ...
  // مشترك بها فعلاً، نجيب مقاطع هذا الفصل تحديداً على دفعات (PAGE_SIZE
  // بالمرة) عبر استعلام مخصص (query+limit+startAfter)، بدل الاعتماد على
  // مصفوفة "clips" الكاملة. لوحة الإدارة وخيار "كل الفصول" وحالة "مادة مدفوعة
  // وغير مشترك" (معاينة عشوائية) تبقى تعتمد على "clips" الكاملة تماماً كما
  // كانت — بدون أي تغيير بسلوكها — لتجنّب أي مخاطرة على منطق موجود ومُختبر.
  const PAGE_SIZE = 20;
  ...
  const paginatedCursorRef = useRef(null);
  ...
  // تحميل مقاطع فصل محدد للطالب على دفعات (راجع تعليق تعريف paginatedClips
  // أعلاه لشرح الفكرة والنطاق). يعاد التحميل من الصفر كل ما تغيّرت المادة/
  // المرحلة/الصف/الفصل المحدد، أو لما يصير عند الطالب صلاحية وصول جديدة.
  ...
  useEffect(()=>{
    if(!usePaginatedClips){
      setPaginatedClips([]); setPaginatedHasMore(true); paginatedCursorRef.current=null;
      return;
    }
    let cancelled=false;
    setPaginatedClips([]); setPaginatedHasMore(true); paginatedCursorRef.current=null;
    ...
    getDocs(q).then(snap=>{
      if(cancelled) return;
      setPaginatedClips(snap.docs.map(d=>({id:d.id,...d.data()})));
      paginatedCursorRef.current = snap.docs[snap.docs.length-1] || null;
      setPaginatedHasMore(snap.docs.length===PAGE_SIZE);
    }).catch(e=>{
      ...
      console.error("تعذّر تحميل مقاطع الفصل بالترقيم (رجعنا للتحميل الكامل):", e);
  ...
  const loadMorePaginatedClips = () => {
    if(!usePaginatedClips || paginatedLoading || !paginatedHasMore || !paginatedCursorRef.current) return;
    setPaginatedLoading(true);
    const q = query(
      collection(db,"clips"),
      where("subject","==",selectedSubject.subject),
      ...
      where("topic","==",selectedSubject.topic),
      orderBy("num"),
      startAfter(paginatedCursorRef.current),
      limit(PAGE_SIZE)
    );
    getDocs(q).then(snap=>{
      setPaginatedClips(prev=>[...prev, ...snap.docs.map(d=>({id:d.id,...d.data()}))]);
      paginatedCursorRef.current = snap.docs[snap.docs.length-1] || paginatedCursorRef.current;
      setPaginatedHasMore(snap.docs.length===PAGE_SIZE);
    }).catch(e=>{
      console.error("فشل تحميل صفحة إضافية من مقاطع الفصل:", e);
    }).finally(()=>setPaginatedLoading(false));
  };
  ...
  const allVideos = React.useMemo(()=>{
    ...
    // نتجاوز فلترة/فرز "clips" الكاملة تماماً هنا، لأن الاستعلام نفسه
    // (orderBy("num")+limit) رجع لنا نتيجة مرتبة بترتيب الفصل الصحيح مسبقاً؛
    // بس نطبّق نفس فلتر تطابق الصف (بما فيها المقاطع بلا صف محدد) وفلتر
    // استبعاد المقاطع ناقصة العنوان، تماماً بنفس منطق المسار العادي أدناه.
    if(usePaginatedClips){
      return paginatedClips.filter(c=>
  ...
  },[clips, selectedSubject, role, globalPrices, examScores, mySubscriptions, usePaginatedClips, paginatedClips]);
          ...
          <div style={{position:"absolute",inset:0,background:video.bg||"linear-gradient(180deg,#0f172a,#1e1b4b)"}}>
            ...
            {role==="student" && selectedSubject?.topic && (
              <div style={{position:"absolute",top:"6px",left:"6px",zIndex:50,backgroundColor:"rgba(220,38,38,0.9)",color:"#fff",fontSize:"10px",fontWeight:"bold",padding:"3px 8px",borderRadius:"6px",pointerEvents:"none"}}>
                ترقيم: {usePaginatedClips?"✅ شغّال":(paginatedFailed?"❌ فشل":"➖ غير مفعّل")} | محمّل: {usePaginatedClips?paginatedClips.length:clips.filter(c=>c.subject===selectedSubject.subject&&c.stage===selectedSubject.stage&&c.topic===selectedSubject.topic).length}
              </div>
            )}
