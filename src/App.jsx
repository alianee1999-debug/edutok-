...
if (typeof document !== "undefined") {
  ...
  document.addEventListener("touchmove", (e) => {
    ...
    if (document.scrollingElement && document.scrollingElement.scrollTop === 0 && y > lastY) {
      e.preventDefault();
    }
    lastY = y;
  }, {passive:false});
...
const callGroq = async (prompt, imageBase64=null, imageMime="image/jpeg") => {
  // Groq supports vision with llama-4-scout-17b
...
// ─── STYLES ─────────────────────────────────────────────
const C = {
  app:{width:"100%",maxWidth:"420px",minHeight:"100vh",backgroundColor:"#09090b",color:"#fff",fontFamily:"system-ui,-apple-system,sans-serif",direction:"rtl",margin:"0 auto",paddingBottom:"72px",boxSizing:"border-box",overflowX:"hidden",position:"relative",overscrollBehavior:"none"},
...
function PDFModal({onClose,isSubbed}) {
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(249,115,22,0.2)"}}>
    ...
    {!isSubbed&&<div style={{backgroundColor:"rgba(234,179,8,0.1)",border:"1px solid rgba(234,179,8,0.3)",borderRadius:"12px",padding:"12px",marginBottom:"14px",textAlign:"center"}}><div style={{color:"#fbbf24",fontWeight:"bold",fontSize:"14px",marginBottom:"4px"}}>🔒 محتوى مدفوع</div><div style={{color:"#71717a",fontSize:"12px",marginBottom:"10px"}}>اشترك للوصول لجميع الملازم</div><button onClick={onClose} style={{backgroundColor:"#f97316",border:"none",borderRadius:"10px",padding:"8px 20px",color:"#fff",fontSize:"13px",fontWeight:"bold",cursor:"pointer"}}>اشترك الآن عبر زين كاش</button></div>}
...
// ─── SOLVE MODAL (with Gemini Vision) ───────────────────
function SolveModal({onClose,video}) {
  const [tab,setTab]=useState("text");
  const [q,setQ]=useState(""); const [imgB64,setImgB64]=useState(null);
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(52,211,153,0.2)"}}>
    ...
    {tab==="img"&&<ImageUploader onUpload={()=>{}} onBase64={b64=>setImgB64(b64)} color="#34d399" label="صوّر السؤال أو اختره من المعرض"/>}
    ...
    {ans&&<div style={{display:"flex",gap:"10px"}}><button onClick={()=>{setAns("");setQ("");setImgB64(null);}} style={C.cancelBtn}>سؤال جديد</button><button onClick={onClose} style={C.saveBtn}>إغلاق ✓</button></div>}
  </div></div>;
}
...
function SlidesStudio({slidesTheme,setSlidesTheme,onSaveClip}) {
  ...
  const generate=async(fromImage)=>{
    ...
    try{
      let raw;
      if(fromImage){
        if(!imgB64){alert("يرجى رفع صورة أولاً");setLoading(false);return;}
        raw=await callGemini("اقرأ هذه الورقة الدراسية وحوّل محتواها إلى "+slidesCount+" شرائح تعليمية."+JSONSUFFIX,imgB64);
      }else{
        if(!topic.trim()){alert("يرجى إدخال الموضوع");setLoading(false);return;}
  ...
  if(mode==="image") return <div>{back}
    ...
    <div style={{backgroundColor:"rgba(139,92,246,0.1)",border:"1px solid rgba(139,92,246,0.3)",borderRadius:"12px",padding:"10px 14px",marginBottom:"12px",fontSize:"12px",color:"#c4b5fd"}}>
      📸 صوّر صفحة الكتاب أو الورقة — Gemini سيقرأها ويحوّلها لشرائح تعليمية
    </div>
    <ImageUploader
      onUpload={url=>setImgPreview(url)}
      onBase64={b64=>setImgB64(b64)}
      color="#a855f7"
      label="صوّر صفحة الكتاب أو اختر من المعرض"
    />
    ...
    {loading?<div style={{textAlign:"center",padding:"20px"}}><Spinner color="#a855f7"/><div style={{marginTop:"10px",fontSize:"14px",color:"#a855f7",fontWeight:"bold"}}>{loadMsg}</div></div>
      :<button disabled={!imgB64} onClick={()=>generate(true)} style={{...C.purpleBtn,opacity:imgB64?1:0.5}}>🤖 Gemini يقرأ الورقة ويبني الشرائح</button>}
  </div>;
...
function AdminPDFTab() {
  ...
  useEffect(()=>{const unsub=onSnapshot(collection(db,"pdfs"),snap=>{setPdfs(snap.docs.map(d=>({id:d.id,...d.data()})));});return()=>unsub();},[]);
  const savePDF=async()=>{
    if(!pdfName.trim()||!pdfUrl.trim()) return alert("أدخل الاسم والرابط");
    setSaving(true);
    try{await addDoc(collection(db,"pdfs"),{name:pdfName,subject:pdfSubject,stage:pdfStage,url:pdfUrl,thumbUrl:pdfThumb,createdAt:serverTimestamp()});alert("✅ تم حفظ الملف!");setShowForm(false);setPdfName("");setPdfUrl("");setPdfThumb(null);}
  ...
  return <div>
    ...
    {!showForm?<button style={C.gradBtn} onClick={()=>setShowForm(true)}><Plus size={18}/> إضافة ملف PDF جديد</button>
...
function AdminWalletTab() {
  ...
  const approvePayment=async(p)=>{
    ...
    try{
      ...
      alert("✅ تم تفعيل اشتراك "+p.subject+" لـ "+p.studentName);
    }catch(e){alert("فشل: "+e.message);}
  };
  const rejectPayment=async(p)=>{
    try{await addDoc(collection(db,"payments"),{...p,status:"rejected"});alert("تم رفض الدفع");}catch(e){alert("فشل");}
  };
  return <div>
    <div style={{background:"linear-gradient(135deg,#14532d,#15803d)",borderRadius:"16px",padding:"20px",marginBottom:"14px",textAlign:"center"}}>
      ...
      {editingNum?<div style={{display:"flex",gap:"8px",justifyContent:"center",alignItems:"center"}}><input value={newNum} onChange={e=>setNewNum(e.target.value)} style={{padding:"8px 12px",backgroundColor:"rgba(0,0,0,0.3)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:"8px",color:"#fff",fontSize:"16px",outline:"none",textAlign:"center",width:"170px"}}/><button onClick={()=>{setZaincash(newNum);setEditingNum(false);}} style={{backgroundColor:"#4ade80",border:"none",borderRadius:"8px",padding:"8px 14px",color:"#000",fontWeight:"bold",cursor:"pointer",fontSize:"13px"}}>حفظ</button></div>
      :<div style={{fontSize:"22px",fontWeight:"bold",color:"#4ade80",letterSpacing:"2px",cursor:"pointer"}} onClick={()=>setEditingNum(true)}>{zaincash} ✏️</div>}
    </div>
  ...
  const saveClip=async()=>{
    ...
    try{
      await addDoc(collection(db,"clips"),{
        ...
        bg:"linear-gradient(180deg,#0f172a,#1e1b4b)",createdAt:serverTimestamp()
      });
      alert("✅ تم حفظ المقطع وسيظهر في الشاشة الرئيسية!");
      setShowClipForm(false);setClipTitle("");setClipTeacher("");setClipVideoUrl("");setClipThumbUrl(null);setClipNum("01");
    }catch(e){alert("فشل الحفظ: "+e.message);}
    setSavingClip(false);
  };
    ...
    <div style={C.app}>
          ...
          <div style={C.section}>
                ...
                {clips.map((clip,i)=>(
                  <div key={i} style={{...C.card,border:"1px solid rgba(139,92,246,0.2)"}}>
                    <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"6px"}}>
                      ...
                      <div style={{flex:1}}><div style={{fontWeight:"bold",fontSize:"14px"}}>{clip.title}</div><div style={{fontSize:"12px",color:"#71717a"}}>{clip.subject} • {clip.stage} • {clip.type||"معلم"}</div></div>
                    </div>
                    {clip.videoUrl&&<div style={{fontSize:"11px",color:"#34d399"}}>✅ {getYoutubeId(clip.videoUrl)?"يوتيوب":"فيديو مباشر"}</div>}
                    {clip.slides&&<div style={{fontSize:"11px",color:"#a855f7"}}>📊 {clip.slides.length} شريحة</div>}
                  </div>
                ))}
            ...
            {adminTab==="clips"&&showClipForm&&(
              <div>
                ...
                <label style={C.label}>🖼️ صورة مصغرة</label>
                ...
                <div style={C.saveRow}>
                  ...
                  <button disabled={savingClip} style={{...C.saveBtn,display:"flex",alignItems:"center",justifyContent:"center",gap:"6px",opacity:savingClip?0.7:1}} onClick={saveClip}>
                    {savingClip?<><Spinner size={15}/> جارٍ الحفظ...</>:<><Save size={15}/> حفظ وعرض</>}
                  </button>
                </div>
              </div>
            )}
            ...
            {adminTab==="prices"&&(
              <div>
                ...
                <button style={{...C.gradBtn,marginTop:"16px"}} onClick={()=>alert("تم حفظ الأسعار!")}><Save size={16}/> حفظ الأسعار</button>
              </div>
            )}
            ...
            {adminTab==="settings"&&(
              <div>
                ...
                <div style={C.card}><span style={{color:"#38bdf8",fontWeight:"bold",fontSize:"13px",display:"block",marginBottom:"10px"}}>📱 تغيير رقم الهاتف</span><input type="text" placeholder="07XX XXX XXXX" style={C.input}/><button style={C.blueBtn} onClick={()=>alert("تم!")}>تغيير</button></div>
                <div style={{...C.card,marginTop:"10px"}}><span style={{color:"#ef4444",fontWeight:"bold",fontSize:"13px",display:"block",marginBottom:"10px"}}>🔑 تغيير كلمة المرور</span><input type="password" placeholder="كلمة المرور الجديدة" style={C.input}/><button style={C.redBtn} onClick={()=>alert("تم!")}>تغيير</button></div>
      ...
      {screen==="account"&&(
        <div style={{padding:"40px 20px",textAlign:"center"}}>
          ...
          <button style={C.redBtn} onClick={()=>{setRole("guest");setCurrentStudent(null);clearSession();setScreen("welcome");}}>تسجيل الخروج</button>
        </div>
      )}
      ...
      {modal==="pdf"    &&<PDFModal     onClose={closeModal} isSubbed={isSubbed}/>}
      ...
      {showAdminLogin   &&<AdminLoginModal onClose={()=>setShowAdminLogin(false)} onSuccess={()=>{setRole("admin");setScreen("admin");saveSession({name:"المدير",phone:"07700000000"},"admin");setShowAdminLogin(false);}}/>}
    </div>
  );
}
