...
const AnimatedSlides = ({video, playing, onClick, ts, slideIdx, setSlideIdx}) => {
  ...
  return (
    ...
    >
      ...
      {/* أزرار التنقل */}
      <div style={{position:"absolute",bottom:"130px",left:"12px",right:"12px",display:"flex",gap:"8px",zIndex:30}}>
        <button disabled={slideIdx===0} onClick={e=>{e.stopPropagation();goTo(slideIdx-1);}} style={{flex:1,padding:"9px",borderRadius:"10px",border:"none",backgroundColor:slideIdx===0?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.18)",color:slideIdx===0?"rgba(255,255,255,0.2)":"#fff",cursor:slideIdx===0?"not-allowed":"pointer",fontWeight:"bold",fontSize:"13px",backdropFilter:"blur(8px)"}}>◀ السابق</button>
        <button disabled={slideIdx===total-1} onClick={e=>{e.stopPropagation();goTo(slideIdx+1);}} style={{flex:1,padding:"9px",borderRadius:"10px",border:"none",backgroundColor:slideIdx===total-1?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.18)",color:slideIdx===total-1?"rgba(255,255,255,0.2)":"#fff",cursor:slideIdx===total-1?"not-allowed":"pointer",fontWeight:"bold",fontSize:"13px",backdropFilter:"blur(8px)"}}>التالي ▶</button>
      </div>
      ...
      {screen==="home"&&(
        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove} style={{...C.fullScreenWrap,userSelect:"none",overscrollBehavior:"none"}}>
          ...
          <div style={{position:"absolute",inset:0,background:video.bg||"linear-gradient(180deg,#0f172a,#1e1b4b)"}}>
            <div style={{position:"absolute",top:0,bottom:"80px",left:"80px",right:0,zIndex:4,cursor:"pointer"}} onClick={()=>setPlaying(p=>!p)}/>
