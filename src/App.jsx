...
// ─── GOOGLE DRIVE PDF LINK HELPER ───────────────────────
// يحول أي رابط Google Drive (مشاركة عادية) إلى رابط عرض/تحميل مباشر
// حتى لا يفتح المتصفح تطبيق خارجي مثل WPS Office ويعلّق بالتحميل
      ...
      :files.map(f=>(
        <div key={f.id} style={{...C.card}}>
          ...
          <div style={{display:"flex",gap:"8px"}}>
            <a href={getPdfViewUrl(f.url)} target="_blank" rel="noreferrer" style={{flex:1,padding:"8px",borderRadius:"8px",border:"1px solid rgba(56,189,248,0.3)",backgroundColor:"rgba(56,189,248,0.1)",color:"#38bdf8",fontSize:"12px",fontWeight:"bold",textDecoration:"none",textAlign:"center"}}>قراءة 👁</a>
            <a href={getPdfDownloadUrl(f.url)} target="_blank" rel="noreferrer" style={{flex:1,padding:"8px",borderRadius:"8px",border:"1px solid rgba(249,115,22,0.3)",backgroundColor:"rgba(249,115,22,0.15)",color:"#f97316",fontSize:"12px",fontWeight:"bold",textDecoration:"none",textAlign:"center"}}>تحميل ⬇</a>
          </div>
        </div>
      ))
    }
        ...
        <div key={f.id} style={{...C.card}}>
          ...
          {!canRead
            ...
            :<div style={{display:"flex",gap:"8px"}}>
              ...
              <a href={getPdfViewUrl(f.url)} target="_blank" rel="noreferrer" onClick={()=>{
              ...
              {canDownload && !f.downloadBlocked
                ?<a href={getPdfDownloadUrl(f.url)} target="_blank" rel="noreferrer" style={{flex:1,padding:"8px",borderRadius:"8px",border:"1px solid rgba(249,115,22,0.3)",backgroundColor:"rgba(249,115,22,0.15)",color:"#f97316",fontSize:"12px",cursor:"pointer",fontWeight:"bold",textDecoration:"none",textAlign:"center"}}>تحميل ⬇</a>
    ...
    <div style={C.app}>
      ...
      {screen==="home"&&(
        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove} style={{...C.fullScreenWrap,userSelect:"none",overscrollBehavior:"none"}}>
          ...
          {showMore&&<div style={C.moreMenu}>
            {[[<FileText size={22} color="#fff"/>,"PDF","pdf"],[<Camera size={22} color="#fff"/>,"حل ذكي","solve"],[<Search size={22} color="#fff"/>,"البحث","search"],[<DollarSign size={22} color="#fff"/>,"زين كاش","wallet"],[<BookOpen size={22} color="#fff"/>,"تصفح","browse"]].map(([icon,label,key])=>(
      ...
      {modal==="browse" &&<BrowseModal onClose={closeModal} clips={clips} globalPrices={globalPrices} onBrowse={(subject,stage,grade,topic,isFree)=>{
        ...
        if(isFree){
          ...
          const otherClips=clips.filter(c=>!(c.subject===subject&&c.stage===stage)).sort(()=>Math.random()-0.5).slice(0,20);
          setSelectedSubject({subject,stage,grade,topic,customList:[...first5,...rest,...otherClips]});
        }
        setVideoIdx(0);
