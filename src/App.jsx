...
export default function App() {
  ...
  useEffect(()=>{
    // استرجاع جلسة الطالب (تخزين محلي، لا صلاحيات حساسة)
    const session = loadSession();
    if(session?.role === "student" && session?.student?.phone && session?.student?.pass && session.student.name && session.student.account){
