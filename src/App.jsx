...
function BrowseModal({onClose, clips, globalPrices, onBrowse}) {
  ...
  // المباحث المتاحة لهذه المادة والمرحلة
  const availableTopics = React.useMemo(()=>{
    if(!selStage||!selSubject) return [];
    const topics=new Set();
    clips.forEach(c=>{
      if(c.stage===selStage&&c.subject===selSubject&&c.topic) topics.add(c.topic);
    });
    return [...topics].sort();
  },[clips,selStage,selSubject]);
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(99,102,241,0.3)"}}>
    ...
    {selStage&&<div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"14px",fontSize:"12px",flexWrap:"wrap"}}>
      <span style={{color:"#818cf8",cursor:"pointer"}} onClick={()=>{setStep("stage");setSelStage("");setSelSubject("");}}>المراحل</span>
      {selStage&&<><span style={{color:"#52525b"}}>←</span>
        <span style={{color:selSubject?"#818cf8":"#fff",cursor:selSubject?"pointer":"default"}}
          onClick={()=>{if(selSubject){setStep("subject");setSelSubject("");}}}>
          {selStage}
        </span></>}
    ...
    {step==="stage"&&<div>
      ...
      {STAGES.map(s=>(
        <div key={s} onClick={()=>{setSelStage(s);setStep("subject");}} style={{...C.card,cursor:"pointer",display:"flex",alignItems:"center",gap:"14px",marginBottom:"8px",border:"1px solid rgba(129,140,248,0.2)"}}>
    ...
    {/* خطوة ٢: اختيار المادة */}
    {step==="subject"&&<div>
      <div style={{fontSize:"13px",color:"#71717a",marginBottom:"12px",textAlign:"center"}}>اختر المادة</div>
      {availableSubjects.length===0
        ?<div style={{textAlign:"center",padding:"24px",color:"#52525b"}}>لا توجد مواد لهذه المرحلة</div>
          ...
          return <div key={subj} onClick={()=>{setSelSubject(subj);setStep("topic");}} style={{...C.card,cursor:"pointer",display:"flex",alignItems:"center",gap:"12px",marginBottom:"8px",border:`1px solid ${isFree?"rgba(52,211,153,0.3)":"rgba(129,140,248,0.2)"}`}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:"11px",color:"#71717a",marginTop:"2px"}}>
                {count} مقطع • {isFree?"🆓 مجاني":"💰 مدفوع"}{hasTopics?" • فيها مباحث":""}
              </div>
            </div>
    ...
    {step==="topic"&&<div>
      ...
      <div onClick={()=>{
        ...
        onBrowse(selSubject,selStage,"",isFree);onClose();
      }} style={{...C.card,cursor:"pointer",display:"flex",alignItems:"center",gap:"12px",marginBottom:"8px",border:"1px solid rgba(56,189,248,0.3)"}}>
      ...
      {availableTopics.length===0
        ...
        :availableTopics.map((topic,i)=>{
          const isFree=isFreeSubject(globalPrices,selSubject,selStage);
          const topicClips=clips.filter(c=>c.stage===selStage&&c.subject===selSubject&&c.topic===topic).length;
          return <div key={topic} onClick={()=>{onBrowse(selSubject,selStage,topic,isFree);onClose();}} style={{...C.card,cursor:"pointer",display:"flex",alignItems:"center",gap:"12px",marginBottom:"8px",border:`1px solid ${isFree?"rgba(52,211,153,0.2)":"rgba(129,140,248,0.2)"}`}}>
            <div style={{width:"44px",height:"44px",borderRadius:"12px",background:"rgba(129,140,248,0.15)",border:"1px solid rgba(129,140,248,0.3)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:"16px",fontWeight:"bold",color:"#818cf8"}}>
              {i+1}
            </div>
            <div style={{flex:1}}>
              <div style={{fontWeight:"bold",fontSize:"14px"}}>{topic}</div>
              <div style={{fontSize:"11px",color:"#71717a"}}>{topicClips} مقطع • {isFree?"🆓 كامل":"💰 أول 5 + عشوائي"}</div>
            </div>
            <span style={{color:isFree?"#34d399":"#818cf8",fontSize:"18px"}}>←</span>
          </div>;
        })
...
export default function App() {
  ...
  return (
    <div style={C.app}>
      ...
      {modal==="browse" &&<BrowseModal onClose={closeModal} clips={clips} globalPrices={globalPrices} onBrowse={(subject,stage,topic,isFree)=>{
        const filtered=clips.filter(c=>
          c.subject===subject && c.stage===stage &&
          (!topic || c.topic===topic)
        ).sort((a,b)=>Number(a.num||0)-Number(b.num||0));
        if(isFree){
          setSelectedSubject({subject,stage,topic,customList:filtered});
        } else {
          ...
          setSelectedSubject({subject,stage,topic,customList:[...first5,...rest,...otherClips]});
        }
        setVideoIdx(0);
