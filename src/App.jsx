  ...
  if (!selClip) return (
    <div>
      ...
      <div style={{backgroundColor:"rgba(56,189,248,0.06)",border:"1px solid rgba(56,189,248,0.15)",borderRadius:"14px",padding:"14px",marginBottom:"14px"}}>
        <div style={{fontSize:"12px",color:"#38bdf8",fontWeight:"bold",marginBottom:"10px",display:"flex",alignItems:"center",gap:"6px"}}><Search size={13}/> فلتر المقاطع</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px",marginBottom:"8px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"8px"}}>
          ...
          {(filterSubj||filterStage||filterGrade)&&<button onClick={()=>{setFilterSubj("");setFilterStage("");setFilterGrade("");}} style={{padding:"8px 12px",background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:"8px",color:"#f87171",fontSize:"12px",cursor:"pointer"}}>مسح</button>}
        </div>
        {(filterSubj||filterStage||filterGrade)&&<div style={{fontSize:"11px",color:"#71717a",marginTop:"8px"}}>
        ...
        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove} style={{...C.fullScreenWrap,userSelect:"none",overscrollBehavior:"none"}}>
          ...
          {showNav&&(
            <div style={C.floatingNav}>
              ...
              {allVideos.length>1&&<>
                <button disabled={videoIdx===0} onClick={()=>{setVideoIdx(i=>i-1);setPlaying(true);}} style={{...C.navItem(false),opacity:videoIdx===0?0.3:1}}><ChevronUp size={20}/><span style={{fontSize:"11px",fontWeight:"bold"}}>السابق</span></button>
                <button disabled={videoIdx===allVideos.length-1} onClick={()=>{setVideoIdx(i=>i+1);setPlaying(true);}} style={{...C.navItem(false),opacity:videoIdx===allVideos.length-1?0.3:1}}><ChevronDown size={20}/><span style={{fontSize:"11px",fontWeight:"bold"}}>التالي</span></button>
              </>}
            </div>
          )}
        </div>
