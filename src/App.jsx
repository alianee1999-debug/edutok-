...
const SAMPLE_VIDEOS = [
  {id:"s1",title:"\u0645\u0642\u062f\u0645\u0629 \u0641\u064a \u0627\u0644\u062c\u0628\u0631",
   teacher:"\u0623. \u0623\u062d\u0645\u062f",subject:"\u0627\u0644\u0631\u064a\u0627\u0636\u064a\u0627\u062a",
   stage:"\u0627\u0644\u0627\u0628\u062a\u062f\u0627\u0626\u064a\u0629",duration:"2:30",
   bg:"linear-gradient(180deg,#0f172a,#1e1b4b)",videoUrl:""},
  {id:"s2",title:"\u0642\u0648\u0627\u0646\u064a\u0646 \u0646\u064a\u0648\u062a\u0646",
   teacher:"\u0623. \u0633\u0627\u0631\u0629",subject:"\u0627\u0644\u0641\u064a\u0632\u064a\u0627\u0621",
   stage:"\u0627\u0644\u0625\u0639\u062f\u0627\u062f\u064a\u0629",duration:"3:15",
   bg:"linear-gradient(180deg,#042f2e,#0d3b2e)",videoUrl:""},
  {id:"s3",title:"\u0627\u0644\u062c\u0647\u0627\u0632 \u0627\u0644\u062a\u0646\u0641\u0633\u064a",
   teacher:"\u0623. \u062e\u0627\u0644\u062f",subject:"\u0627\u0644\u0623\u062d\u064a\u0627\u0621",
   stage:"\u0627\u0644\u0645\u062a\u0648\u0633\u0637\u0629",duration:"4:00",
   bg:"linear-gradient(180deg,#1a1a2e,#16213e)",videoUrl:""}
];
...
const callGroq = async (prompt,imageBase64,imageMime) => {
  ...
  const res=await fetch(GROQ_URL,{
      ...
      body:JSON.stringify({model:"meta-llama/llama-4-scout-17b-16e-instruct",messages:[{role:"user",content:[{type:"image_url",image_url:{url:"data:"+mime+";base64,"+imageBase64}},{type:"text",text:prompt}]}],temperature:0.7,max_tokens:2000})
    });
  const d=await res.json();
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
        {[["واتساب","#25D366"],["تيليغرام","#229ED9"],["نسخ الرابط","#6366f1"],["المزيد","#f97316"]].map(([n,col])=>(
          <button key={n} onClick={()=>{
            if(n==="نسخ الرابط"&&navigator.clipboard) navigator.clipboard.writeText(link);
            onClose();
          }} style={{padding:"12px",borderRadius:"12px",border:"none",backgroundColor:col,color:"#fff",fontSize:"13px",fontWeight:"bold",cursor:"pointer"}}>{n}</button>
        ))}
      </div>
    </div></div>
  );
    ...
    <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(168,85,247,0.2)"}}>
      ...
      <div style={{display:"flex",gap:"8px"}}>
        <input
          ...
          style={{flex:1,padding:"10px 14px",backgroundColor:"#09090b",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"10px",color:"#fff",fontSize:"13px",outline:"none"}}
        />
...
function SolveModal({onClose,video}) {
  ...
  return (
    <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(52,211,153,0.2)"}}>
      ...
      {ans&&(
        <div style={{backgroundColor:"#09090b",borderRadius:"12px",padding:"14px",fontSize:"14px",color:"#e4e4e7",lineHeight:"1.8",marginBottom:"14px",border:"1px solid rgba(52,211,153,0.15)",whiteSpace:"pre-wrap",maxHeight:"240px",overflowY:"auto"}}>
          <div style={{color:"#34d399",fontSize:"11px",fontWeight:"bold",marginBottom:"6px"}}>\u0627\u0644\u062d\u0644:</div>{ans}
        </div>
      )}
      ...
      <div>
        ...
        <div style={{display:"flex",gap:"10px",marginBottom:"12px"}}>
          ...
          <button
            ...
            onClick={()=>setCurSlide(i=>i+1)}
            style={{flex:1,padding:"11px",borderRadius:"12px",border:"none",backgroundColor:curSlide===slides.length-1?"#1c1c1e":"#27272a",color:curSlide===slides.length-1?"#3f3f46":"#fff",cursor:curSlide===slides.length-1?"not-allowed":"pointer",fontWeight:"bold"}}
          >{"\u0627\u0644\u062a\u0627\u0644\u064a"}</button>
        </div>
        ...
        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove} style={{userSelect:"none",overscrollBehavior:"none"}}>
          <div style={{...C.videoWrap,background:video.bg||"linear-gradient(180deg,#0f172a,#1e1b4b)"}}>
            ...
            {showMore&&(
              <div style={C.moreMenu}>
                {[
                  [<FileText size={22} color="#fff"/>,"PDF","pdf"],
                  [<Camera size={22} color="#fff"/>,"حل","solve"],
                  [<Search size={22} color="#fff"/>,"بحث","search"],
                  [<DollarSign size={22} color="#fff"/>,"زين","wallet"]
                ].map(([icon,label,key])=>(
                  <button key={key} style={C.moreItem} onClick={()=>{setModal(key);setShowMore(false);}}>
                    {icon}<span style={{fontSize:"11px",marginTop:"4px"}}>{label}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          <div style={{padding:"0 20px",marginTop:"10px"}}>
            ...
            <div style={{display:"flex",gap:"6px",marginTop:"10px",justifyContent:"center"}}>
              {allVideos.slice(0,8).map((_,i)=>(
                <div key={i} onClick={()=>setVideoIdx(i)} style={{width:i===videoIdx?"20px":"6px",height:"6px",borderRadius:"3px",backgroundColor:i===videoIdx?"#38bdf8":"#3f3f46",cursor:"pointer",transition:"width 0.2s"}}/>
              ))}
            </div>
          </div>
        </div>
        ...
        <div>
          <div style={C.tabsGrid}>
            {ADMIN_TABS.map(({key,label,Icon})=>(
              <button key={key} style={C.tab(adminTab===key)} onClick={()=>{setAdminTab(key);setShowClipForm(false);}}>
                <Icon size={11}/>{label}
              </button>
            ))}
          </div>
          <div style={C.section}>
            ...
            {adminTab==="clips"&&!showClipForm&&(
              <div>
                {confirmDeleteClip&&(
                  <div style={{backgroundColor:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:"14px",padding:"16px",marginBottom:"14px",textAlign:"center"}}>
                    <div style={{color:"#f87171",fontWeight:"bold",marginBottom:"8px"}}>هل تريد حذف هذا المقطع؟</div>
                    <div style={{display:"flex",gap:"8px",justifyContent:"center"}}>
                      <button onClick={()=>doDeleteClip(confirmDeleteClip)} style={{padding:"8px 20px",backgroundColor:"#ef4444",border:"none",borderRadius:"8px",color:"#fff",fontWeight:"bold",cursor:"pointer"}}>نعم، احذف</button>
                ...
                <button style={C.gradBtn} onClick={()=>{setEditingClip(null);setShowClipForm(true);}}>
                  <Plus size={18}/> إضافة مقطع جديد
                </button>
              </div>
            )}
            ...
            {adminTab==="clips"&&showClipForm&&(
              <div>
                <div style={{...C.infoBanner,marginBottom:"12px"}}>
                  <Film size={16}/>
                  <span style={{fontWeight:"bold"}}>{editingClip?"تعديل المقطع":"مقطع جديد"}</span>
                </div>
                <button
                  onClick={resetClipForm}
                  style={{width:"100%",padding:"11px",backgroundColor:"#27272a",color:"#ef4444",border:"1px solid rgba(239,68,68,0.3)",borderRadius:"12px",fontSize:"13px",fontWeight:"bold",cursor:"pointer",marginBottom:"14px",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}
                >
                  <X size={14}/> {"إغلاق"}
                </button>
                <div style={C.twoCol}>
                  <div>
                    <label style={C.label}>المرحلة</label>
                    <select style={C.select} value={clipStage} onChange={e=>{setClipStage(e.target.value);setClipGrade((GRADES[e.target.value]||[])[0]||"");}}>
                      {STAGES.map(s=><option key={s}>{s}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={C.label}>الصف</label>
                    <select style={C.select} value={clipGrade} onChange={e=>setClipGrade(e.target.value)}>
                      {(GRADES[clipStage]||[]).map(g=><option key={g}>{g}</option>)}
                    </select>
                  </div>
                </div>
                <div style={C.twoCol}>
                  <div>
                    <label style={C.label}>المادة</label>
                    <select style={C.select} value={clipSubject} onChange={e=>setClipSubject(e.target.value)}>
                      {SUBJECTS.map(s=><option key={s}>{s}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={C.label}>الفصل</label>
                    <select style={C.select} value={clipSemester} onChange={e=>setClipSemester(e.target.value)}>
                ...
                <div style={C.twoCol}>
                  <div>
                    <label style={C.label}>نوع المقطع</label>
                    <select style={C.select} value={clipType} onChange={e=>setClipType(e.target.value)}>
                      {CLIP_TYPES.map(t=><option key={t}>{t}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={C.label}>رقم المقطع</label>
                    <input type="text" value={clipNum} onChange={e=>setClipNum(e.target.value)} style={C.input} placeholder="01"/>
                  </div>
                </div>
                <label style={C.label}>العنوان</label>
                <input type="text" value={clipTitle} onChange={e=>setClipTitle(e.target.value)} placeholder="عنوان المقطع" style={C.input}/>
                <div style={C.twoCol}>
                  <div>
                    <label style={C.label}>المعلم</label>
                    <input type="text" value={clipTeacher} onChange={e=>setClipTeacher(e.target.value)} placeholder="أ. محمد" style={C.input}/>
                  </div>
                  <div>
                    <label style={C.label}>الموبايل</label>
                    <input type="text" placeholder="07XX..." style={C.input}/>
                  </div>
                </div>
                <label style={C.label}>صورة مصغرة</label>
                <ImageUploader onUpload={url=>setClipThumbUrl(url)} onBase64={()=>{}} color="#a855f7" label="اختر صورة مصغرة"/>
                ...
                <input type="text" placeholder="https://youtube.com/watch?v=..." value={clipVideoUrl} onChange={e=>setClipVideoUrl(e.target.value)} style={C.input}/>
                <div style={C.saveRow}>
              ...
              <div>
                ...
                {students.length===0
                  ? <div style={{textAlign:"center",padding:"40px 20px"}}>
                      ...
                      <span style={{fontSize:"15px",color:"#52525b"}}>لا يوجد طلاب مسجلون بعد</span>
                    </div>
                  : students.map((s,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:"10px",...C.card}}>
                      <div style={{width:42,height:42,borderRadius:"50%",background:"linear-gradient(135deg,#0ea5e9,#a855f7)",display:"flex",justifyContent:"center",alignItems:"center",flexShrink:0}}>
                        <User size={20} color="#fff"/>
                      </div>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:"bold",fontSize:"14px"}}>{s.name}</div>
                        <div style={{color:"#38bdf8",fontSize:"12px"}}>@{s.account}</div>
                        <div style={{color:"#71717a",fontSize:"12px"}}>{s.phone}</div>
                      </div>
            ...
            {adminTab==="prices"&&(
              <div>
                ...
                <button style={{...C.gradBtn,marginTop:"16px"}} onClick={()=>showMsg("تم حفظ الأسعار!")}>
                  <Save size={16}/> حفظ الأسعار
                </button>
              </div>
            )}
            ...
            {adminTab==="notifications"&&(
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"14px"}}>
                  {[["expire","انتهاء الاشتراك"],["new_video","مقطع جديد"],["remind","تذكير"],["offer","عرض خاص"]].map(([k,l])=>(
                    <div key={k} onClick={()=>applyTemplate(k)} style={{...C.card,cursor:"pointer",textAlign:"center",padding:"12px",marginBottom:0}}>{l}</div>
                  ))}
                </div>
                <label style={C.label}>عنوان الإشعار</label>
                <input type="text" value={notifTitle} onChange={e=>setNotifTitle(e.target.value)} placeholder="العنوان" style={C.input}/>
                <label style={C.label}>نص الإشعار</label>
                <textarea rows={3} value={notifBody} onChange={e=>setNotifBody(e.target.value)} style={{...C.input,resize:"none"}}/>
              ...
              <div>
                <div style={{backgroundColor:"#141417",border:"1px solid rgba(234,179,8,0.2)",borderRadius:"14px",padding:"16px",marginBottom:"14px"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:"8px"}}>
                    <span style={{fontWeight:"bold"}}>{ADMIN_PHONE}</span>
                    <span style={{color:"#a1a1aa"}}>الهاتف</span>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    <span style={{letterSpacing:"2px"}}>••••••••</span>
                    <span style={{color:"#a1a1aa"}}>المرور</span>
                  </div>
                </div>
                <div style={C.card}>
                  <span style={{color:"#38bdf8",fontWeight:"bold",fontSize:"13px",display:"block",marginBottom:"10px"}}>تغيير رقم الهاتف</span>
                  <input type="text" placeholder="07XX XXX XXXX" style={C.input}/>
