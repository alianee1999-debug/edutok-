...
import { Bookmark, Share2, Bot, MessageCircle, MoreHorizontal, FileText, Camera, Search, ChevronUp, ChevronDown, Settings, User, Home, Bell, DollarSign, Users, Layers, Film, Sparkles, X, Save, BookOpen, GraduationCap, Plus, Play, Pause, Loader, Key, Copy, CheckCircle, Trash2 } from "lucide-react";
...
// يحسب تاريخ الانتهاء الجديد بعد تفعيل كود: يمدد الاشتراك الحالي (إن وجد ولم ينتهِ) بدل استبداله
const computeExtendedExpiry = (currentSubs,subject,stage,durationDays) => {
  const now = new Date();
  const existing = currentSubs?.[subKey(subject,stage)];
...
const C = {
  ...
  sideBtn:(a)=>({width:"40px",height:"40px",borderRadius:"50%",backgroundColor:a?"rgba(34,211,238,0.2)":"rgba(15,23,42,0.75)",border:a?"1px solid #22d3ee":"1px solid rgba(255,255,255,0.1)",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",cursor:"pointer",color:"#fff",gap:"1px"}),
  sideTxt:(a)=>({fontSize:"9px",color:a?"#22d3ee":"#cbd5e1"}),
...
const MathText = ({text,style}) => {
  ...
  if(!hasMath) return <span style={style}>{raw}</span>;
  return (
    <span style={{...style,unicodeBidi:"plaintext"}}>
      {parts.map((part,i)=>{
        if(part.startsWith("$")&&part.endsWith("$")&&part.length>1){
          ...
          return <span key={i} dir="ltr" style={{unicodeBidi:"isolate",display:"inline-block",verticalAlign:"middle"}} dangerouslySetInnerHTML={{__html:html}}/>;
        }
...
// ─── AI MODAL ────────────────────────────────────────────
function AIModal({onClose,video,currentSlide}) {
  const [q,setQ]=useState(""); const [ans,setAns]=useState(""); const [loading,setLoading]=useState(false);
  // نبني وصف دقيق لمحتوى الشريحة الحالية (العنوان + كل النقاط) حتى يجيب المساعد بناءً على ما يراه الطالب فعلاً، مو بس اسم الدرس العام
  ...
  const ask=async(customQ)=>{
    ...
    if(!question.trim())return; setLoading(true); setAns("");
    try{
      const prompt = "أنت مساعد تعليمي متخصص. الطالب يشاهد درس \""+video.title+"\" في مادة "+video.subject+".\n"
        + slideContext + "\n\n"
        + "سؤال الطالب: "+question+"\n\n"
        + "مهم جداً: أجب بالاعتماد على محتوى الشريحة أعلاه بالضبط (نفس الرموز والخطوات المذكورة فيها)، وليس بشرح عام عن الموضوع. "
        + "اشرح بالتفصيل الكامل: وضّح كل خطوة وسبب استخدامها، اربطها بالمحتوى المعروض، وإن أمكن أضف مثالاً إضافياً بسيطاً يوضح نفس الفكرة. لا تختصر الإجابة إذا كانت تحتاج تفصيلاً ليفهم الطالب فعلاً. أجب بالعربية.\n\n"
        + "قاعدة إلزامية بصيغة الكتابة: أي رمز أو معادلة رياضية يجب إحاطتها بعلامتي $ من الطرفين وكتابتها بصيغة LaTeX (مثال: $x^2$ للأس، $\\frac{1}{2}$ للكسر، $\\sqrt{x}$ للجذر). أي معادلة أو رمز كيميائي يجب كتابته داخل $\\ce{...}$ (مثال: $\\ce{H2O}$، $\\ce{N2 + 3H2 <=> 2NH3}$). لا تكتب أي رمز رياضي أو كيميائي كنص عادي خارج هذه العلامات.";
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(56,189,248,0.2)"}}>
    ...
    {!ans&&!loading&&<div style={{display:"flex",gap:"8px",flexWrap:"wrap",marginBottom:"12px"}}>
    ...
    {ans&&<div style={{backgroundColor:"#09090b",borderRadius:"12px",padding:"14px",fontSize:"14px",color:"#e4e4e7",lineHeight:"1.7",marginBottom:"14px",border:"1px solid rgba(56,189,248,0.15)",whiteSpace:"pre-wrap"}}><div style={{color:"#38bdf8",fontSize:"11px",fontWeight:"bold",marginBottom:"6px"}}> الإجابة:</div><MathText text={ans}/></div>}
      ...
      :files.map(f=>(
        <div key={f.id} style={{...C.card}}>
          <div style={{marginBottom:"8px"}}>
            ...
            <div style={{fontSize:"11px",color:"#71717a",marginTop:"2px"}}>{f.subject} • {f.stage}</div>
          </div>
...
function SolveModal({onClose,video}) {
  ...
  const KATEX_RULE = " قاعدة إلزامية بصيغة الكتابة: أي رمز أو معادلة رياضية يجب إحاطتها بعلامتي $ من الطرفين وكتابتها بصيغة LaTeX (مثال: $x^2$ للأس، $\\frac{1}{2}$ للكسر، $\\sqrt{x}$ للجذر). أي معادلة أو رمز كيميائي يجب كتابته داخل $\\ce{...}$ (مثال: $\\ce{H2O}$). لا تكتب أي رمز رياضي أو كيميائي كنص عادي خارج هذه العلامات.";
  ...
  const solve=async()=>{
    ...
    try{
      let r;
      if(tab==="text"){
        r=await callGemini("أنت مساعد تعليمي. الطالب يدرس "+video.subject+". السؤال: "+q+". حله خطوة بخطوة بالعربية."+KATEX_RULE);
      } else {
        if(!imgB64){setAns("يرجى رفع صورة أولاً.");setLoading(false);return;}
        r=await callGemini("أنت مساعد تعليمي. الطالب يدرس "+video.subject+". انظر للصورة وحل هذا السؤال خطوة بخطوة بالعربية."+KATEX_RULE,imgB64);
      }
      setAns(r||"لم أتمكن من الإجابة.");
...
// ─── SEARCH MODAL ────────────────────────────────────────
// ─── BROWSE MODAL ────────────────────────────────────────
function BrowseModal({onClose, clips, globalPrices, onBrowse}) {
  ...
  const availableSubjects = React.useMemo(()=>{
    ...
    const subjs=new Set();
    // نعرض كل المواد الموجودة في المرحلة بغض النظر عن الصف
    clips.forEach(c=>{ if(c.stage===selStage) subjs.add(c.subject); });
    return [...subjs].sort();
  ...
  const availableTopics = React.useMemo(()=>{
    ...
    clips.forEach(c=>{ if(c.stage===selStage&&c.subject===selSubject&&c.topic) topics.add(c.topic); });
    return [...topics].sort();
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(99,102,241,0.3)"}}>
    <MHead icon={<BookOpen size={20} color="#818cf8"/>} title="تصفح المواد" color="#818cf8" onClose={onClose}/>
    ...
    {step==="grade"&&<div>
      ...
      {(GRADES[selStage]||["الأول","الثاني","الثالث"]).map((g,i)=>{
        const count=clips.filter(c=>c.stage===selStage&&(c.grade===g||!c.grade)).length;
    ...
    {/* خطوة ٤: المبحث */}
    {step==="topic"&&<div>
      <div style={{fontSize:"13px",color:"#71717a",marginBottom:"12px",textAlign:"center"}}>اختر المبحث</div>
      <div onClick={()=>{const isFree=isFreeSubject(globalPrices,selSubject,selStage);onBrowse(selSubject,selStage,selGrade,"",isFree);onClose();}} style={{...C.card,cursor:"pointer",display:"flex",alignItems:"center",gap:"12px",marginBottom:"8px",border:"1px solid rgba(56,189,248,0.3)"}}>
        ...
        <div style={{flex:1}}>
          <div style={{fontWeight:"bold",fontSize:"14px"}}>كل المباحث</div>
          <div style={{fontSize:"11px",color:"#71717a"}}>جميع مقاطع {selSubject}</div>
        </div>
        <span style={{color:"#38bdf8",fontSize:"18px"}}>←</span>
      </div>
      {availableTopics.length===0
        ?<div style={{textAlign:"center",padding:"16px",color:"#52525b",fontSize:"12px"}}>لا توجد مباحث محددة — اضغط "كل المباحث" أعلاه</div>
        :availableTopics.map((topic,i)=>{
          ...
          return <div key={topic} onClick={()=>{onBrowse(selSubject,selStage,selGrade,topic,isFree);onClose();}} style={{...C.card,cursor:"pointer",display:"flex",alignItems:"center",gap:"12px",marginBottom:"8px",border:`1px solid ${isFree?"rgba(52,211,153,0.2)":"rgba(129,140,248,0.2)"}`}}>
            <div style={{width:"44px",height:"44px",borderRadius:"12px",background:"rgba(129,140,248,0.15)",border:"1px solid rgba(129,140,248,0.3)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:"16px",fontWeight:"bold",color:"#818cf8"}}>{i+1}</div>
            <div style={{flex:1}}>
              <div style={{fontWeight:"bold",fontSize:"14px"}}>{topic}</div>
              <div style={{fontSize:"11px",color:"#71717a"}}>{count} مقطع • {isFree?"🆓 كامل":"💰 أول 5 + عشوائي"}</div>
            </div>
            <span style={{color:isFree?"#34d399":"#818cf8",fontSize:"18px"}}>←</span>
          </div>;
        })
...
function SearchModal({onClose,allVideos,onSelectVideo}) {
  const [q,setQ]=useState("");
  const filtered=allVideos.map((v,idx)=>({...v,_idx:idx})).filter(v=>!q||(v.title?.includes(q)||v.subject?.includes(q)||v.teacher?.includes(q)||v.grade?.includes(q)));
  return <div style={C.overlay}><div style={C.modalBox}>
    ...
    {filtered.slice(0,20).map((v,i)=>(
      <div key={i} onClick={()=>{onSelectVideo(v._idx,v);onClose();}} style={{display:"flex",alignItems:"center",gap:"10px",...C.card,cursor:"pointer",marginBottom:"6px"}}>
...
function CodeModal({onClose,student,mySubscriptions}) {
  ...
  const redeem=async()=>{
    ...
    try{
      ...
      // حساب تاريخ الانتهاء الجديد (يمدد الاشتراك الحالي إن وجد وإلا يبدأ من الآن)
      let newExpiry = computeExtendedExpiry(mySubscriptions, data.subject, data.stage, data.durationDays);
      ...
      await addDoc(collection(db,"notifications"),{
        ...
        body:"تم تفعيل/تمديد اشتراكك في "+data.subject+" ("+data.stage+")"+(bonusApplied>0?" (تشمل "+bonusApplied+" يوم هدية ترحيبية 🎁)":"")+". ينتهي في "+newExpiry.toLocaleDateString("ar"),
        targetPhone: student.phone,
        sentAt: serverTimestamp(),
      });
      setResult({subject:data.subject,stage:data.stage,newExpiry,bonusApplied});
    }catch(e){
  ...
  if(result) return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(34,197,94,0.2)",textAlign:"center"}}>
    ...
    <div style={{color:"#a1a1aa",fontSize:"13px",marginBottom:"4px"}}>المادة: <strong style={{color:"#fff"}}>{result.subject} — {result.stage}</strong></div>
...
// ─── CERTIFICATE MODAL (شهادة إتمام مبحث — قابلة للمشاركة) ─
function CertificateModal({onClose,student,cert}) {
  const [copied,setCopied]=useState(false);
  const dateStr = new Date().toLocaleDateString("ar",{year:"numeric",month:"long",day:"numeric"});
  const shareText = "🎓 أنهيت مبحث \""+cert.topic+"\" في مادة "+cert.subject+" ("+cert.stage+") على تطبيق EduTok!\nتاريخ الإنجاز: "+dateStr;
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(251,191,36,0.35)",textAlign:"center"}}>
    <div style={{background:"linear-gradient(135deg,#78350f,#b45309)",borderRadius:"18px",padding:"28px 20px",marginBottom:"16px",border:"1px solid rgba(251,191,36,0.4)"}}>
      ...
      <div style={{fontSize:"12px",color:"rgba(255,255,255,0.7)",marginBottom:"4px"}}>شهادة إتمام مبحث</div>
    ...
    <div style={{fontSize:"13px",color:"#a1a1aa",marginBottom:"16px"}}>🎉 مبروك! أنهيت هذا المبحث بالكامل</div>
...
function SlidesStudio({slidesTheme,setSlidesTheme,onSaveClip,clips}) {
  // يحسب رقم المقطع التالي واسم المبحث المقترح تلقائياً، بناءً على آخر مقطع محفوظ بنفس المادة/المرحلة/الصف
  ...
  // تعبئة تلقائية للمبحث ورقم المقطع كلما تغيّرت المادة/المرحلة/الصف (تبقى قابلة للتعديل يدوياً)
  useEffect(()=>{
  ...
  const saveToFirestore=async()=>{
    ...
    setSaving(true);
    try{
      await addDoc(collection(db,"clips"),{title:clipTitle,subject:clipSubj,stage:clipStage,grade:clipGrade,semester:clipSemester,topic:clipTopic,num:Number(clipNum)||1,slides,theme:slidesTheme,type:"شرائح AI",bg:"linear-gradient(135deg,#1e1b4b,#312e81)",createdAt:serverTimestamp()});
      onSaveClip({title:clipTitle,subject:clipSubj,stage:clipStage,grade:clipGrade,semester:clipSemester,topic:clipTopic,num:Number(clipNum)||1,slides,theme:slidesTheme,type:"شرائح AI",bg:"linear-gradient(135deg,#1e1b4b,#312e81)"});
  ...
  // تعبئة تلقائية للمبحث ورقم بداية المقاطع كلما تغيّرت المادة/المرحلة/الصف (تبقى قابلة للتعديل يدوياً)
  useEffect(()=>{
  ...
  if(mode==="bulk") return <div>{back}
    ...
    <div style={{...C.card,border:"1px solid rgba(34,197,94,0.2)",marginBottom:"12px"}}>
      ...
      <div style={{fontSize:"10px",color:"#71717a",margin:"8px 0 6px"}}>المبحث ورقم بداية المقاطع مُعبّآن تلقائياً حسب آخر مقطع محفوظ بنفس المادة/المرحلة/الصف — تقدر تعدّلهما يدوياً:</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>
        <input value={bulkTopic} onChange={e=>setBulkTopic(e.target.value)} placeholder="المبحث" style={{padding:"8px",backgroundColor:"#09090b",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"8px",color:"#fff",fontSize:"12px"}}/>
  ...
  if(mode==="result"){const sl=slides[curSlide]||{}; return <div style={{paddingBottom:"80px"}}>
    ...
    <div style={{...C.card,border:"1px solid rgba(56,189,248,0.15)"}}>
      ...
      <div style={{...C.twoCol,marginBottom:"10px"}}>
        <input value={clipTopic} onChange={e=>setClipTopic(e.target.value)} placeholder="المبحث (تلقائي، قابل للتعديل)" style={{padding:"10px",backgroundColor:"#09090b",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"10px",color:"#fff",fontSize:"12px",outline:"none"}}/>
      ...
      :requests.map(r=>(
        <div key={r.id} style={{...C.card,border:`1px solid ${r.status==="pending"?"rgba(251,191,36,0.3)":r.status==="approved"?"rgba(74,222,128,0.2)":"rgba(248,113,113,0.2)"}`,marginBottom:"10px"}}>
          ...
          <div style={{display:"flex",gap:"8px"}}>
            ...
            {r.status==="pending"&&<>
              <button onClick={()=>approve(r)} style={{flex:1,padding:"8px",borderRadius:"8px",border:"1px solid rgba(74,222,128,0.3)",background:"rgba(74,222,128,0.1)",color:"#4ade80",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>موافقة ✅</button>
              <button onClick={()=>reject(r)} style={{flex:1,padding:"8px",borderRadius:"8px",border:"1px solid rgba(248,113,113,0.3)",background:"rgba(248,113,113,0.1)",color:"#f87171",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>رفض ❌</button>
            </>}
          </div>
        </div>
      ))
...
function AdminWalletTab() {
  ...
  const approvePayment=async(p)=>{
    ...
    try{
      ...
      await addDoc(collection(db,"notifications"),{title:" تم تفعيل اشتراكك!",body:"تم تفعيل اشتراكك في "+p.subject+" ("+p.stage+") لمدة "+days+" يوم"+(bonusApplied>0?" (تشمل "+bonusApplied+" يوم هدية ترحيبية 🎁)":"")+". ينتهي في "+expiresAt.toLocaleDateString("ar"),targetPhone:p.studentPhone,sentAt:serverTimestamp()});
...
export default function App() {
  ...
  const [newCertificate,setNewCertificate]=useState(null); // {subject,stage,topic}
  ...
  const [clipTopic,setClipTopic]=useState(""); // المبحث
  ...
  useEffect(()=>{
    ...
    const unsub=onSnapshot(collection(db,"subscriptions"),snap=>{
      ...
      snap.docs.forEach(d=>{
        ...
        const key=subKey(s.subject,s.stage);
        // لو فيه أكثر من سجل اشتراك لنفس المادة (بسبب تجديد أو تفعيل كود)
  ...
  useEffect(()=>{
    if(role!=="student"||!currentStudent?.phone){setStudentProgress({});setWatchedClipIds([]);setCertifiedTopics([]);return;}
    const unsub=onSnapshot(doc(db,"progress",currentStudent.phone),snap=>{
      if(snap.exists()){
        ...
        setStudentProgress({});setWatchedClipIds([]);setCertifiedTopics([]);
      }
    });
    return ()=>unsub();
  },[role,currentStudent]);
  ...
  useEffect(()=>{
    ...
    const updateStreak=async()=>{
      ...
      if(!snap.exists()){
        // أول دخول
        await setDoc(ref,{days:1,lastDate:today,maxDays:1});
      ...
      if(data.lastDate===today){
        // دخل اليوم من قبل
        setStreak({days:data.days,lastDate:data.lastDate,newRecord:false});
        return;
      }
      const yesterday=new Date();
      yesterday.setDate(yesterday.getDate()-1);
      const isConsecutive=data.lastDate===yesterday.toDateString();
      const newDays=isConsecutive?data.days+1:1;
      const newMax=Math.max(newDays,data.maxDays||0);
      const isNewRecord=newDays>=(data.maxDays||0)&&newDays>1;
      await setDoc(ref,{days:newDays,lastDate:today,maxDays:newMax},{merge:true});
      setStreak({days:newDays,lastDate:today,newRecord:isNewRecord,maxDays:newMax});
    };
    updateStreak().catch(()=>{});
  },[role,currentStudent]);
  ...
  const allVideos = React.useMemo(()=>{
    ...
    if(selectedSubject){
      ...
      const filtered=firebaseClips.filter(c=>
      ...
      if(selectedSubject.isFree===false){
        // مدفوعة: أول 5 بالتسلسل ثم الباقي عشوائي
        const first5=filtered.slice(0,5);
        const rest=filtered.slice(5).sort(()=>Math.random()-0.5);
        return [...first5,...rest];
      }
      return filtered;
    }
    ...
    // طالب: تقسيم المقاطع حسب سعر كل مادة
    const grouped={};
    ...
    const orderedClips=[];   // مواد مجانية — بالتسلسل
    const randomClips=[];    // مواد مدفوعة — عشوائية

    Object.entries(grouped).forEach(([key,group])=>{
      const [subject,stage]=key.split("__");
      const isFree=isFreeSubject(globalPrices,subject,stage);
      if(isFree){
        // مجانية → بالتسلسل
        const sorted=[...group].sort((a,b)=>Number(a.num||0)-Number(b.num||0));
        orderedClips.push(...sorted);
      } else {
        // مدفوعة → عشوائية (10 فقط)
        const shuffled=[...group].sort(()=>Math.random()-0.5);
        randomClips.push(...shuffled.slice(0,10));
      }
    });

    // المجانية أولاً بالتسلسل، ثم المدفوعة عشوائياً
    return [...orderedClips, ...randomClips.sort(()=>Math.random()-0.5)];

  },[clips, selectedSubject, role, globalPrices]);
  ...
  const doRegister=async()=>{
    ...
    try{
      ...
      const code = regPartnerCode.trim().toUpperCase();
      if(code){
        const campSnap = await getDoc(doc(db,"campaignCodes",code));
        if(!campSnap.exists()){
          return setRegErr("كود الشراكة غير صحيح");
        }
        const camp = campSnap.data();
        if(!camp.active) return setRegErr("كود الشراكة موقوف حالياً");
        if(new Date(camp.expiresAt) < new Date()) return setRegErr("انتهت صلاحية كود الشراكة");
        if((camp.usedCount||0) >= (camp.maxUses||0)) return setRegErr("وصل كود الشراكة للحد الأقصى من الطلاب");
      ...
      const stored = {name:regName.trim(),phone:phoneKey,account:regAccount.trim(),stage:regStage,grade:regGrade,accountType:regAccountType,passHash:hash,passSalt:salt,createdAt:serverTimestamp()};
      if(campaignInfo){
      ...
      // نزيد عداد الاستخدام لكود الشراكة (بعد التأكد من نجاح إنشاء الحساب)
      if(campaignInfo){
        try{ await updateDoc(doc(db,"campaignCodes",campaignInfo.code),{usedCount:campaignInfo.usedCount+1}); }catch{}
      }
      const s = {name:stored.name,phone:stored.phone,account:stored.account,stage:stored.stage,grade:stored.grade,accountType:stored.accountType||"student"};
  ...
  const doLogin=async()=>{
    ...
    try{
      ...
      const ex = snap.data();
      if(ex.banned) return setLoginErr("🚫 هذا الحساب محظور. تواصل مع المدير");
      const ok = await verifyPassword(loginPass.trim(), ex.passHash, ex.passSalt);
      if(ok){
        const s = {name:ex.name,phone:ex.phone,account:ex.account,stage:ex.stage||"الابتدائية",grade:ex.grade||"الأول",accountType:ex.accountType||"student"};
        setCurrentStudent(s);setRole("student");setScreen("home");saveSession(s,"student");
  ...
  const getAutoNum = (subject, stage, grade) => {
    const similar = clips.filter(c=>
      c.subject===subject && c.stage===stage && (!grade||c.grade===grade)
    );
    if(similar.length===0) return "01";
    const maxNum = Math.max(...similar.map(c=>Number(c.num)||0));
  ...
  const saveClip=async()=>{
    ...
    try{
      const data={
        ...
        semester:clipSemester,topic:clipTopic,type:clipType,num:clipNum,teacher:clipTeacher,
  ...
  const saveProgress = (idx) => {
    ...
    if(!isSubscribed(mySubscriptions,v.subject,v.stage)) return;
  ...
  const markClipWatched = (v) => {
    ...
    if(newWatched!==watchedClipIds) setWatchedClipIds(newWatched);

    let newCertified = certifiedTopics;
    // نتحقق فقط للمقاطع اللي فيها "مبحث" محدد (بدون مبحث ما نقدر نحدد "فصل مكتمل")
    if(v.topic){
      const topicKey = v.subject+"__"+v.stage+"__"+v.topic;
      const topicClips = clips.filter(c=>c.subject===v.subject && c.stage===v.stage && c.topic===v.topic);
      const allWatched = topicClips.length>0 && topicClips.every(c=>newWatched.includes(c.id));
      if(allWatched && !certifiedTopics.includes(topicKey)){
        newCertified = [...certifiedTopics, topicKey];
        setCertifiedTopics(newCertified);
        setNewCertificate({subject:v.subject, stage:v.stage, topic:v.topic});
      }
    }
  ...
  return (
    <div style={C.app}>
        ...
        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove} style={{...C.fullScreenWrap,userSelect:"none",overscrollBehavior:"none"}}>
          ...
          <div style={C.sidebar}>
            {[
              ...
              [<BookOpen size={18} color="#fff"/>,"تصفح",()=>{setModal("browse");setPlaying(false);},false],
          ...
          {showMore&&<div style={C.moreMenu}>
            {[[<Camera size={22} color="#fff"/>,"حل ذكي","solve"],[<Search size={22} color="#fff"/>,"البحث","search"],[<DollarSign size={22} color="#fff"/>,"زين كاش","wallet"],[<Key size={22} color="#fff"/>,"الكود","code"]].map(([icon,label,key])=>(
          ...
          <div style={C.section}>
              ...
              <div>
                  ...
                  return <>
                    <div style={{backgroundColor:"rgba(139,92,246,0.06)",border:"1px solid rgba(139,92,246,0.15)",borderRadius:"14px",padding:"12px",marginBottom:"12px"}}>
                      ...
                      {(cSearch||cSubj||cStage)&&<div style={{fontSize:"11px",color:"#71717a",marginTop:"6px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        ...
                        <button onClick={()=>{setCSearch("");setCSubj("");setCStage("");}} style={{background:"none",border:"none",color:"#f87171",fontSize:"11px",cursor:"pointer"}}>مسح ✕</button>
                      </div>}
                    </div>
                    ...
                    {filtered.map((clip,i)=>(
                      <div key={clip.id||i} style={{...C.card,border:"1px solid rgba(139,92,246,0.2)"}}>
                ...
                <div style={{textAlign:"center",padding:"16px 0 10px"}}>
                  ...
                  <button style={C.gradBtn} onClick={()=>{
                    setEditingClip(null);
                    const autoNum=getAutoNum(clipSubject,clipStage,clipGrade);
                    setClipNum(autoNum);
                    setShowClipForm(true);
            ...
            {adminTab==="clips"&&showClipForm&&(
              <div>
                ...
                <div style={C.twoCol}>
                  <div><label style={C.label}>المرحلة</label><select style={C.select} value={clipStage} onChange={e=>{const s=e.target.value;const g=(GRADES[s]||[])[0]||"";setClipStage(s);setClipGrade(g);if(!editingClip)setClipNum(getAutoNum(clipSubject,s,g));}}>{STAGES.map(s=><option key={s}>{s}</option>)}</select></div>
                  <div><label style={C.label}>الصف</label><select style={C.select} value={clipGrade} onChange={e=>{setClipGrade(e.target.value);if(!editingClip)setClipNum(getAutoNum(clipSubject,clipStage,e.target.value));}}>{(GRADES[clipStage]||[]).map(g=><option key={g}>{g}</option>)}</select></div>
                </div>
                <div style={C.twoCol}>
                  <div><label style={C.label}>المادة</label><select style={C.select} value={clipSubject} onChange={e=>{setClipSubject(e.target.value);if(!editingClip)setClipNum(getAutoNum(e.target.value,clipStage,clipGrade));}}>{SUBJECTS.map(s=><option key={s}>{s}</option>)}</select></div>
                ...
                <label style={C.label}>المبحث (اختياري)</label><input type="text" value={clipTopic} onChange={e=>setClipTopic(e.target.value)} placeholder="مثال: الخلية، المعادلات التربيعية..." style={C.input}/>
                <div style={C.twoCol}>
      ...
      {/* MODALS */}
      {modal==="ai"     &&<AIModal      onClose={closeModal} video={video} currentSlide={video.slides?.[currentSlideIdx]}/>}
      ...
      {modal==="search" &&<SearchModal  onClose={closeModal} allVideos={allVideos} onSelectVideo={(idx)=>{setVideoIdx(idx);setCurrentSlideIdx(0);setModal(null);setScreen("home");}}/>}
      {modal==="browse" &&<BrowseModal onClose={closeModal} clips={clips} globalPrices={globalPrices} onBrowse={(subject,stage,grade,topic,isFree)=>{
      ...
      {modal==="description" &&<VideoDescriptionModal onClose={closeModal} video={video} role={role} mySubscriptions={mySubscriptions} globalPrices={globalPrices} onOpenWallet={()=>setModal("wallet")} onSelectSubject={(subject,stage)=>{setSelectedSubject({subject,stage});const key=subKey(subject,stage);setVideoIdx(studentProgress[key]||0);setCurrentSlideIdx(0);}} videoIdx={videoIdx} totalVideos={allVideos.length}/>}
      ...
      {newCertificate   &&<CertificateModal onClose={()=>setNewCertificate(null)} student={currentStudent} cert={newCertificate}/>}
