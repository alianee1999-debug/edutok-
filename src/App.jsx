...
// ─── CEREBRAS AI (عبر السيرفر، المفتاح غير مكشوف بالمتصفح) ──
// ملاحظة: نماذج Cerebras الحالية نصّية فقط، لكن نمرر imageBase64/imageMime هنا
// بنفس الشكل لضمان توافق التوقيع (signature) مع باقي دوال callX — السيرفر
// يتجاهل الصورة تلقائياً لو وصلت (راجع api/cerebras.js)
const callCerebras = async (prompt, imageBase64=null, imageMime="image/jpeg") => {
  ...
  if(d.error) throw new Error(d.error||"خطأ بالاتصال بـ Cerebras");
  return d.text || "";
};

// ─── DEEPSEEK AI (عبر السيرفر، المفتاح غير مكشوف بالمتصفح) ──
// ملاحظة: نماذج DeepSeek الحالية عبر الـ API نصّية فقط، لكن نمرر imageBase64/imageMime
// هنا بنفس الشكل لضمان توافق التوقيع (signature) مع باقي دوال callX — السيرفر
// يتجاهل الصورة تلقائياً لو وصلت (راجع api/deepseek.js)
const callDeepSeek = async (prompt, imageBase64=null, imageMime="image/jpeg") => {
  ...
  if(d.error) throw new Error(d.error||"خطأ بالاتصال بـ DeepSeek");
  return d.text || "";
};
...
//
// ملاحظة مهمة: الصور (OCR) تحتاج مزود يدعم الرؤية. Cerebras وDeepSeek حالياً
...
const AI_PROVIDERS = [
  ...
  { name: "Cerebras", fn: callCerebras, supportsImage: false },
  { name: "DeepSeek", fn: callDeepSeek, supportsImage: false },
];
...
const callAI = async (prompt, imageBase64=null, imageMime="image/jpeg") => {
  ...
  for(let i=0; i<order.length; i++){
    try{
      ...
      // لو الخطأ مو بسبب Quota/ازدحام (يعني خطأ حقيقي بالمفتاح/الموديل)، ولسه
      // فيه مزودين تانيين بالقائمة، نكمل نجرب بدل ما نوقف — الاحتياطي هنا أشمل
      // من السلوك القديم عمداً، لأن هدفنا الأساسي الآن ضمان استمرارية الخدمة
      if(isLast) throw lastError;
      // نكمل للمزود التالي بالترتيب العشوائي
    }
  }
  throw lastError; // احتياطي نظري، ما المفروض نوصله أبداً
};
...
function AIDiagnosticsCard() {
  ...
  const runTest=async()=>{
    setTesting(true);
    setResults(AI_PROVIDERS.map(p=>({name:p.name,status:"جارٍ الاختبار..."})));
    const testPrompt="أجب بكلمة واحدة فقط: تم";
    const settled=await Promise.all(AI_PROVIDERS.map(async p=>{
      const started=Date.now();
      try{
        await p.fn(testPrompt);
        return {name:p.name,ok:true,ms:Date.now()-started};
      }catch(e){
        return {name:p.name,ok:false,ms:Date.now()-started,error:(e?.message||"خطأ غير معروف").slice(0,120)};
      }
    }));
    setResults(settled);
    setTesting(false);
...
export default function App() {
  ...
  useEffect(()=>{
    if(role!=="student"||!currentStudent?.phone){setStudentProgress({});setWatchedClipIds([]);setCertifiedTopics([]);setExamScores({});setXp(0);return;}
    const unsub=onSnapshot(doc(db,"progress",currentStudent.phone),snap=>{
      if(snap.exists()){
        ...
        setStudentProgress({});setWatchedClipIds([]);setCertifiedTopics([]);setExamScores({});setXp(0);
      }
    });
    return ()=>unsub();
  },[role,currentStudent]);
