...
import { getFirestore, initializeFirestore, persistentLocalCache, persistentMultipleTabManager, collection, addDoc, onSnapshot, serverTimestamp, deleteDoc, updateDoc, setDoc, getDoc, getDocs, doc, query, where, orderBy, limit, arrayUnion, arrayRemove } from "firebase/firestore";
...
// ─── استدعاء ذكي بخيار احتياطي تلقائي (Gemini أولاً، وإذا رجع خطأ "تجاوز الحصة/الازدحام" ينتقل تلقائياً لـ Groq) ───
// هذا يمنع توقف ميزات الذكاء الاصطناعي بالتطبيق (زكي، توليد الأسئلة، الشرائح) بالكامل
// بسبب امتلاء الحصة المجانية لـ Gemini، بدون ما يحتاج الطالب/المدير يعرف أو يتدخل
const callAI = async (prompt, imageBase64=null, imageMime="image/jpeg") => {
  try{
    return await callGemini(prompt, imageBase64, imageMime);
  }catch(e){
    const msg = (e.message||"").toLowerCase();
    const isQuotaOrOverload = msg.includes("quota") || msg.includes("rate limit") || msg.includes("429")
      || msg.includes("resource_exhausted") || msg.includes("high demand") || msg.includes("overloaded");
    if(isQuotaOrOverload){
      return await callGroq(prompt, imageBase64, imageMime); // لو فشل هذا كمان، الخطأ يطلع للمستخدم بشكل طبيعي
    }
    throw e;
  }
};
...
function AIModal({onClose,video,currentSlide}) {
  ...
  const ask=async(customQ, isContinue)=>{
    ...
    try{
      ...
      setMessages(cur=>[...cur, {role:"assistant", text:"حدث خطأ: "+e.message}]);
    }
    setLoading(false);
  };
  ...
  const sendNotif=async()=>{
    ...
    try{
      ...
      if(notifTarget==="single"){
        d = await sendNotification({phone:notifStudent.phone,title:notifTitle,body:notifBody});
      }else{
        d = await sendNotification({phone:null,title:notifTitle,body:notifBody});
      }
      // 🔧 تشخيص مؤقت
      if(d && d.ok){ showMsg("🔧 تشخيص: sentTo="+d.sentTo+" | pushSent="+d.pushSent); }
      else { showMsg("🔧 تشخيص: فشل — "+(d?.error||"رد غير متوقع")); }
