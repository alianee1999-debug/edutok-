  ...
  return <div>
    ...
    {pdfs.map(f=>(
      <div key={f.id} style={{...C.card,border:"1px solid rgba(249,115,22,0.2)"}}>
        ...
        <div style={{display:"flex",gap:"8px"}}>
          <button onClick={()=>{
            const newName=window["prompt"]("اسم الملف الجديد:",f.name);
            if(newName&&newName.trim()&&f.id){
              updateDoc(doc(db,"pdfs",f.id),{name:newName.trim()})
                .then(()=>alert("✅ تم تعديل الاسم!"))
                .catch(e=>alert("فشل: "+e.message));
            }
          }} style={{flex:1,padding:"7px",borderRadius:"8px",border:"1px solid rgba(56,189,248,0.3)",backgroundColor:"rgba(56,189,248,0.1)",color:"#38bdf8",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>✏️ تعديل</button>
          <button onClick={()=>{
            if(window["confirm"]("هل تريد حذف هذا الملف نهائياً؟")){
              if(f.id) deleteDoc(doc(db,"pdfs",f.id)).then(()=>alert("✅ تم الحذف!")).catch(e=>alert("فشل: "+e.message));
            }
          }} style={{flex:1,padding:"7px",borderRadius:"8px",border:"1px solid rgba(239,68,68,0.3)",backgroundColor:"rgba(239,68,68,0.1)",color:"#f87171",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>🗑️ حذف</button>
        </div>
      </div>
    ))}
...
export default function App() {
  ...
  const [editingClip,setEditingClip]=useState(null); // null = add mode, clip object = edit mode
                ...
                {clips.map((clip,i)=>(
                  <div key={i} style={{...C.card,border:"1px solid rgba(139,92,246,0.2)"}}>
                    ...
                    <div style={{display:"flex",gap:"8px",marginTop:"6px"}}>
                      ...
                      <button onClick={()=>{
                        if(window["confirm"]("هل تريد حذف هذا المقطع نهائياً؟")){
                          if(clip.id){
                            deleteDoc(doc(db,"clips",clip.id))
                              .then(()=>alert("✅ تم حذف المقطع!"))
                              .catch(e=>alert("فشل: "+e.message));
                          }
                        }
                      }} style={{flex:1,padding:"8px",borderRadius:"10px",border:"1px solid rgba(239,68,68,0.3)",backgroundColor:"rgba(239,68,68,0.1)",color:"#f87171",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>
                        🗑️ حذف
                      </button>
                    </div>
                  </div>
