...
import { Bookmark, Share2, Bot, MessageCircle, MoreHorizontal, FileText, Camera, Search, ChevronUp, ChevronDown, Settings, User, Home, Bell, DollarSign, Users, Layers, Film, Sparkles, X, Save, BookOpen, GraduationCap, Plus, Play, Loader, Trash2, Edit3, Check } from "lucide-react";
...
const C = {
  ...
  confirmBox:{backgroundColor:"#1c1c1f",border:"1px solid rgba(239,68,68,0.4)",borderRadius:"20px",padding:"24px",margin:"0 16px 14px",textAlign:"center"},
  editBadge:{display:"inline-flex",alignItems:"center",gap:"5px",backgroundColor:"rgba(56,189,248,0.15)",border:"1px solid rgba(56,189,248,0.3)",borderRadius:"8px",padding:"3px 10px",fontSize:"11px",color:"#38bdf8",marginBottom:"12px"},
};
...
const ConfirmDelete = ({title, subtitle, onConfirm, onCancel}) => (
  <div style={C.confirmBox}>
    <div style={{width:48,height:48,borderRadius:"50%",backgroundColor:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 12px"}}>
      <Trash2 size={22} color="#f87171"/>
    </div>
    <div style={{color:"#f87171",fontWeight:"bold",fontSize:"15px",marginBottom:"6px"}}>تأكيد الحذف</div>
    <div style={{color:"#e4e4e7",fontSize:"13px",fontWeight:"bold",marginBottom:"4px"}}>{title}</div>
    {subtitle&&<div style={{color:"#71717a",fontSize:"12px",marginBottom:"16px"}}>{subtitle}</div>}
    <div style={{display:"flex",gap:"10px",marginTop:"16px"}}>
      <button onClick={onCancel} style={{flex:1,padding:"12px",backgroundColor:"#27272a",border:"none",borderRadius:"12px",color:"#a1a1aa",fontWeight:"bold",fontSize:"14px",cursor:"pointer"}}>إلغاء</button>
      <button onClick={onConfirm} style={{flex:1,padding:"12px",backgroundColor:"#ef4444",border:"none",borderRadius:"12px",color:"#fff",fontWeight:"bold",fontSize:"14px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}><Trash2 size={15}/>
        احذف
      </button>
    </div>
  </div>
...
const VideoPlayer = ({video, playing, onClick}) => {
      ...
      <div style={{position:"absolute",inset:0,zIndex:2,background:ts.bg,display:"flex",flexDirection:"column",justifyContent:"center",padding:"20px 16px",overflowY:"auto"}} onClick={onClick}>
        ...
        <div style={{display:"flex",gap:"10px",marginTop:"14px"}}>
          <button
            ...
            style={{flex:1,padding:"10px",borderRadius:"12px",border:"none",backgroundColor:slideIdx===0?"rgba(255,255,255,0.05)":"rgba(255,255,255,0.15)",color:slideIdx===0?"rgba(255,255,255,0.3)":"#fff",cursor:slideIdx===0?"not-allowed":"pointer",fontWeight:"bold",fontSize:"13px"}}
          >
            السابق
          </button>
          <button
  ...
  if(video.videoUrl) return (
    <video src={video.videoUrl} autoPlay={playing} loop muted playsInline style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",zIndex:2}}/>
  );
  return (
    <>
...
function AIModal({onClose,video}) {
  ...
  const ask=async()=>{
    ...
    try{
      const prompt1 = "أنت مساعد تعليمي. الطالب يشاهد درس: "
        +video.title+" في مادة "+video.subject
        +". سؤاله: "+q+". أجب بإيجاز وبالعربية.";
      const r=await callGemini(prompt1);
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(56,189,248,0.2)"}}>
    ...
    {ans&&<div style={{backgroundColor:"#09090b",borderRadius:"12px",padding:"14px",fontSize:"14px",color:"#e4e4e7",lineHeight:"1.7",marginBottom:"14px",border:"1px solid rgba(56,189,248,0.15)",whiteSpace:"pre-wrap"}}><div style={{color:"#38bdf8",fontSize:"11px",fontWeight:"bold",marginBottom:"6px"}}>
      الإجابة:
    </div>{ans}</div>}
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(249,115,22,0.2)"}}>
    ...
    :files.map(f=>(
      <div key={f.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",...C.card}}>
        ...
        {isSubbed?<a href={f.url} target="_blank" rel="noreferrer" style={{backgroundColor:"rgba(249,115,22,0.15)",border:"1px solid rgba(249,115,22,0.3)",borderRadius:"8px",padding:"6px 12px",color:"#f97316",fontSize:"12px",cursor:"pointer",fontWeight:"bold",textDecoration:"none"}}>
          تحميل
        </a>
...
function SolveModal({onClose,video}) {
  ...
  const solve=async()=>{
    ...
    try{
      let r;
      if(tab==="text"){
        const p2="أنت مساعد تعليمي. الطالب يدرس "
          +video.subject+". السؤال: "+q
          +". حله خطوة بخطوة بالعربية.";
        r=await callGemini(p2);
      } else{
        if(!imgB64){
          setAns("يرجى رفع صورة أولاً.");
          setLoading(false);return;
        }
        const p3="أنت مساعد تعليمي. الطالب يدرس "
          +video.subject+". انظر للصورة وحل هذا السؤال خطوة بخطوة بالعربية.";
        r=await callGemini(p3,imgB64);
      }
      setAns(r||"\u0644\u0645 \u0623\u062a\u0645\u0643\u0646 \u0645\u0646 \u0627\u0644\u0625\u062c\u0627\u0628\u0629.");
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(52,211,153,0.2)"}}>
    ...
    <div style={{display:"flex",gap:"8px",marginBottom:"14px"}}>
      <button onClick={()=>setTab("text")} style={{flex:1,padding:"10px",borderRadius:"10px",border:"none",backgroundColor:tab==="text"?"#34d399":"#27272a",color:tab==="text"?"#000":"#a1a1aa",fontWeight:"bold",fontSize:"13px",cursor:"pointer"}}>
        اكتب السؤال
      </button>
      <button onClick={()=>setTab("img")} style={{flex:1,padding:"10px",borderRadius:"10px",border:"none",backgroundColor:tab==="img"?"#34d399":"#27272a",color:tab==="img"?"#000":"#a1a1aa",fontWeight:"bold",fontSize:"13px",cursor:"pointer"}}>
        صوّر السؤال
      </button>
    </div>
    {tab==="text" && (
          <textarea
            ...
            placeholder="مثال: احسب مساحة مثلث"
    ...
    {ans&&<div style={{backgroundColor:"#09090b",borderRadius:"12px",padding:"14px",fontSize:"14px",color:"#e4e4e7",lineHeight:"1.8",marginBottom:"14px",border:"1px solid rgba(52,211,153,0.15)",whiteSpace:"pre-wrap",maxHeight:"240px",overflowY:"auto"}}><div style={{color:"#34d399",fontSize:"11px",fontWeight:"bold",marginBottom:"6px"}}>
      الحل:
    </div>{ans}</div>}
...
function AdminLoginModal({onClose,onSuccess}) {
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,maxWidth:"340px",border:"1px solid rgba(234,179,8,0.2)"}}>
    <div style={{textAlign:"center",marginBottom:"20px"}}><GraduationCap size={40} color="#eab308" style={{margin:"0 auto 8px"}}/><h3 style={{color:"#eab308",fontWeight:"bold",fontSize:"18px",margin:"0 0 4px"}}>دخول المدير</h3></div>
    <label style={C.label}> رقم الهاتف</label><input type="text" value={phone} onChange={e=>{setPhone(e.target.value);setErr("");}} placeholder="07XX XXX XXXX" style={C.input}/>
    <label style={C.label}> كلمة المرور</label><input type="password" value={pass} onChange={e=>{setPass(e.target.value);setErr("");}} placeholder="\u0643\u0644\u0645\u0629 \u0627\u0644\u0645\u0631\u0648\u0631" style={C.input} onKeyDown={e=>e.key==="Enter"&&login()}/>
    <ErrBox msg={err}/>
...
function WalletModal({onClose,student,subscriptions}) {
  ...
  const sendPayment=async()=>{
    ...
    setSending(true);
    try{
      await addDoc(collection(db,"payments"),{studentName:student?.name||"",studentPhone:student?.phone||"",studentId:student?.id||"",subject:selSubject,stage:selStage,duration:selDuration.days,durationLabel:selDuration.label,amount,receiptUrl:receipt||"",status:"pending",createdAt:serverTimestamp()});
      setSent(true);
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(34,197,94,0.2)"}}>
    ...
    <label style={C.label}> المادة</label><select value={selSubject} onChange={e=>setSelSubject(e.target.value)} style={C.select}>{SUBJECTS.map(s=><option key={s}>{s}</option>)}</select>
    <label style={C.label}> المرحلة</label><select value={selStage} onChange={e=>setSelStage(e.target.value)} style={C.select}>{STAGES.map(s=><option key={s}>{s}</option>)}</select>
    ...
    <div style={{display:"flex",gap:"6px",marginBottom:"14px"}}>
      {DURATIONS.map(d => (
        <button
          ...
          style={{flex:1,padding:"9px 4px",borderRadius:"10px",border:"none",backgroundColor:selDuration.days===d.days?"#4ade80":"#27272a",color:selDuration.days===d.days?"#000":"#a1a1aa",fontWeight:"bold",fontSize:"10px",cursor:"pointer"}}
        >
          {d.label}
        </button>
      ))}
    </div>
    <label style={C.label}> المبلغ المحوّل (د.ع)</label><input type="number" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="\u0623\u062f\u062e\u0644 \u0627\u0644\u0645\u0628\u0644\u063a" style={C.input}/>
...
// ══════════════════════════════════════════════════════════
// ─── SLIDES STUDIO (with Edit/Delete per slide) ──────────
// ══════════════════════════════════════════════════════════
function SlidesStudio({slidesTheme,setSlidesTheme,onSaveClip}) {
  ...
  // ── NEW: Edit slide state ──
  const [editingSlideIdx,setEditingSlideIdx]=useState(null);
  const [editSlideTitle,setEditSlideTitle]=useState("");
  const [editSlidePoints,setEditSlidePoints]=useState([]);
  // ── NEW: Delete slide confirm ──
  const [confirmDeleteSlideIdx,setConfirmDeleteSlideIdx]=useState(null);
  // ── NEW: Saved clips from Firestore ──
  const [savedClips,setSavedClips]=useState([]);
  const [showSavedClips,setShowSavedClips]=useState(false);
  const [editingClipDoc,setEditingClipDoc]=useState(null);
  const [confirmDeleteClipDoc,setConfirmDeleteClipDoc]=useState(null);

  useEffect(()=>{
    const unsub=onSnapshot(collection(db,"clips"),snap=>{
      const slideClips=snap.docs.map(d=>({id:d.id,...d.data()})).filter(c=>c.type==="\u0634\u0631\u0627\u0626\u062d AI");
      setSavedClips(slideClips);
    });
    return()=>unsub();
  },[]);
  ...
  const generate=async(fromImage)=>{
    ...
    try{
      ...
      if(fromImage){
        ...
        const p4 = "\u0623\u0646\u0634\u0626 " + slidesCount
          + " \u0634\u0631\u0627\u0626\u062d \u062a\u0639\u0644\u064a\u0645\u064a\u0629 \u0639\u0646 \u0627\u0644\u0645\u0648\u0636\u0648\u0639: " + topic
          + ". \u0627\u062c\u0639\u0644 \u0643\u0644 \u0634\u0631\u064a\u062d\u0629 3-4 \u0646\u0642\u0627\u0637 \u0648\u0627\u0636\u062d\u0629." + JSONSUFFIX;
        raw = await callGemini(p4);
        raw = await callGemini(imgPrompt, imgB64);
      }else{
        if(!topic.trim()){showMsg("\u064a\u0631\u062c\u0649 \u0625\u062f\u062e\u0627\u0644 \u0627\u0644\u0645\u0648\u0636\u0648\u0639");setLoading(false);return;}
        const p4="أنشئ "+slidesCount
          +" شرائح تعليمية احترافية عن الموضوع التالي: "+topic
          +". اجعل كل شريحة تحتوي على 3-4 نقاط مفيدة وواضحة."+JSONSUFFIX;
        raw=await callGemini(p4);
      }
  ...
  // ── NEW: Open edit for a slide ──
  const openEditSlide=(idx)=>{
    setEditingSlideIdx(idx);
    setEditSlideTitle(slides[idx].title||"");
    setEditSlidePoints([...(slides[idx].points||[])]);
  };

  // ── NEW: Save edited slide ──
  const saveEditedSlide=()=>{
    const updated=[...slides];
    updated[editingSlideIdx]={title:editSlideTitle,points:editSlidePoints.filter(p=>p.trim())};
    setSlides(updated);
    setEditingSlideIdx(null);
    showMsg(" \u062a\u0645 \u062a\u0639\u062f\u064a\u0644 \u0627\u0644\u0634\u0631\u064a\u062d\u0629");
  };
  ...
  const deleteSlide=(idx)=>{
    ...
    setSlides(updated);
    if(curSlide>=updated.length) setCurSlide(Math.max(0,updated.length-1));
    setConfirmDeleteSlideIdx(null);
    showMsg("\u062a\u0645 \u062d\u0630\u0641 \u0627\u0644\u0634\u0631\u064a\u062d\u0629");
  };

  // ── NEW: Add empty slide ──
  const addSlide=()=>{
    const updated=[...slides,{title:"\u0634\u0631\u064a\u062d\u0629 \u062c\u062f\u064a\u062f\u0629",points:["\u0646\u0642\u0637\u0629 \u0623\u0648\u0644\u0649","\u0646\u0642\u0637\u0629 \u062b\u0627\u0646\u064a\u0629"]}];
    setSlides(updated);
    setCurSlide(updated.length-1);
    showMsg(" \u062a\u0645\u062a \u0625\u0636\u0627\u0641\u0629 \u0634\u0631\u064a\u062d\u0629 \u062c\u062f\u064a\u062f\u0629");
  };

  // ── NEW: Load saved clip for editing ──
  const loadClipForEdit=(clip)=>{
    setSlides(clip.slides||[]);
    setClipTitle(clip.title||"");
    setClipSubj(clip.subject||"\u0627\u0644\u0631\u064a\u0627\u0636\u064a\u0627\u062a");
    setClipStage(clip.stage||"\u0627\u0644\u0627\u0628\u062a\u062f\u0627\u0626\u064a\u0629");
    setEditingClipDoc(clip);
    setCurSlide(0);
    setMode("result");
    setShowSavedClips(false);
    showMsg("\u062a\u0645 \u062a\u062d\u0645\u064a\u0644 \u0627\u0644\u0634\u0631\u0627\u0626\u062d \u0644\u0644\u062a\u0639\u062f\u064a\u0644");
  };

  // ── NEW: Update existing clip in Firestore ──
  const updateClipInFirestore=async()=>{
    if(!clipTitle.trim()) return showMsg("\u0623\u062f\u062e\u0644 \u0639\u0646\u0648\u0627\u0646 \u0627\u0644\u0645\u0642\u0637\u0639");
    if(!editingClipDoc?.id) return;
    setSaving(true);
    try{
      await updateDoc(doc(db,"clips",editingClipDoc.id),{title:clipTitle,subject:clipSubj,stage:clipStage,slides,theme:slidesTheme});
      showMsg(" \u062a\u0645 \u062d\u0641\u0638 \u0627\u0644\u062a\u0639\u062f\u064a\u0644\u0627\u062a!");
      setEditingClipDoc(null);
      setMode("menu");
    }catch(e){showMsg("\u0641\u0634\u0644 \u0627\u0644\u062d\u0641\u0638: "+e.message);}
    setSaving(false);
  };

  // ── NEW: Delete clip from Firestore ──
  const deleteClipFromFirestore=async(clip)=>{
    try{
      await deleteDoc(doc(db,"clips",clip.id));
      showMsg("\u062a\u0645 \u062d\u0630\u0641 \u0627\u0644\u0645\u0642\u0637\u0639");
    }catch(e){showMsg("\u0641\u0634\u0644 \u0627\u0644\u062d\u0630\u0641: "+e.message);}
    setConfirmDeleteClipDoc(null);
  };
    ...
    <div style={{display:"flex",gap:"8px",marginBottom:"14px"}}>
      {[4,6,8,10,12].map(n => (
        <button
          key={n}
          onClick={() => setCount(n)}
          style={{flex:1,padding:"9px",borderRadius:"10px",border:"none",backgroundColor:slidesCount===n?"#7c3aed":"#27272a",color:slidesCount===n?"#fff":"#a1a1aa",fontWeight:"bold",fontSize:"13px",cursor:"pointer"}}
        >
          {n}
        </button>
      ))}
  ...
  const back=<button onClick={()=>{setMode("menu");setEditingClipDoc(null);}} style={{background:"none",border:"none",color:"#71717a",cursor:"pointer",fontSize:"13px",marginBottom:"14px",display:"flex",alignItems:"center",gap:"4px"}}>← رجوع</button>;

  // ── EDIT SLIDE PANEL ──
  if(editingSlideIdx!==null) return (
    <div>
      <button onClick={()=>setEditingSlideIdx(null)} style={{background:"none",border:"none",color:"#71717a",cursor:"pointer",fontSize:"13px",marginBottom:"14px",display:"flex",alignItems:"center",gap:"4px"}}>← رجوع للشرائح</button>
      <div style={{...C.infoBanner,marginBottom:"12px"}}><Edit3 size={14}/> تعديل الشريحة {editingSlideIdx+1}</div>
      <label style={C.label}>عنوان الشريحة</label>
      <input value={editSlideTitle} onChange={e=>setEditSlideTitle(e.target.value)} style={C.input} placeholder="\u0639\u0646\u0648\u0627\u0646 \u0627\u0644\u0634\u0631\u064a\u062d\u0629"/>
      <label style={C.label}>النقاط (نقطة في كل سطر)</label>
      {editSlidePoints.map((pt,i)=>(
        <div key={i} style={{display:"flex",gap:"8px",marginBottom:"8px",alignItems:"center"}}>
          <input value={pt} onChange={e=>{const updated=[...editSlidePoints];updated[i]=e.target.value;setEditSlidePoints(updated);}} style={{...C.input,marginBottom:0,flex:1}} placeholder={"\u0646\u0642\u0637\u0629 "+(i+1)}/>
          <button onClick={()=>setEditSlidePoints(p=>p.filter((_,j)=>j!==i))} style={{width:36,height:36,borderRadius:"8px",backgroundColor:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",color:"#f87171",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><X size={14}/></button>
        </div>
      ))}
      <button
        onClick={() => setEditSlidePoints(p => [...p, ""])}
        style={{width:"100%",padding:"10px",borderRadius:"10px",border:"1px dashed rgba(56,189,248,0.4)",backgroundColor:"rgba(56,189,248,0.06)",color:"#38bdf8",fontSize:"13px",cursor:"pointer",marginBottom:"14px",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}
      >
        <Plus size={14}/> إضافة نقطة
      </button>
      <div style={C.saveRow}>
        <button onClick={()=>setEditingSlideIdx(null)} style={C.cancelBtn}>إلغاء</button>
        <button onClick={saveEditedSlide} style={{...C.saveBtn,display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}><Check size={15}/> حفظ التعديل</button>
      </div>
    </div>
  );
  ...
  if(mode==="menu") return <div>
    ...
    {/* ── NEW: Manage saved slides clips ── */}
    <div style={{marginBottom:"16px"}}>
      <button onClick={()=>setShowSavedClips(v=>!v)} style={{width:"100%",padding:"12px",borderRadius:"12px",border:"1px solid rgba(139,92,246,0.3)",backgroundColor:"rgba(139,92,246,0.1)",color:"#c4b5fd",fontSize:"13px",fontWeight:"bold",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px"}}>
        <Layers size={16}/> إدارة الشرائح المحفوظة ({savedClips.length})
      </button>
      {showSavedClips&&(
        <div style={{marginTop:"10px"}}>
          {savedClips.length===0
            ?<div style={{textAlign:"center",padding:"20px",color:"#52525b",fontSize:"13px"}}>لا توجد شرائح محفوظة</div>
            :savedClips.map(clip=>(
              <div key={clip.id} style={{...C.card,border:"1px solid rgba(139,92,246,0.2)",marginBottom:"8px"}}>
                {confirmDeleteClipDoc?.id===clip.id
                  ?<ConfirmDelete title={clip.title} subtitle={clip.subject+" • "+clip.stage+" • "+(clip.slides?.length||0)+" \u0634\u0631\u064a\u062d\u0629"} onConfirm={()=>deleteClipFromFirestore(clip)} onCancel={()=>setConfirmDeleteClipDoc(null)}/>
                  :<>
                    <div style={{marginBottom:"8px"}}>
                      <div style={{fontWeight:"bold",fontSize:"14px",color:"#fff"}}>{clip.title}</div>
                      <div style={{fontSize:"12px",color:"#71717a",marginTop:"2px"}}>{clip.subject} • {clip.stage} • {clip.slides?.length||0} شريحة</div>
                    </div>
                    <div style={{display:"flex",gap:"8px"}}>
                      <button onClick={()=>loadClipForEdit(clip)} style={{flex:1,padding:"9px",borderRadius:"10px",border:"1px solid rgba(56,189,248,0.3)",backgroundColor:"rgba(56,189,248,0.1)",color:"#38bdf8",fontSize:"12px",fontWeight:"bold",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"5px"}}><Edit3 size={13}/>
                        تعديل
                      </button>
                      <button
                        onClick={() => setConfirmDeleteClipDoc(clip)}
                        style={{flex:1,padding:"9px",borderRadius:"10px",border:"1px solid rgba(239,68,68,0.3)",backgroundColor:"rgba(239,68,68,0.1)",color:"#f87171",fontSize:"12px",fontWeight:"bold",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"5px"}}
                      >
                        <Trash2 size={13}/> حذف
                      </button>
                    </div>
  ...
  if(mode==="image") return <div>{back}
    ...
    <div style={{marginBottom:"12px"}}>
      ...
      <label style={{display:"block",width:"100%",padding:"14px",backgroundColor:"rgba(139,92,246,0.08)",border:"2px dashed rgba(139,92,246,0.35)",borderRadius:"14px",textAlign:"center",cursor:"pointer",boxSizing:"border-box"}}>
        ...
        <input type="file" accept="image/*" style={{display:"none"}} onChange={e=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=(ev)=>{const full=ev.target.result;setImgPreview(full);setImgB64(full.split(",")[1]);};reader.readAsDataURL(file);}}/>
      </label>
    </div>
  ...
  // ── RESULT (view + edit/delete per slide) ──
  if(mode==="result"){
    ...
    return <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"10px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
          {editingClipDoc&&<span style={C.editBadge}><Edit3 size={11}/> وضع التعديل</span>}
          <span style={{color:"#a1a1aa",fontSize:"12px"}}>{curSlide+1} / {slides.length}</span>
        </div>
      </div>

      {/* ── Slide preview ── */}
      <div style={{background:ts.bg,borderRadius:"20px",padding:"20px 16px",minHeight:"200px",marginBottom:"10px",border:"1px solid rgba(255,255,255,0.08)",position:"relative"}}>
        <div style={{position:"absolute",top:"10px",left:"12px",backgroundColor:ts.card,borderRadius:"8px",padding:"3px 10px",border:"1px solid "+ts.accent+"44"}}><span style={{color:ts.accent,fontSize:"11px",fontWeight:"bold"}}>
          شريحة {curSlide+1}
        </span></div>
        {/* ── NEW: Per-slide action buttons ── */}
        <div style={{position:"absolute",top:"8px",right:"10px",display:"flex",gap:"6px"}}>
          <button
            onClick={() => openEditSlide(curSlide)}
            style={{width:32,height:32,borderRadius:"8px",backgroundColor:"rgba(56,189,248,0.2)",border:"1px solid rgba(56,189,248,0.3)",color:"#38bdf8",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}
          >
            <Edit3 size={14}/>
          </button>
          <button
            onClick={()=>setConfirmDeleteSlideIdx(curSlide)}
            disabled={slides.length<=1}
            style={{width:32,height:32,borderRadius:"8px",backgroundColor:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",color:"#f87171",cursor:slides.length<=1?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}
      ...
      {confirmDeleteSlideIdx===curSlide&&(
        <ConfirmDelete title={"\u062d\u0630\u0641 \u0627\u0644\u0634\u0631\u064a\u062d\u0629 "+(curSlide+1)} subtitle={sl.title} onConfirm={()=>deleteSlide(curSlide)} onCancel={()=>setConfirmDeleteSlideIdx(null)}/>
      )}
      ...
      <div style={{display:"flex",gap:"10px",marginBottom:"10px"}}>
        ...
        <button disabled={curSlide===slides.length-1} onClick={()=>setCurSlide(i=>i+1)} style={{flex:1,padding:"11px",borderRadius:"12px",border:"none",backgroundColor:curSlide===slides.length-1?"#1c1c1e":"#27272a",color:curSlide===slides.length-1?"#3f3f46":"#fff",cursor:curSlide===slides.length-1?"not-allowed":"pointer",fontWeight:"bold"}}>
          التالي ▶
        </button>
      </div>

      {/* ── NEW: Add slide button ── */}
      <button onClick={addSlide} style={{width:"100%",padding:"11px",borderRadius:"12px",border:"1px dashed rgba(139,92,246,0.4)",backgroundColor:"rgba(139,92,246,0.06)",color:"#c4b5fd",fontSize:"13px",cursor:"pointer",marginBottom:"12px",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}><Plus size={14}/>
        إضافة شريحة جديدة
      </button>

      {/* ── Save panel ── */}
      <div style={{...C.card,border:"1px solid rgba(56,189,248,0.15)"}}>
        <div style={{fontSize:"13px",fontWeight:"bold",color:editingClipDoc?"#f97316":"#38bdf8",marginBottom:"10px",display:"flex",alignItems:"center",gap:"6px"}}>
          {editingClipDoc?<><Edit3 size={14}/> تعديل الشرائح المحفوظة</>:<><Save size={14}/> حفظ في Firebase</>}
        </div>
        ...
        {editingClipDoc
          ?<div style={{display:"flex",gap:"8px"}}>
            <button onClick={()=>{setEditingClipDoc(null);setMode("menu");}} style={C.cancelBtn}>إلغاء</button>
            <button onClick={updateClipInFirestore} disabled={saving} style={{...C.saveBtn,display:"flex",alignItems:"center",justifyContent:"center",gap:"6px",opacity:saving?0.7:1}}>
              {saving?<><Spinner size={15}/> جارٍ...</>:<><Check size={15}/> حفظ التعديلات</>}
            </button>
          </div>
...
function AdminPDFTab() {
  ...
  const savePDF=async()=>{
    ...
    setSaving(true);
    try{
      if(editingPdf?.id){await updateDoc(doc(db,"pdfs",editingPdf.id),{name:pdfName,subject:pdfSubject,stage:pdfStage,url:pdfUrl});showMsg("\u062a\u0645 \u062a\u0639\u062f\u064a\u0644 \u0627\u0644\u0645\u0644\u0641");}
  ...
  return <div>
    ...
    {confirmDelete&&<ConfirmDelete title={confirmDelete.name} subtitle={confirmDelete.subject+" • "+confirmDelete.stage} onConfirm={()=>doDelete(confirmDelete)} onCancel={()=>setConfirmDelete(null)}/>}
    {pdfs.map(f=>(
      <div key={f.id} style={{...C.card,border:"1px solid rgba(249,115,22,0.2)"}}>
        <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"8px"}}>
          ...
          <a href={f.url} target="_blank" rel="noreferrer" style={{backgroundColor:"rgba(249,115,22,0.15)",border:"1px solid rgba(249,115,22,0.3)",borderRadius:"8px",padding:"6px 10px",color:"#f97316",fontSize:"12px",textDecoration:"none",fontWeight:"bold"}}>
            فتح
          </a>
        </div>
        <div style={{display:"flex",gap:"8px"}}>
          <button onClick={()=>openEdit(f)} style={{flex:1,padding:"7px",borderRadius:"8px",border:"1px solid rgba(56,189,248,0.3)",backgroundColor:"rgba(56,189,248,0.1)",color:"#38bdf8",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>
            تعديل
          </button>
          <button onClick={()=>setConfirmDelete(f)} style={{flex:1,padding:"7px",borderRadius:"8px",border:"1px solid rgba(239,68,68,0.3)",backgroundColor:"rgba(239,68,68,0.1)",color:"#f87171",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>
            حذف
          </button>
        </div>
      </div>
    ...
    :<div style={{...C.card,border:"1px solid rgba(249,115,22,0.2)"}}>
      ...
      <label style={C.label}>اسم الملف</label><input type="text" value={pdfName} onChange={e=>setPdfName(e.target.value)} placeholder="\u0645\u062b\u0627\u0644: \u0645\u0644\u0632\u0645\u0629 \u0627\u0644\u0631\u064a\u0627\u0636\u064a\u0627\u062a \u0627\u0644\u0641\u0635\u0644 \u0627\u0644\u0623\u0648\u0644" style={C.input}/>
      <div style={C.twoCol}>
      ...
      <label style={C.label}> رابط PDF</label><input type="url" value={pdfUrl} onChange={e=>setPdfUrl(e.target.value)} placeholder="https://drive.google.com/file/..." style={C.input}/>
...
function AdminWalletTab() {
  ...
  const approvePayment=async(p)=>{
    ...
    try{
      ...
      await addDoc(collection(db, "notifications"), {
        ...
        body: "\u062a\u0645 \u062a\u0641\u0639\u064a\u0644 \u0627\u0634\u062a\u0631\u0627\u0643\u0643 \u0641\u064a " + p.subject + " (" + p.stage + ") \u0644\u0645\u062f\u0629 " + days + " \u064a\u0648\u0645.",
        targetPhone: p.studentPhone,
  ...
  const rejectPayment=async(p)=>{
    try{await addDoc(collection(db,"payments"),{...p,status:"rejected"});showMsg("\u062a\u0645 \u0631\u0641\u0636 \u0627\u0644\u062f\u0641\u0639");}catch{showMsg("\u0641\u0634\u0644");}
  };
  return <div>
      ...
      <div key={p.id} style={{...C.card,borderRight:`3px solid ${p.status==="approved"?"#4ade80":"#f87171"}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{textAlign:"left"}}>
            ...
            <div style={{fontSize:"11px",color:p.status==="approved"?"#4ade80":"#f87171"}}>
    ...
    return (
      <div style={{backgroundColor:"rgba(234,179,8,0.1)",border:"1px solid rgba(234,179,8,0.3)",borderRadius:"10px",padding:"8px 12px",fontSize:"12px",color:"#fbbf24",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <button
          ...
          style={{backgroundColor:"#f97316",border:"none",borderRadius:"6px",padding:"4px 10px",color:"#fff",fontSize:"11px",cursor:"pointer",fontWeight:"bold"}}
        >
          جدد
        </button>
      </div>
  ...
  if (!sub) {
    return (
      <div style={{backgroundColor:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:"10px",padding:"8px 12px",fontSize:"12px",color:"#f87171",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <button
          onClick={() => setModal("wallet")}
          style={{backgroundColor:"#ef4444",border:"none",borderRadius:"6px",padding:"4px 10px",color:"#fff",fontSize:"11px",cursor:"pointer",fontWeight:"bold"}}
        >
          اشترك
        </button>
      </div>
...
export default function App() {
  ...
  const handleTouchMove=(e)=>{
    if(e.touches[0].clientY>touchStartY.current&&window.scrollY===0)e.preventDefault();
  };
  ...
  const openEditClip=(clip)=>{
    setEditingClip(clip);
    setClipTitle(clip.title||"");setClipTeacher(clip.teacher||"");
  ...
  // ── NEW: Delete clip handler ──
  const doDeleteClip=async(clip)=>{
    try{
      if(clip.id) await deleteDoc(doc(db,"clips",clip.id));
      showMsg("\u062a\u0645 \u062d\u0630\u0641 \u0627\u0644\u0645\u0642\u0637\u0639");
    }catch(e){showMsg("\u0641\u0634\u0644 \u0627\u0644\u062d\u0630\u0641: "+e.message);}
  ...
  const applyTemplate=(t)=>{
    ...
    setNotifTitle(templates[t][0]);
    setNotifBody(templates[t][1]);
  };
    ...
    <div style={C.app}>
      ...
      {screen==="register"&&(
        <div style={{padding:"32px 24px"}}>
          ...
          <div style={{textAlign:"center",marginTop:"10px"}}><span style={{color:"#a1a1aa",fontSize:"13px"}}>
            لدي حساب؟
          </span><span style={{color:"#38bdf8",cursor:"pointer",fontWeight:"bold",fontSize:"13px"}} onClick={()=>{setLoginPhone("");setLoginPass("");setLoginErr("");setScreen("login");}}>
            تسجيل الدخول
          </span></div>
          <div style={{textAlign:"center",marginTop:"8px"}}><span style={{color:"#52525b",cursor:"pointer",fontSize:"12px"}} onClick={()=>setScreen("welcome")}>← رجوع</span><
