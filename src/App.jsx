...
import { getFirestore, collection, addDoc, onSnapshot, serverTimestamp, deleteDoc, updateDoc, setDoc, getDoc, getDocs, doc, query, where, orderBy, limit, arrayUnion, arrayRemove } from "firebase/firestore";
...
const sendNotification = async({phone, title, body}) => {
  ...
  }catch(e){ /* الإشعار ثانوي، ما نوقف تنفيذ العملية الأساسية بسببه */ }
};
...
const registerPushToken = async(phone) => {
  // نكتب نتيجة التشخيص كـ "توكن" مؤقت بنفس حقل fcmTokens بدل رسالة تختفي بثوانٍ —
  // هذا يسمح لك تتفرج على النتيجة بهدوء من Firebase Console (تبويب Data) بأي وقت تناسبك
  const logDebug = async(msg) => {
    try{ await updateDoc(doc(db,"students",phone),{fcmTokens:arrayUnion("[DEBUG] "+msg)}); }catch(e2){ /* حتى لو فشل التسجيل نفسه، ما نوقف التطبيق */ }
  };
  try{
    if(!FIREBASE_VAPID_KEY){ await logDebug("مفتاح VAPID غير موجود بالبيئة"); return; }
    if(!("serviceWorker" in navigator) || !("Notification" in window)){ await logDebug("المتصفح لا يدعم الإشعارات"); return; }
    const permission = await Notification.requestPermission();
    if(permission!=="granted"){ await logDebug("الإذن غير ممنوح: "+permission); return; }
    ...
    if(token){
      ...
      await logDebug("getToken رجع فارغ بدون خطأ");
    }
  }catch(e){ await logDebug("خطأ: "+e.message); }
};
...
function AdminWalletTab() {
  ...
  const approvePayment=async(p)=>{
    ...
    try{
      ...
      await sendNotification({phone:p.studentPhone,title:" تم تفعيل اشتراكك!",body:"تم تفعيل اشتراكك في "+p.subject+" ("+p.stage+") لمدة "+days+" يوم"+(bonusApplied>0?" (تشمل "+bonusApplied+" يوم هدية ترحيبية 🎁)":"")+". ينتهي في "+expiresAt.toLocaleDateString("ar")});
...
function MiniLineChart({points, color, formatValue}) {
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",fontSize:"10px",color:"#52525b",marginTop:"2px"}}>
