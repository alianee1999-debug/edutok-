...
export default function App() {
  ...
  const allVideos = React.useMemo(()=>{
    ...
    if(selectedSubject){
      // لو فيه customList من BrowseModal استخدمه مباشرة
      if(selectedSubject.customList) return selectedSubject.customList;
  ...
  return (
    <div style={C.app}>
      ...
      {modal==="browse" &&<BrowseModal onClose={closeModal} clips={clips} globalPrices={globalPrices} onBrowse={(subject,stage,grade,topic,isFree)=>{
        const filtered=clips.filter(c=>
          c.subject===subject && c.stage===stage &&
          (!grade || c.grade===grade || !c.grade) &&
          (!topic || c.topic===topic)
        ).sort((a,b)=>Number(a.num||0)-Number(b.num||0));
        if(isFree){
          setSelectedSubject({subject,stage,grade,topic,customList:filtered});
        } else {
          // ملاحظة: لا نضيف مقاطع من مواد/مراحل أخرى هنا حتى لا تختلط
          // مقاطع من مادة مختلفة ضمن قائمة مادة أخرى (كانت هذه مشكلة سابقة).
          const first5=filtered.slice(0,5);
          const rest=filtered.slice(5).sort(()=>Math.random()-0.5);
          setSelectedSubject({subject,stage,grade,topic,customList:[...first5,...rest]});
        }
        setVideoIdx(0);
