      ...
      :myFiles.map(f=>(
        <div key={f.id} style={{...C.card}}>
          ...
          {!canRead
            ...
            :<div style={{display:"flex",gap:"8px"}}>
              {/* زر القراءة — دائماً متاح لو مؤهل */}
              <a href={f.url} target="_blank" rel="noreferrer" style={{flex:1,padding:"8px",borderRadius:"8px",border:"1px solid rgba(56,189,248,0.3)",backgroundColor:"rgba(56,189,248,0.1)",color:"#38bdf8",fontSize:"12px",cursor:"pointer",fontWeight:"bold",textDecoration:"none",textAlign:"center"}}>قراءة 👁</a>
...
function BrowseModal({onClose, clips, globalPrices, onBrowse}) {
  const [step,setStep]=useState("stage"); // stage → semester → subject
  const [selStage,setSelStage]=useState("");
  const [selSemester,setSelSemester]=useState("");

  // استخرج المواد المتاحة لهذه المرحلة والفصل
  const availableSubjects = React.useMemo(()=>{
    if(!selStage||!selSemester) return [];
    const subjs=new Set();
    clips.forEach(c=>{
      if(c.stage===selStage&&(c.semester===selSemester||!c.semester)) subjs.add(c.subject);
    });
    return [...subjs].sort();
  },[clips,selStage,selSemester]);

  const stageEmoji={"الابتدائية":"🏫","المتوسطة":"📚","الإعدادية":"🎓"};
  const semesterEmoji={"الأول":"١","الثاني":"٢","الثالث":"٣"};
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(99,102,241,0.3)"}}>
    ...
    {/* خبز الفتات */}
    {(selStage||selSemester)&&<div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"14px",fontSize:"12px",color:"#71717a",flexWrap:"wrap"}}>
      <span style={{color:"#818cf8",cursor:"pointer"}} onClick={()=>{setStep("stage");setSelStage("");setSelSemester("");}}>المراحل</span>
      {selStage&&<><span>←</span><span style={{color:selSemester?"#818cf8":"#fff",cursor:selSemester?"pointer":"default"}} onClick={()=>{if(selSemester){setStep("semester");setSelSemester("");} }}>{selStage}</span></>}
      {selSemester&&<><span>←</span><span style={{color:"#fff"}}>{selSemester==="الأول"?"الفصل الأول":selSemester==="الثاني"?"الفصل الثاني":"الفصل الثالث"}</span></>}
    </div>}

    {/* خطوة ١: اختيار المرحلة */}
    {step==="stage"&&<div>
      <div style={{fontSize:"13px",color:"#71717a",marginBottom:"12px",textAlign:"center"}}>اختر المرحلة الدراسية</div>
      {STAGES.map(s=>(
        <div key={s} onClick={()=>{setSelStage(s);setStep("semester");}} style={{...C.card,cursor:"pointer",display:"flex",alignItems:"center",gap:"14px",marginBottom:"8px",border:"1px solid rgba(129,140,248,0.2)"}}>
          <span style={{fontSize:"32px"}}>{stageEmoji[s]||"📖"}</span>
          <div>
            <div style={{fontWeight:"bold",fontSize:"15px"}}>{s}</div>
            <div style={{fontSize:"11px",color:"#71717a"}}>
              {clips.filter(c=>c.stage===s).length} مقطع متاح
            </div>
          </div>
          <span style={{marginRight:"auto",color:"#818cf8",fontSize:"18px"}}>←</span>
        </div>
      ))}
    </div>}
    ...
    {/* خطوة ٢: اختيار الفصل */}
    {step==="semester"&&<div>
      <div style={{fontSize:"13px",color:"#71717a",marginBottom:"12px",textAlign:"center"}}>اختر الفصل الدراسي</div>
      {["الأول","الثاني","الثالث"].map(sem=>{
        const count=clips.filter(c=>c.stage===selStage&&(c.semester===sem||!c.semester)).length;
        return <div key={sem} onClick={()=>{setSelSemester(sem);setStep("subject");}} style={{...C.card,cursor:"pointer",display:"flex",alignItems:"center",gap:"14px",marginBottom:"8px",border:"1px solid rgba(129,140,248,0.2)"}}>
          <div style={{width:"44px",height:"44px",borderRadius:"12px",background:"linear-gradient(135deg,#6366f1,#818cf8)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"20px",fontWeight:"bold",color:"#fff",flexShrink:0}}>
            {semesterEmoji[sem]}
          </div>
          <div>
            <div style={{fontWeight:"bold",fontSize:"15px"}}>الفصل {sem}</div>
            <div style={{fontSize:"11px",color:"#71717a"}}>{count} مقطع متاح</div>
          </div>
          <span style={{marginRight:"auto",color:"#818cf8",fontSize:"18px"}}>←</span>
        </div>;
      })}
    ...
    {/* خطوة ٣: اختيار المادة */}
    {step==="subject"&&<div>
      <div style={{fontSize:"13px",color:"#71717a",marginBottom:"12px",textAlign:"center"}}>اختر المادة</div>
      {availableSubjects.length===0
        ?<div style={{textAlign:"center",padding:"24px",color:"#52525b"}}>
          <BookOpen size={40} color="#3f3f46" style={{margin:"0 auto 8px"}}/>
          <div>لا توجد مقاطع لهذه المرحلة والفصل</div>
        </div>
        :availableSubjects.map(subj=>{
          const isFree=isFreeSubject(globalPrices,subj,selStage);
          const subjClips=clips.filter(c=>c.stage===selStage&&c.subject===subj&&(c.semester===selSemester||!c.semester));
          return <div key={subj} onClick={()=>{onBrowse(subj,selStage,selSemester,isFree);onClose();}} style={{...C.card,cursor:"pointer",display:"flex",alignItems:"center",gap:"12px",marginBottom:"8px",border:`1px solid ${isFree?"rgba(52,211,153,0.3)":"rgba(129,140,248,0.2)"}`}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:"11px",color:"#71717a",marginTop:"2px"}}>
                {subjClips.length} مقطع • {isFree?"🆓 مجاني — ترتيب كامل":"💰 مدفوع — 5 مقاطع + عشوائي"}
              </div>
            </div>
...
export default function App() {
  ...
  const [regName,setRegName]=useState(""); const [regPhone,setRegPhone]=useState(""); const [regAccount,setRegAccount]=useState(""); const [regPass,setRegPass]=useState(""); const [regStage,setRegStage]=useState("الابتدائية"); const [regGrade,setRegGrade]=useState("الأول"); const [regErr,setRegErr]=useState("");
  ...
  const doRegister=async()=>{
    ...
    try{
      ...
      const stored = {name:regName.trim(),phone:phoneKey,account:regAccount.trim(),stage:regStage,grade:regGrade,passHash:hash,passSalt:salt,createdAt:serverTimestamp()};
      await setDoc(doc(db,"students",phoneKey),stored);
      const s = {name:stored.name,phone:stored.phone,account:stored.account,stage:stored.stage,grade:stored.grade};
      setCurrentStudent(s);setRole("student");setScreen("home");saveSession(s,"student");
      setRegName("");setRegPhone("");setRegAccount("");setRegPass("");setRegStage("الابتدائية");setRegGrade("الأول");setRegErr("");
    }
  ...
  const doLogin=async()=>{
    ...
    try{
      ...
      if(ok){
        const s = {name:ex.name,phone:ex.phone,account:ex.account,stage:ex.stage||"الابتدائية",grade:ex.grade||"الأول"};
        setCurrentStudent(s);setRole("student");setScreen("home");saveSession(s,"student");
  ...
  return (
    <div style={C.app}>
      ...
      {modal==="browse" &&<BrowseModal onClose={closeModal} clips={clips} globalPrices={globalPrices} onBrowse={(subject,stage,semester,isFree)=>{
        // بناء قائمة المقاطع حسب المادة والمرحلة والفصل
        const filtered=clips.filter(c=>c.subject===subject&&c.stage===stage&&(c.semester===semester||!c.semester))
          .sort((a,b)=>Number(a.num||0)-Number(b.num||0));
        if(isFree){
          // مجانية: كل المقاطع بالترتيب
          setSelectedSubject({subject,stage,semester,customList:filtered});
        } else {
          // مدفوعة: أول 5 بالترتيب + الباقي يختلط مع الكل
          ...
          setSelectedSubject({subject,stage,semester,customList:[...first5,...rest,...otherClips]});
        }
        setVideoIdx(0);
