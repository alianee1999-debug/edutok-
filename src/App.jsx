...
export default function App() {
  ...
  const allVideos = React.useMemo(()=>{
    ...
    Object.entries(grouped).forEach(([key,group])=>{
      ...
      // نطبّق هنا نفس قاعدة قفل الفصول بالامتحان المستخدمة بمسار "فصل محدد/كل
      // الفصول" أعلاه بالضبط — وإلا الفيد العام (لما الطالب يفتح التطبيق مباشرة
      // بدون ما يختار مادة) كان يعرض كل فصول أي مادة بالتسلسل من غير أي التزام
      // بقفل الامتحانات بينها، بعكس مسار "تصفّح وبحث" اللي يطبّقه صح.
      const topicMinNum={};
      group.forEach(c=>{
        if(c.topic){
          const n=Number(c.num||0);
      ...
      if(role==="student"){
        orderedTopics.forEach((topic,i)=>{
          const prevTopic = i>0 ? orderedTopics[i-1] : null;
          const isLocked = prevTopic && !examScores?.[topicKey(subject,stage,prevTopic)]?.passed;
          if(isLocked) lockedTopics.add(topic);
        });
      }
      ...
      group.forEach(clip=>{
        if(clip.topic && lockedTopics.has(clip.topic)) return; // فصل مقفول بالامتحان — يُستبعد من الفيد العام تماماً
