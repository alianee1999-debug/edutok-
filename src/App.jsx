...
import { getFirestore, collection, addDoc, onSnapshot, serverTimestamp, deleteDoc, updateDoc, setDoc, getDoc, getDocs, doc, query, where, orderBy, arrayUnion, arrayRemove } from "firebase/firestore";
...
import { Bookmark, Share2, Bot, MessageCircle, MoreHorizontal, FileText, Camera, Search, ChevronUp, ChevronDown, Settings, User, Home, Bell, DollarSign, Users, Layers, Film, Sparkles, X, Save, BookOpen, GraduationCap, Plus, Play, Pause, Loader, Key, Copy, CheckCircle, Trash2, ClipboardList, Lock, Wand2, Pencil } from "lucide-react";
...
// ─── SEARCH MODAL ────────────────────────────────────────
// ─── BROWSE MODAL ────────────────────────────────────────
function BrowseModal({onClose, clips, globalPrices, onBrowse, role, examScores}) {
  ...
  const availableTopics = React.useMemo(()=>{
    ...
    const topicMinNum={};
    clips.forEach(c=>{
      if(c.stage===selStage&&c.subject===selSubject&&c.topic){
...
function ExamModal({onClose, initial, currentStudent, mySubscriptions, globalPrices, clips, examScores, onResult}) {
  ...
  const [pickerSub,setPickerSub]=useState(initial?"topic":"stage"); // مرحلة فرعية بداخل خطوة "picker"
  ...
  const availableSubjects = React.useMemo(()=>{
    if(!pickStage) return [];
    const subjs = new Set();
    clips.forEach(c=>{ if(c.stage===pickStage && c.topic) subjs.add(c.subject); });
    return [...subjs].filter(s=>hasAccess(mySubscriptions,globalPrices,s,pickStage)).sort();
  },[clips,pickStage,mySubscriptions,globalPrices]);
  const availableTopics = React.useMemo(()=>{
    if(!pickStage||!pickSubject) return [];
    const topicMinNum={};
    clips.forEach(c=>{
      if(c.stage===pickStage&&c.subject===pickSubject&&c.topic){
        const n=Number(c.num||0);
        if(!(c.topic in topicMinNum)||n<topicMinNum[c.topic]) topicMinNum[c.topic]=n;
      }
    });
    return Object.keys(topicMinNum).sort((a,b)=>topicMinNum[a]-topicMinNum[b]);
  },[clips,pickStage,pickSubject]);
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(168,85,247,0.3)"}}>
    ...
    {step==="picker"&&pickerSub==="stage"&&<div>
      ...
      {availableStages.map(s=>(
        <div key={s} onClick={()=>{setPickStage(s);setPickerSub("subject");}} style={{...C.card,cursor:"pointer",display:"flex",alignItems:"center",gap:"14px",marginBottom:"8px",border:"1px solid rgba(168,85,247,0.2)"}}>
    ...
    {step==="picker"&&pickerSub==="subject"&&<div>
      ...
      {availableSubjects.length===0
        ?<div style={{textAlign:"center",padding:"24px",color:"#52525b",fontSize:"13px"}}>لا توجد مواد متاحة لك بهذه المرحلة</div>
        :availableSubjects.map(subj=>(
    ...
    {step==="picker"&&pickerSub==="topic"&&<div>
      <div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"12px",fontSize:"12px",flexWrap:"wrap"}}>
        ...
        <span style={{color:"#c4b5fd",cursor:"pointer"}} onClick={()=>setPickerSub("subject")}>{pickStage}</span>
    ...
    {step==="quiz"&&q&&<div>
      ...
      {(q.options||[]).map((opt,i)=>{
        ...
        return <div key={i} onClick={()=>selectAnswer(i)} style={{
          ...
          <div style={{width:"22px",height:"22px",borderRadius:"50%",flexShrink:0,border:`2px solid ${isSelected?"#a855f7":"rgba(255,255,255,0.2)"}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
            {isSelected&&<div style={{width:"10px",height:"10px",borderRadius:"50%",background:"#a855f7"}}/>}
          </div>
...
function AdminExamsTab() {
  ...
  const topics = React.useMemo(()=>{
    const set=new Set();
    clips.forEach(c=>{ if(c.subject===subject&&c.stage===stage&&c.topic) set.add(c.topic); });
    return [...set].sort();
  },[clips,subject,stage]);
  useEffect(()=>{ setTopic(topics[0]||""); },[subject,stage]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const buildGroundingText = ()=>{
    const topicClips = clips.filter(c=>c.subject===subject&&c.stage===stage&&c.topic===topic&&Array.isArray(c.slides));
    const chunks=[];
  ...
  const generate = async()=>{
    ...
    try{
      const prompt = "أنت معلّم متخصص. بناءً على المحتوى الدراسي التالي لمادة "+subject+" ("+stage+" — فصل: "+topic+"):\n\n"+grounding
        +"\n\nأنشئ "+count+" سؤال اختيار من متعدد (MCQ) مبنية حصراً على هذا المحتوى، بحيث كل سؤال له 4 خيارات وخيار واحد صحيح فقط. "
        ...
        +'أجب بصيغة JSON فقط بلا أي نص خارجه، على شكل مصفوفة: [{"question":"نص السؤال","options":["خيار1","خيار2","خيار3","خيار4"],"correctIndex":0}]';
      const raw = await callGroq(prompt);
      const clean = raw.replace(/```json/g,"").replace(/```/g,"").trim();
      const start = clean.indexOf("[");
      const end = clean.lastIndexOf("]");
      if(start===-1||end===-1) throw new Error("لم يُرجع المساعد صيغة صحيحة، حاول مرة أخرى");
      const parsed = JSON.parse(clean.substring(start,end+1));
      const valid = parsed.filter(q=>q.question && Array.isArray(q.options) && q.options.length===4 && Number.isInteger(q.correctIndex) && q.correctIndex>=0 && q.correctIndex<=3);
      if(valid.length===0) throw new Error("لم يتم توليد أي سؤال صالح");
      setDrafts(prev=>[...prev, ...valid.map(q=>({...q, _tempId:Math.random().toString(36).slice(2)}))]);
      showMsg("تم توليد "+valid.length+" سؤال — راجعها ثم انشرها");
    }catch(e){
      showMsg("فشل التوليد: "+e.message);
    }
    setGenerating(false);
  };
  ...
  const addManual = async()=>{
    ...
    try{
      await addDoc(collection(db,"examQuestions"),{subject,stage,topic,question:manualQ,options:manualOpts,correctIndex:manualCorrect,published:true,createdAt:serverTimestamp()});
  ...
  return <div>
    ...
    {topics.length===0
      ?<div style={{...C.infoBanner,backgroundColor:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.25)",color:"#f87171"}}>لا توجد فصول (مقاطع فيها اسم فصل) لهذه المادة/المرحلة بعد</div>
    ...
    <label style={C.label}>عدد الأسئلة المطلوب توليدها</label>
    <input type="number" min={5} max={40} value={count} onChange={e=>setCount(Number(e.target.value)||20)} style={C.input}/>

    <button disabled={generating||!topic} onClick={generate} style={{...C.purpleBtn,opacity:(generating||!topic)?0.6:1,marginBottom:"18px"}}>
    ...
    {drafts.length>0&&<div style={{marginBottom:"20px"}}>
      ...
      {drafts.map(d=>(
        <div key={d._tempId} style={{...C.card,border:"1px solid rgba(251,191,36,0.25)",marginBottom:"8px"}}>
          <textarea value={d.question} onChange={e=>updateDraft(d._tempId,"question",e.target.value)} style={{...C.input,minHeight:"50px",marginBottom:"8px",fontFamily:"inherit"}}/>
          {d.options.map((opt,i)=>(
    ...
    {topic&&topicPublished.length>0&&<div>
      ...
      {topicPublished.map((q,i)=>(
        <div key={q.id} style={{...C.card,border:"1px solid rgba(34,197,94,0.15)",marginBottom:"6px"}}>
          ...
          <div style={{fontSize:"11px",color:"#4ade80",marginTop:"6px"}}>✓ <MathText text={q.options?.[q.correctIndex]} style={{fontSize:"11px",color:"#4ade80"}}/></div>
        </div>
      ))}
    </div>}
  </div>;
...
export default function App() {
  ...
  useEffect(()=>{
    if(role!=="student"||!currentStudent?.phone){setStudentProgress({});setWatchedClipIds([]);setCertifiedTopics([]);setExamScores({});return;}
    const unsub=onSnapshot(doc(db,"progress",currentStudent.phone),snap=>{
      if(snap.exists()){
        ...
        setStudentProgress({});setWatchedClipIds([]);setCertifiedTopics([]);setExamScores({});
      }
    });
    return ()=>unsub();
  },[role,currentStudent]);
  ...
  const saveClip=async()=>{
    ...
    try{
      const data={
        ...
        topic:clipTopic,type:clipType,num:clipNum,teacher:clipTeacher,
  ...
  return (
    <div style={C.app}>
          ...
          <div style={C.section}>
            ...
            {adminTab==="clips"&&showClipForm&&(
              <div>
                ...
                <div>
                  <label style={C.label}>المادة</label><select style={C.select} value={clipSubject} onChange={e=>{setClipSubject(e.target.value);if(!editingClip)setClipNum(getAutoNum(e.target.value,clipStage,clipGrade));}}>{SUBJECTS.map(s=><option key={s}>{s}</option>)}</select>
                </div>
                <div style={C.twoCol}>
      ...
      {modal==="browse" &&<BrowseModal onClose={closeModal} clips={clips} globalPrices={globalPrices} role={role} examScores={examScores} onBrowse={(subject,stage,grade,topic,isFree)=>{
