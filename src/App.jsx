...
const callMistral = async (prompt, imageBase64=null, imageMime="image/jpeg") => {
  let res;
  try{
    res = await fetchWithTimeout("/api/mistral", {
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ prompt, imageBase64, imageMime })
    });
  }catch(e){
    if(e.name==="AbortError") throw new Error("انتهت المهلة — Mistral تأخر بالرد أكثر من اللازم");
    throw new Error("تعذّر الاتصال بـ Mistral: "+e.message);
  }
  let d;
  try{ d = await res.json(); }
  catch{
