...
// ─── GITHUB MODELS (عبر السيرفر، المفتاح غير مكشوف بالمتصفح) ──
// مجاني بالكامل بدون بطاقة بنكية — يعتمد على حساب GitHub (Personal Access Token)
// بدل حساب مستقل، فيعطينا تنويع حقيقي بمزود مختلف البنية التحتية عن الباقي.
// الموديل النصي فقط، نفس منطق الصورة المتجاهلة تلقائياً بالسيرفر (راجع
// api/github-models.js)
const callGithubModels = async (prompt, imageBase64=null, imageMime="image/jpeg") => {
  let res;
  try{
    res = await fetchWithTimeout("/api/github-models", {
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ prompt, imageBase64, imageMime })
    });
  }catch(e){
    if(e.name==="AbortError") throw new Error("انتهت المهلة — GitHub Models تأخر بالرد أكثر من اللازم");
    throw new Error("تعذّر الاتصال بـ GitHub Models: "+e.message);
  }
  let d;
  try{ d = await res.json(); }
  catch{
  ...
  if(d.error) throw new Error(d.error||"خطأ بالاتصال بـ GitHub Models");
  return d.text || "";
};
...
const AI_PROVIDERS = [
  ...
  { name: "GitHub Models", fn: callGithubModels, supportsImage: false },
