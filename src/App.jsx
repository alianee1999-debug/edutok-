    ...
    <div style={C.app}>
          ...
          <div style={C.section}>
            ...
            {adminTab==="clips"&&showClipForm&&(
              <div>
                ...
                <button onClick={resetClipForm} style={{width:"100%",padding:"11px",backgroundColor:"#27272a",color:"#ef4444",border:"1px solid rgba(239,68,68,0.3)",borderRadius:"12px",fontSize:"13px",fontWeight:"bold",cursor:"pointer",marginBottom:"14px",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}>
                  <X size={14}/> إغلاق
                </button>
                <div style={C.twoCol}>
                  <div>
      ...
      {screen==="account"&&(
        <div style={{padding:"40px 20px",textAlign:"center"}}>
          ...
          {currentStudent&&(
            <div>
              <div style={{fontSize:"20px",fontWeight:"bold",marginBottom:"4px"}}>{currentStudent.name}</div>
              <div style={{color:"#38bdf8",fontSize:"14px",marginBottom:"4px"}}>@{currentStudent.account}</div>
              <div style={{color:"#71717a",fontSize:"13px",marginBottom:"20px"}}>{currentStudent.phone}</div>
            </div>
          )}
          {role==="admin"&&(
            <div style={{backgroundColor:"rgba(234,179,8,0.1)",border:"1px solid rgba(234,179,8,0.3)",borderRadius:"10px",padding:"10px",marginBottom:"16px",color:"#fbbf24",fontSize:"13px",fontWeight:"bold"}}>
              حساب المدير
            </div>
          )}
          <button onClick={()=>{setCurrentStudent(null);setRole("guest");setScreen("welcome");clearSession();}} style={{...C.redBtn,marginTop:"10px"}}>
            تسجيل الخروج
          </button>
        </div>
      )}
      ...
      {showNav&&(
        <div style={C.bottomNav}>
          {[
            [<Home size={20}/>,"الرئيسية","home",null],
            [<Search size={20}/>,"بحث",null,()=>setModal("s
