...
function WalletModal({onClose, student}) {
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(34,197,94,0.2)"}}>
    ...
    {sent ? (
      <div style={{textAlign:"center",padding:"24px"}}>
        ...
        <div style={{color:"#4ade80",fontWeight:"bold",fontSize:"16px",marginBottom:"8px"}}>تم إرسال طلب الدفع!</div>
        <div style={{color:"#71717a",fontSize:"13px"}}>سيتم تفعيل اشتراكك بعد مراجعة المدير</div>
        <button onClick={onClose} style={{...C.primaryBtn,marginTop:"16px"}}>إغلاق</button>
      </div>
    ) : (
      <>
        ...
        <input type="number" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="مثال: 5000" style={C.input}/>
...
function AdminWalletTab({students}) {
  ...
  useEffect(()=>{
    const unsub=onSnapshot(collection(db,"payments"),snap=>{
      setPayments(snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>b.createdAt?.seconds-a.createdAt?.seconds));
    });
    return ()=>unsub();
  },[]);
  ...
  return <div>
    ...
    <div style={{background:"linear-gradient(135deg,#14532d,#15803d)",borderRadius:"16px",padding:"20px",marginBottom:"14px",textAlign:"center"}}>
      ...
      <div style={{fontSize:"12px",color:"rgba(255,255,255,0.6)"}} onClick={()=>setEditingNum(true)}>✏️ اضغط لتغيير الرقم</div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px",marginBottom:"16px"}}>
      {[["💰",totalReceived.toLocaleString()+" د.ع","إجمالي المستلم"],["⏳",pending.length,"طلبات معلقة"],["✅",payments.filter(p=>p.status==="approved").length,"مؤكدة"]].map(([icon,val,label])=>(
        <div key={label} style={{...C.statCard}}>
          ...
          <div style={{fontSize:"16px",fontWeight:"bold",color:"#4ade80",margin:"2px 0"}}>{val}</div>
    ...
    {pending.length>0&&(
      <div style={{marginBottom:"12px"}}>
        <div style={{fontSize:"13px",fontWeight:"bold",color:"#fbbf24",marginBottom:"8px",display:"flex",alignItems:"center",gap:"6px"}}>⏳ طلبات تحتاج موافقة</div>
        {pending.map(p=>(
          <div key={p.id} style={{...C.card,border:"1px solid rgba(234,179,8,0.3)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"}}>
              ...
              {p.receiptUrl&&<img src={p.receiptUrl} alt="إيصال" style={{width:60,height:60,borderRadius:"8px",objectFit:"cover",border:"1px solid rgba(255,255,255,0.1)"}}/>}
            </div>
            <div style={{display:"flex",gap:"8px"}}>
              <button onClick={async()=>{try{await addDoc(collection(db,"payments"),{...p,status:"approved"});alert("✅ تم قبول الدفع!");}catch(e){alert("فشل");}}} style={{flex:1,padding:"10px",backgroundColor:"rgba(34,197,94,0.15)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:"10px",color:"#4ade80",fontSize:"13px",fontWeight:"bold",cursor:"pointer"}}>✅ قبول</button>
              <button onClick={async()=>{try{await addDoc(collection(db,"payments"),{...p,status:"rejected"});alert("تم رفض الدفع");}catch(e){alert("فشل");}}} style={{flex:1,padding:"10px",backgroundColor:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:"10px",color:"#f87171",fontSize:"13px",fontWeight:"bold",cursor:"pointer"}}>❌ رفض</button>
            </div>
          </div>
        ))}
      </div>
    ...
    {payments.length===0
      ...
      :payments.slice(0,20).map(p=>(
        <div key={p.id} style={{...C.card,borderRight:`3px solid ${p.status==="approved"?"#4ade80":p.status==="rejected"?"#f87171":"#fbbf24"}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{textAlign:"left"}}>
              ...
              <div style={{fontSize:"11px",color:p.status==="approved"?"#4ade80":p.status==="rejected"?"#f87171":"#fbbf24"}}>{p.status==="approved"?"✅ مقبول":p.status==="rejected"?"❌ مرفوض":"⏳ معلق"}</div>
            </div>
          </div>
        </div>
      ))
...
export default function App() {
  ...
  return (
    <div style={C.app}>
      ...
      {modal==="pdf"&&<PDFModal onClose={closeModal} isSubscribed={currentStudent?.isSubscribed||false}/>}
      {modal==="wallet"&&<WalletModal onClose={closeModal} student={currentStudent}/>}
