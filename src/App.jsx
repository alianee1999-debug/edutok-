...
export default function App() {
  ...
  // ─── ترقيم (Pagination) مقاطع الطالب بفصل محدد ──────────────────────
  // بدل تحميل كل مقاطع كل التطبيق دفعة وحدة (السطر أدناه بـonSnapshot)، لما
  // الطالب (role==="student") يفتح فصلاً محدداً (topic) بمادة مجانية أو
  // مشترك بها فعلاً، نجيب مقاطع هذا الفصل تحديداً فقط (بدل مصفوفة "clips"
  // الكاملة)، نرتبها بأنفسنا برقمها (Number()، مو Firestore)، ونكشف منها
  // دفعة (PAGE_SIZE) تزيد تدريجياً كل ما اقترب الطالب من نهاية المكشوف حالياً.
  // لوحة الإدارة وخيار "كل الفصول" وحالة "مادة مدفوعة وغير مشترك" (معاينة
  // عشوائية) تبقى تعتمد على "clips" الكاملة تماماً كما كانت — بدون أي تغيير
  // بسلوكها — لتجنّب أي مخاطرة على منطق موجود ومُختبر.
  const PAGE_SIZE = 20;
  const [paginatedClips, setPaginatedClips] = useState([]);
  const [paginatedHasMore, setPaginatedHasMore] = useState(true);
  const [paginatedLoading, setPaginatedLoading] = useState(false);
  const [paginatedFailed, setPaginatedFailed] = useState(false); // يرجعنا تلقائياً للمسار القديم الكامل لو فشل الاستعلام (مثلاً فهرس Firestore مفقود)
  ...
  // ✅ إضافة: نوع عرض رابط الفيديو — "embed" (افتراضي، يضمّن الفيديو بداخل
  // التطبيق مباشرة، السلوك الحالي) أو "external" (يفتح المحتوى بمصدره
  // الأصلي بتبويب/تطبيق منفصل بدل تضمينه). "external" ضروري لمحتوى مو ملك
  // المدير (محاضرة أستاذ خارجي)، ومطلوب تقنياً أيضاً لروابط Google Drive/
  // Docs/Slides لأنها لا تدعم التضمين بتطبيق ثاني أصلاً بغض النظر عن الخيار.
  const [clipLinkType,setClipLinkType]=useState("embed");
  // ✅ إضافة: رابط مرجعي خارجي اختياري (يوتيوب/جوجل) — منفصل تماماً عن
  // clipVideoUrl/clipLinkType أعلاه. الفيديو الأساسي يبقى يُعرض بالشاشة
  // الرئيسية كالمعتاد بغض النظر عن هذا الحقل؛ هذا الرابط يظهر فقط كبطاقة
  // اختيارية جوا محادثة "مساعد" (زكي) كمصدر مراجعة إضافي للطالب.
  const [clipReferenceLink,setClipReferenceLink]=useState("");
  ...
  // تحميل مقاطع فصل محدد للطالب (راجع تعليق تعريف paginatedClips أعلاه لشرح
  // الفكرة والنطاق). نجيب مقاطع الفصل المحدد فقط (بدل كل مقاطع كل التطبيق)
  // بدون طلب ترتيب من Firestore نفسه، ثم نرتبها بأنفسنا بنفس طريقة صفحة
  // الإدارة بالضبط (تحويل صريح لرقم عبر Number() قبل المقارنة). هذا يتفادى
  // مشكلة حقيقية واجهناها: Firestore يرتّب حسب نوع القيمة المخزّنة فعلياً
  // (نص أو رقم)، فلو تراكمت مقاطع قديمة برقم مخزّن كنص، ترتيب Firestore
  // نفسه (orderBy) يطلع خاطئاً — بعكس المقارنة الرقمية بجافاسكربت اللي تصلح
  // هذا تلقائياً بغض النظر عن نوع التخزين الفعلي بكل مستند.
  // "الترقيم" هنا يصير بعد الجلب: نعرض دفعة من القائمة المرتبة، ونكشف أكثر
  // تدريجياً — أبسط وأضمن من صفحات Firestore المؤشّرة (startAfter)، وما زال
  // يقلل القراءة الفعلية بشكل كبير مقارنة بتحميل كل مقاطع كل مادة وكل صف.
  const hasSelectedSubjectAccess = selectedSubject &&
    (selectedSubject.isFree!==false || hasAccess(mySubscriptions,globalPrices,selectedSubject.subject,selectedSubject.stage,selectedSubject.grade));
  const usePaginatedClips = role==="student" && !!selectedSubject?.topic && !!hasSelectedSubjectAccess && !paginatedFailed;
  const [paginatedVisibleCount, setPaginatedVisibleCount] = useState(PAGE_SIZE);

  useEffect(()=>{
    if(!usePaginatedClips){
      setPaginatedClips([]); setPaginatedHasMore(true); setPaginatedVisibleCount(PAGE_SIZE);
      return;
    }
    let cancelled=false;
    setPaginatedClips([]); setPaginatedVisibleCount(PAGE_SIZE);
    setPaginatedLoading(true);
    const q = query(
      collection(db,"clips"),
      where("subject","==",selectedSubject.subject),
      where("stage","==",selectedSubject.stage),
      where("topic","==",selectedSubject.topic)
    );
    getDocs(q).then(snap=>{
      if(cancelled) return;
      const docs = snap.docs.map(d=>({id:d.id,...d.data()}));
      docs.sort((a,b)=>Number(a.num||0)-Number(b.num||0)); // نفس منطق ترتيب الإدارة بالضبط
      setPaginatedClips(docs);
      setPaginatedHasMore(docs.length>PAGE_SIZE);
    }).catch(e=>{
      // فشل الاستعلام (غالباً فهرس مركّب مفقود بـFirestore أول مرة يشتغل هذا
      // المسار) — نرجع فوراً للمسار الكامل القديم بدل ما نعرض للطالب شاشة
      // فارغة. رسالة الخطأ بالكونسول عادة تحتوي رابط مباشر لإنشاء الفهرس
      // المطلوب تلقائياً بـFirebase Console — أنشئه مرة وحدة ليشتغل الترقيم.
      console.error("تعذّر تحميل مقاطع الفصل (رجعنا للتحميل الكامل):", e);
      setPaginatedFailed(true);
    }).finally(()=>{ if(!cancelled) setPaginatedLoading(false); });
    return ()=>{ cancelled=true; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  },[usePaginatedClips, selectedSubject?.subject, selectedSubject?.stage, selectedSubject?.grade, selectedSubject?.topic]);

  const loadMorePaginatedClips = () => {
    if(!usePaginatedClips || !paginatedHasMore) return;
    setPaginatedVisibleCount(prev=>{
      const next = prev+PAGE_SIZE;
      setPaginatedHasMore(next<paginatedClips.length);
      return next;
    });
  };
  ...
  const allVideos = React.useMemo(()=>{
    // ─── مسار الترقيم (فصل محدد + صلاحية وصول كاملة) ───────────────────
    // "paginatedClips" مرتّبة مسبقاً بأنفسنا برقم المقطع (Number()، ليس
    // Firestore) — هنا بس نطبّق فلتر تطابق الصف (بما فيها المقاطع بلا صف
    // محدد) وفلتر استبعاد المقاطع ناقصة العنوان (نفس منطق المسار العادي
    // أدناه بالضبط)، ثم نكشف أول "paginatedVisibleCount" منها فقط.
    if(usePaginatedClips){
      const filtered = paginatedClips.filter(c=>
        (!selectedSubject.grade || c.grade===selectedSubject.grade || !c.grade) &&
        (role!=="student" || !!(c.title&&c.title.trim()))
      );
      return filtered.slice(0, paginatedVisibleCount);
    }
    ...
    // ملاحظة مهمة: لازم نضيف الصف كمعيار ترتيب أساسي هنا (وليس فقط المادة)، وإلا
    // مقاطع بنفس المادة/المرحلة لكن بصفوف مختلفة (مثلاً أحياء سادس اعدادي وأحياء
    // ثالث متوسط) تتداخل مع بعض إذا تصادف تطابق أرقام مقاطعها — كل صف كان له
    // ترتيبه الرقمي المستقل أصلاً، فبدون فصل الصف بالترتيب هنا، تختلط المقاطع
    // بترتيب عرضي حسب تطابق الأرقام فقط لا حسب المنطق التعليمي الصحيح.
    orderedClips.sort((a,b)=>{
      const subjectCompare=(a.subject||"").localeCompare(b.subject||"","ar");
      if(subjectCompare!==0) return subjectCompare;
      const stageCompare=(a.stage||"").localeCompare(b.stage||"","ar");
      if(stageCompare!==0) return stageCompare;
      const gradeCompare=(a.grade||"").localeCompare(b.grade||"","ar");
      if(gradeCompare!==0) return gradeCompare;
      return Number(a.num||0)-Number(b.num||0);
    });
    // نأخذ عشوائياً 10 فقط من كل مادة/مرحلة/صف غير مشترك بها (معاينة)، بدل كل
    // المقاطع — نفس سبب إضافة الصف بالترتيب أعلاه: بدونه تختلط معاينات صفوف
    // مختلفة مع بعض بنفس الدفعة العشوائية
    const randomBySubject={};
    randomClips.forEach(c=>{
      const k=(c.subject||"")+"__"+(c.stage||"")+"__"+(c.grade||"");
  ...
  },[clips, selectedSubject, role, globalPrices, examScores, mySubscriptions, usePaginatedClips, paginatedClips, paginatedVisibleCount]);
  ...
  // نجيب الصفحة التالية من مقاطع الفصل تلقائياً لما الطالب يقترب من آخر
  // المقاطع المحمّلة حالياً — بدون أي زر "تحميل المزيد" ظاهر، ينسجم مع تجربة
  // التمرير المستمر بالتطبيق
  useEffect(()=>{
    if(usePaginatedClips && paginatedHasMore && !paginatedLoading && videoIdx>=allVideos.length-4){
      loadMorePaginatedClips();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  },[videoIdx, allVideos.length, usePaginatedClips, paginatedHasMore, paginatedLoading]);
  ...
  const openEditClip=(clip)=>{
    ...
    setClipLinkType(clip.linkType||"embed");
    setClipReferenceLink(clip.referenceLink||"");
  ...
  const resetClipForm=()=>{
    setEditingClip(null);
    setClipTitle("");setClipTeacher("");setClipVideoUrl("");setClipLinkType("embed");setClipReferenceLink("");
    setClipPhone("");
  ...
  const saveClip=async(goToNext)=>{
    ...
    try{
      ...
      const data={
        ...
        videoUrl:clipVideoUrl,linkType:clipLinkType,referenceLink:clipReferenceLink,thumbUrl:clipThumbUrl,
  ...
  return (
    <div style={C.app} ref={appScrollRef}>
      ...
      {screen==="home"&&(
        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove} style={{...C.fullScreenWrap,userSelect:"none",overscrollBehavior:"none"}}>
          ...
          <div style={{position:"absolute",inset:0,background:video.bg||"linear-gradient(180deg,#0f172a,#1e1b4b)"}}>
            ...
            {/* ⚠️ شارة تشخيص مؤقتة للترقيم — نحذفها بعد ما نتأكد */}
            {role==="student" && selectedSubject?.topic && (
              <div style={{position:"absolute",top:"6px",left:"6px",zIndex:50,backgroundColor:"rgba(220,38,38,0.9)",color:"#fff",fontSize:"10px",fontWeight:"bold",padding:"3px 8px",borderRadius:"6px",pointerEvents:"none"}}>
                ترقيم: {usePaginatedClips?"✅ شغّال":(paginatedFailed?"❌ فشل":"➖ غير مفعّل")} | ظاهر: {usePaginatedClips?Math.min(paginatedVisibleCount,paginatedClips.length):clips.filter(c=>c.subject===selectedSubject.subject&&c.stage===selectedSubject.stage&&c.topic===selectedSubject.topic).length} من {usePaginatedClips?paginatedClips.length:"—"}
              </div>
            )}
          ...
          <div style={C.section}>
            ...
            {adminTab==="clips"&&showClipForm&&(
              <div>
                ...
                {/* ✅ إضافة: اختيار طريقة عرض الرابط — تضمين بداخل التطبيق (الافتراضي)،
                    أو فتح خارجي فقط (لمحتوى مو ملكك، أو روابط Google Drive/Docs/Slides
                    التي لا تدعم التضمين أصلاً) */}
                <label style={C.label}>طريقة عرض هذا الرابط</label>
                <div style={{display:"flex",gap:"8px",marginBottom:"14px"}}>
                  <button type="button" onClick={()=>setClipLinkType("embed")} style={{flex:1,padding:"10px 6px",borderRadius:"10px",border:`1px solid ${clipLinkType==="embed"?"rgba(56,189,248,0.5)":"rgba(255,255,255,0.1)"}`,backgroundColor:clipLinkType==="embed"?"rgba(56,189,248,0.15)":"transparent",color:clipLinkType==="embed"?"#38bdf8":"#a1a1aa",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>
                    ▶ تضمين بداخل التطبيق
                  </button>
                  <button type="button" onClick={()=>setClipLinkType("external")} style={{flex:1,padding:"10px 6px",borderRadius:"10px",border:`1px solid ${clipLinkType==="external"?"rgba(56,189,248,0.5)":"rgba(255,255,255,0.1)"}`,backgroundColor:clipLinkType==="external"?"rgba(56,189,248,0.15)":"transparent",color:clipLinkType==="external"?"#38bdf8":"#a1a1aa",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>
                    🔗 فتح خارجي فقط
                  </button>
                </div>
                {clipLinkType==="external"&&(
                  <div style={{...C.infoBanner,backgroundColor:"rgba(234,179,8,0.1)",border:"1px solid rgba(234,179,8,0.3)",color:"#fbbf24",marginBottom:"14px",fontSize:"12px"}}>
                    ⚠ الطالب بيشوف زر "فتح المحتوى الأصلي" بس، ويروح لصفحة يوتيوب/جوجل درايف الأصلية بدل ما يشوفه جوا التطبيق. استخدم هذا الخيار لمحاضرات أستاذ خارجي أو روابط Google Drive/Docs/Slides (ما تشتغل بالتضمين أصلاً).
                  </div>
                )}
                {/* ✅ إضافة: رابط مرجعي خارجي اختياري — منفصل تماماً عن رابط الفيديو
                    أعلاه. لا يؤثر على عرض الفيديو بالشاشة الرئيسية إطلاقاً، ويظهر
                    فقط كبطاقة اختيارية جوا محادثة "مساعد" (زكي) كمصدر مراجعة إضافي. */}
                <label style={C.label}>رابط مرجعي إضافي (اختياري)</label>
                <div style={{...C.infoBanner,marginBottom:"10px",fontSize:"12px",backgroundColor:"rgba(52,211,153,0.08)",border:"1px solid rgba(52,211,153,0.25)",color:"#34d399"}}>
                  📎 هذا الرابط لا يظهر بالشاشة الرئيسية — يظهر بس كبطاقة اختيارية جوا محادثة "مساعد" (زكي) كمصدر شرح إضافي للطالب.
                </div>
                <input type="text" placeholder="https://youtube.com/... أو رابط صفحة جوجل" value={clipReferenceLink} onChange={e=>setClipReferenceLink(e.target.value)} style={C.input}/>
                <div style={C.saveRow}>
      ...
      {modal==="subscriptiondetails" &&<SubscriptionDetailsModal onClose={closeModal} mySubscriptions={mySubscriptions} globalPrices={globalPrices} student={currentStudent} clips={clips} studentProgress={studentProgress} progressUpdatedAt={progressUpdatedAt} onOpenWallet={()=>setModal("wallet")} onOpenCode={()=>setModal("code")} onOpenPartnerCode={()=>setModal("partnercode")} onSelectSubject={(subject,stage)=>{setSelectedSubject({subject,stage,grade:currentStudent?.grade});const key=subKey(subject,stage);setVideoIdx(studentProgress[key]||0);setCurrentSlideIdx(0);}}/>}
