...
const VideoPlayer = ({video, playing, onClick, canAccess=true, onSubscribe, externalSlideIdx, onExternalSlideChange}) => {
  ...
  if(video.type==="شرائح AI" && video.slides?.length){
    ...
    return <AnimatedSlides video={video} playing={playing} onClick={onClick} ts={ts} slideIdx={activeIdx} setSlideIdx={setActiveIdx}/>;
  }
...
export default function App() {
  ...
  const handleTouchEnd=(e)=>{
    ...
    if(absX>absY&&isSlides){
      ...
      if(diffY>0) {setVideoIdx(i=>Math.min(i+1,allVideos.length-1));setCurrentSlideIdx(0);}
      else {setVideoIdx(i=>Math.max(i-1,0));setCurrentSlideIdx(0);}
    }
