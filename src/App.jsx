...
const ADMIN_TABS = [{key:"clips",label:"المقاطع",Icon:Film},{key:"slides",label:"شرائح",Icon:Layers},{key:"students",label:"الطلاب",Icon:Users},{key:"prices",label:"الأسعار",Icon:DollarSign},{key:"notifications",label:"إشعارات",Icon:Bell},{key:"settings",label:"الإعدادات",Icon:Settings}];
...
function PDFModal({onClose}) {
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(249,115,22,0.2)"}}>
    ...
    {[["ملزمة الرياضيات — الفصل الأول","2.4 MB"],["أسئلة الاختبارات السابقة","1.1 MB"],["شرح مبسط بالصور","3.8 MB"]].map(([n,s])=>(
      <div key={n} style={{display:"flex",alignItems:"center",justifyContent:"space-between",...C.card}}>
        <div><div style={{fontSize:"13px",fontWeight:"bold"}}>{n}</div><div style={{fontSize:"11px",color:"#71717a",marginTop:"2px"}}>{s}</div></div>
        <button onClick={onClose} style={{backgroundColor:"rgba(249,115,22,0.15)",border:"1px solid rgba(249,115,22,0.3)",borderRadius:"8px",padding:"6px 12px",color:"#f97316",fontSize:"12px",cursor:"pointer",fontWeight:"bold"}}>تحميل</button>
      </div>
    ))}
  </div></div>;
}
...
export default function App() {
  ...
  return (
    <div style={C.app}>
      ...
      {modal==="pdf"&&<PDFModal onClose={closeModal}/>}
