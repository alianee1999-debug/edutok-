...
function AdminPDFTab() {
  ...
  const doDelete=async(f)=>{
    try{ await deleteDoc(doc(db,"pdfs",f.id)); alert("تم الحذف"); }
              ...
              <div>
                {confirmDeleteClip&&(
                  <div style={{backgroundColor:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:"14px",padding:"16px",marginBottom:"14px",textAlign:"center"}}>
                    ...
                    <div style={{display:"flex",gap:"8px",justifyContent:"center"}}>
                      <button onClick={()=>{
                        if(confirmDeleteClip.id) deleteDoc(doc(db,"clips",confirmDeleteClip.id)).then(()=>alert("تم الحذف")).catch(e=>alert("فشل: "+e.message));
