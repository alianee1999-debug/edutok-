...
function AdminExamsTab() {
  ...
  const extractQuestionsArray = (clean)=>{
    // المحاولة 1: أول [ وآخر ] بالنص كاملاً (الحالة الشائعة: مصفوفة مباشرة)
    const start = clean.indexOf("[");
    const end = clean.lastIndexOf("]");
  ...
  const generateBatch = async(grounding, batchCount)=>{
    ...
    const rawArray = extractQuestionsArray(clean);
    ...
    if(valid.length===0){
      ...
      throw new Error("لم يجتز أي سؤال التحقق من الصيغة. معاينة الرد: "+clean.slice(0,180));
    }
    return valid;
  };
