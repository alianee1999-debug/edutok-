...
function SlidesStudio({slidesTheme,setSlidesTheme,onSaveClip,clips}) {
  ...
  const generate=async(fromImage)=>{
    ...
    try{
      ...
      // نصلح أي "\" خام غير صحيحة (رموز LaTeX مثل \omega أو \frac) حتى لا تكسر JSON.parse
      const fixed=clean.substring(start,end+1).replace(/\\(?!["\\\/bfnrtu])/g,"\\\\");
  ...
  const importFromJSON=()=>{
    ...
    try{
      ...
      // نصلح أي "\" خام غير صحيحة (رموز LaTeX مثل \omega أو \frac) حتى لا تكسر JSON.parse
      const clean=clean0.replace(/\\(?!["\\\/bfnrtu])/g,"\\\\");
...
function AdminExamsTab() {
  ...
  // النموذج أحياناً يرجّعها كـ "\" مفردة رغم التعليمات، وهذا يكسر JSON.parse لأن "\" رمز خاص فيه.
  // نحافظ فقط على تسلسلات الهروب الصحيحة (\" \\ \/ \b \f \n \r \t \uXXXX) ونضاعف أي "\" غير معروفة.
  const sanitizeJsonBackslashes = (s)=>{
    return s.replace(/\\(?!["\\\/bfnrtu])/g, "\\\\");
  };
  const extractQuestionsArray = (clean)=>{
    const fixed = sanitizeJsonBackslashes(clean);
