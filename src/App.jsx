...
const AnimatedSlides = ({video, playing, onClick, ts, slideIdx, setSlideIdx, fontSize="medium"}) => {
  ...
  return (
    ...
    >
      ...
      <div style={{position:"absolute",top:"12px",left:"50%",transform:"translateX(-50%)",zIndex:6,backgroundColor:ts.card,borderRadius:"8px",padding:"3px 12px",border:`1px solid ${ts.accent}44`}}>
...
function AIModal({onClose,video,currentSlide,audioTracks,currentTrack,setCurrentTrack,audioPlaying,setAudioPlaying,audioVolume,setAudioVolume}) {
...
function SearchModal({onClose,allVideos,onSelectVideo}) {
  const [q,setQ]=useState("");
  // نطابق كل الحقول المفيدة للبحث مع بعض — العنوان، المادة، المعلم، الصف، ورقم الصفحة —
  // بدون ما ندمجهم فعلياً ببيانات المقطع (يبقون منفصلين بقاعدة البيانات لسهولة التعديل والفرز).
  // رقم الصفحة قد يكون مدى ("120-121")، فنطابقه بشكل نصي يشمل أي جزء من المدى، وأيضاً نتحقق
  // رقمياً لو كان البحث رقم يقع ضمن المدى (مثال: بحث "121" يطابق صفحة "120-121").
  const matchesPage = (pageStr, query) => {
    if(!pageStr) return false;
    const p = String(pageStr).trim();
    if(p.includes(query)) return true; // مطابقة نصية مباشرة (يشمل "120" أو "120-121")
  ...
  const filtered=allVideos.map((v,idx)=>({...v,_idx:idx})).filter(v=>!q||(
    v.title?.includes(q)||v.subject?.includes(q)||v.teacher?.includes(q)||v.grade?.includes(q)||matchesPage(v.page,q)
  ));
  return <div style={C.overlay}><div style={C.modalBox}>
    ...
    {filtered.slice(0,20).map((v,i)=>(
      <div key={i} onClick={()=>{onSelectVideo(v._idx,v);onClose();}} style={{display:"flex",alignItems:"center",gap:"10px",...C.card,cursor:"pointer",marginBottom:"6px"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:"11px",color:"#71717a"}}>{v.subject} • {v.stage}{v.grade?` • الصف ${v.grade}`:""}{v.page?` • صفحة ${v.page}`:""}</div>
        </div>
...
function VideoDescriptionModal({onClose,video,role,mySubscriptions,globalPrices,onOpenWallet,onSelectSubject,videoIdx,totalVideos,clips,watchedClipIds,examScores,progressUpdatedAt,allVideos,studentProgress,onJumpToVideo}) {
  ...
  const chapterInfo = (()=>{
    ...
    const firstUnwatched = topicClips.find(c=>!watchedClipIds?.includes(c.id));
    const tKey = topicKey(video.subject,video.stage,video.topic);
    const score = examScores?.[tKey];
    return {topicClips,watchedCount,missingCount,unlocked,remainingAllowance,stillNeededToUnlock,firstUnwatched,score};
  })();

  const jumpToFirstUnwatched = () => {
    if(!chapterInfo?.firstUnwatched||!allVideos||!onJumpToVideo) return;
    const idx = allVideos.findIndex(v=>v.id===chapterInfo.firstUnwatched.id);
    ...
    {chapterInfo&&(
      <div style={{backgroundColor:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"12px",padding:"12px 14px",marginBottom:"12px"}}>
        ...
        {chapterInfo.firstUnwatched&&(
          <button onClick={jumpToFirstUnwatched} style={{width:"100%",padding:"8px",borderRadius:"8px",border:"1px solid rgba(56,189,248,0.35)",backgroundColor:"rgba(56,189,248,0.1)",color:"#38bdf8",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>
            الانتقال لأول مقطع لم يُشاهد ←
          </button>
        )}
      </div>
    )}
...
function PartnerCodeModal({onClose,student}) {
  const [code,setCode]=useState("");
  const [checking,setChecking]=useState(false);
  const [err,setErr]=useState("");
  ...
  const redeem=async()=>{
    ...
    try{
      ...
      if(!d.ok){ setErr(d.error||"فشل تفعيل كود الشراكة"); setChecking(false); return; }
...
export default function App() {
  ...
  const saveClip=async()=>{
    ...
    try{
      const data={
        ...
        topic:clipTopic,page:clipPage,pageImage:clipPageImage,num:clipNum,teacher:clipTeacher,phone:clipPhone,
  ...
  return (
    <div style={C.app}>
      ...
      {/* عنصر تحكم عائم صغير بالصوت — يظهر فقط للطالب ولما يكون فيه مسار مختار
          حالياً، بغض النظر عن الشاشة (يستمر أثناء تصفح الشرائح، الامتحان، إلخ) */}
      {role==="student"&&currentTrack&&(
        <div style={{position:"fixed",bottom:"78px",left:"12px",right:"12px",zIndex:60,backgroundColor:"rgba(15,15,20,0.92)",backdropFilter:"blur(10px)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:"14px",padding:"8px 12px",display:"flex",alignItems:"center",gap:"10px",boxShadow:"0 4px 20px rgba(0,0,0,0.4)"}}>
          <button onClick={()=>setAudioPlaying(p=>!p)} style={{background:"rgba(56,189,248,0.15)",border:"none",borderRadius:"50%",width:"32px",height:"32px",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
            {audioPlaying?<Pause size={15} color="#38bdf8" fill="#38bdf8"/>:<Play size={15} color="#38bdf8" fill="#38bdf8"/>}
          </button>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:"11px",fontWeight:"bold",color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{currentTrack.title}</div>
          ...
          <button onClick={()=>{setAudioPlaying(false);setCurrentTrack(null);}} style={{background:"none",border:"none",color:"#71717a",cursor:"pointer",flexShrink:0,display:"flex"}}>
            <X size={16}/>
          </button>
        </div>
      )}
        ...
        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove} style={{...C.fullScreenWrap,userSelect:"none",overscrollBehavior:"none"}}>
          ...
          <div style={C.fullHeader}>
            <div>
              ...
              {video.grade&&(
                <div style={{display:"inline-flex",alignItems:"center",gap:"4px",background:"linear-gradient(135deg,#f59e0b,#d97706)",borderRadius:"20px",padding:"3px 10px 3px 8px",boxShadow:"0 2px 10px rgba(245,158,11,0.4)",marginTop:"6px",animation:"gradeBadgeIn 0.5s ease-out both"}}>
          ...
          <div style={C.sidebar}>
            {[
              ...
              [<FileText size={18} color="#fff"/>,"تفاصيل الاشتراك",()=>{setModal("description");setPlaying(false);},false],
          ...
          {showMore&&<div style={C.moreMenu}>
            {[[<Camera size={22} color="#fff"/>,"حل ذكي","solve"],[<Search size={22} color="#fff"/>,"البحث","search"],[<DollarSign size={22} color="#fff"/>,"زين كاش","wallet"],[<Key size={22} color="#fff"/>,"الكود","code"],[<Users size={22} color="#fff"/>,"كود شراكة","partnercode"]].map(([icon,label,key])=>(
            ...
            {adminTab==="clips"&&showClipForm&&(
              <div>
                ...
                {clipPageImage
                  ...
                  : <ImageUploader onUpload={url=>setClipPageImage(url)} onBase64={()=>{}} color="#38bdf8" label="ارفع صورة صفحة الكتاب" compress={true}/>
                }
      ...
      {/* MODALS */}
      {modal==="ai"     &&<AIModal      onClose={closeModal} video={video} currentSlide={video.slides?.[currentSlideIdx]} audioTracks={audioTracks} currentTrack={currentTrack} setCurrentTrack={setCurrentTrack} audioPlaying={audioPlaying} setAudioPlaying={setAudioPlaying} audioVolume={audioVolume} setAudioVolume={setAudioVolume}/>}
      ...
      {modal==="partnercode" &&<PartnerCodeModal onClose={closeModal} student={currentStudent}/>}
