...
export default function App() {
  ...
  const allVideos = React.useMemo(()=>{
    ...
    if(selectedSubject){
      ...
      if(!selectedSubject.topic){
        ...
        // "رقم المقطع" (num) غالباً يُعاد ترقيمه من جديد بكل فصل (01,02,...)، فلو رتّبنا حسب
        // num فقط بدون مراعاة الفصل، تتداخل مقاطع الفصول ببعضها. لذلك نرتب أولاً حسب تسلسل
        // الفصل نفسه، ثم حسب رقم المقطع داخل كل فصل.
        filtered = filtered.sort((a,b)=>{
          const ra = a.topic!=null ? (topicRank[a.topic] ?? 0) : 0;
          const rb = b.topic!=null ? (topicRank[b.topic] ?? 0) : 0;
  ...
  },[clips, selectedSubject, role, globalPrices, examScores]);
