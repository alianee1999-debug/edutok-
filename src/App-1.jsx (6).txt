...
function AdminExamsTab() {
  ...
  const generateBatch = async(grounding, batchCount)=>{
    const prompt = "أنت معلّم متخصص. بناءً على المحتوى الدراسي التالي لمادة "+subject+" ("+stage+" — فصل: "+topic+"):\n\n"+grounding
      +"\n\nأنشئ "+batchCount+" سؤال اختيار من متعدد (MCQ) مبنية حصراً على هذا المحتوى، بحيث كل سؤال له 4 خيارات وخيار واحد صحيح فقط. "
    ...
    const start = clean.indexOf("[");
    const end = clean.lastIndexOf("]");
    if(start===-1||end===-1) throw new Error("لم يُرجع المساعد صيغة صحيحة");
    const parsed = JSON.parse(clean.substring(start,end+1));
    return parsed.filter(q=>q.question && Array.isArray(q.options) && q.options.length===4 && Number.isInteger(q.correctIndex) && q.correctIndex>=0 && q.correctIndex<=3);
  };

  // العدد الأقصى لكل طلب واحد للذكاء الاصطناعي — أعداد كبيرة تُقسّم تلقائياً لعدة دفعات متتالية لضمان استجابة JSON سليمة وكاملة من النموذج في كل مرة
    ...
    try{
      ...
      for(let b=0;b<batches;b++){
        ...
        try{
          ...
          showMsg("تنبيه: فشلت إحدى الدفعات ("+e.message+")، جارٍ متابعة الباقي...");
        }
      }
      if(collected.length===0) throw new Error("لم يتم توليد أي سؤال صالح");
  ...
  return <div>
    ...
    <button disabled={generating||!topic} onClick={generate} style={{...C.purpleBtn,opacity:(generating||!topic)?0.6:1,marginBottom:genProgress?"8px":"18px"}}>
