...
function VideoDescriptionModal({onClose,video,role,mySubscriptions,globalPrices,onOpenWallet,onOpenCode,onSelectSubject,videoIdx,totalVideos,clips,watchedClipIds,examScores,progressUpdatedAt,allVideos,studentProgress,onJumpToVideo}) {
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(255,255,255,0.1)"}}>
    ...
    {/* بديل عن زين كاش — يفعّل نفس المادة فوراً بكود جاهز بدل انتظار مراجعة
        الدفع، معروض بنفس مكان خيار الدفع مباشرة عشان الطالب يشوف الخيارين مع
        بعض بلحظة اتخاذ القرار */}
    {role==="student"&&video.subject&&!sub&&(
      <button onClick={onOpenCode} style={{width:"100%",padding:"9px",borderRadius:"10px",border:"1px dashed rgba(196,181,253,0.4)",backgroundColor:"rgba(168,85,247,0.06)",color:"#c4b5fd",fontSize:"12px",fontWeight:"bold",cursor:"pointer",marginBottom:"14px",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}>
        <Key size={13}/> أو لدي كود تفعيل جاهز
      </button>
    )}
      ...
      {screen==="home"&&(
        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove} style={{...C.fullScreenWrap,userSelect:"none",overscrollBehavior:"none"}}>
          ...
          {showMore&&<div style={C.moreMenu}>
            ...
            {role==="student"&&(
              <button style={C.moreItem} onClick={()=>{setPlaying(false);openExam(video.subject,video.stage,video.topic);setShowMore(false);}}>
                <GraduationCap size={22} color="#fff"/><span style={{fontSize:"11px",marginTop:"4px"}}>امتحان</span>
              </button>
            )}
