...
import { ExplainBackModal } from "./ExplainBackModal";
...
@keyframes explainIconGlow{0%,100%{box-shadow:0 0 8px 2px rgba(129,140,248,0.5)}50%{box-shadow:0 0 18px 7px rgba(129,140,248,0.9)}}
@keyframes explainIconEnter{0%{transform:scale(0) rotate(-20deg);opacity:0}70%{transform:scale(1.15) rotate(6deg);opacity:1}100%{transform:scale(1) rotate(0deg);opacity:1}}
@keyframes explainThinking{0%,100%{transform:scale(1) rotate(0deg)}25%{transform:scale(1.08) rotate(-4deg)}75%{transform:scale(1.08) rotate(4deg)}}
@keyframes explainGloryGlow{0%,100%{filter:drop-shadow(0 0 10px rgba(250,204,21,0.5))}50%{filter:drop-shadow(0 0 18px rgba(250,204,21,0.9))}}
`;
...
const AnimatedSlides = ({video, playing, onClick, ts, slideIdx, setSlideIdx, fontSize="medium"}) => {
  ...
  useEffect(()=>{ setShowExplain(false); }, [video.id]); // نغلق نافذة "اشرحلي بكلامك" عند تغيّر المقطع

  // ─── سحب رمز "اشرحلي بكلامك" — نفس منطق سحب رمز السؤال بالضبط (مثبت
  // الشغل)، بحالة مستقلة تماماً عشان ما يتعارضوا لو ظهروا بنفس الشريحة ───
  const [brainPos, setBrainPos] = useState(null); // {x,y}px، null = افتراضي (وسط أسفل الشاشة)
  const [brainDragging, setBrainDragging] = useState(false);
  const brainDragInfo = useRef({offsetX:0,offsetY:0,startX:0,startY:0,moved:false});
  const brainJustDraggedRef = useRef(false);
  const brainSuppressNextClick = useRef(false);
  const BRAIN_ICON_SIZE = 36;
  useEffect(()=>{ setBrainPos(null); }, [video.id]); // نرجّعه لموضعه الافتراضي عند تغيّر المقطع
  const beginBrainDrag = (e) => {
    e.stopPropagation();
    const t = e.touches ? e.touches[0] : e;
    const rect = containerRef.current.getBoundingClientRect();
    const btnRect = e.currentTarget.getBoundingClientRect();
    brainDragInfo.current = {
      offsetX: t.clientX - btnRect.left,
      offsetY: t.clientY - btnRect.top,
      startX: t.clientX, startY: t.clientY,
      moved: false,
    };
    setBrainDragging(true);
  };
  const moveBrainTo = (clientX, clientY) => {
    const rect = containerRef.current.getBoundingClientRect();
    const dx = clientX - brainDragInfo.current.startX;
    const dy = clientY - brainDragInfo.current.startY;
    if(Math.abs(dx)>8 || Math.abs(dy)>8){ brainDragInfo.current.moved = true; brainJustDraggedRef.current = true; }
    let x = clientX - rect.left - brainDragInfo.current.offsetX;
    let y = clientY - rect.top - brainDragInfo.current.offsetY;
    x = Math.max(4, Math.min(x, rect.width - BRAIN_ICON_SIZE - 4));
    y = Math.max(4, Math.min(y, rect.height - BRAIN_ICON_SIZE - 4));
    setBrainPos({x,y});
  };
  const handleBrainTouchMove = (e) => {
    if(!brainDragging) return;
    e.stopPropagation();
    const t = e.touches[0];
    moveBrainTo(t.clientX, t.clientY);
    // نفس المبدأ: preventDefault بس بعد تأكيد سحب حقيقي، مو على أي رعشة بسيطة
    if(brainDragInfo.current.moved && e.cancelable) e.preventDefault();
  };
  const endBrainDrag = (e) => {
    e.stopPropagation();
    setBrainDragging(false);
    // نفس مبدأ رمز السؤال: نفتح مباشرة عند رفع الإصبع، مو بالاعتماد على click
    if(!brainDragInfo.current.moved){
      setShowExplain(true);
      brainSuppressNextClick.current = true;
    }
  };
  useEffect(()=>{
    if(!brainDragging) return;
    const onMove=(e)=>moveBrainTo(e.clientX, e.clientY);
    const onUp=()=>setBrainDragging(false);
  ...
  const handleBrainIconClick = (e) => {
    e.stopPropagation();
    if(brainSuppressNextClick.current){ brainSuppressNextClick.current=false; return; }
    if(brainJustDraggedRef.current){ brainJustDraggedRef.current=false; return; }
    setShowExplain(true);
  };
  const DURATION = 5000;
  ...
  const quizSuppressNextClick = useRef(false);
  ...
  const handleQuizTouchMove = (e) => {
    ...
    // نستدعي preventDefault فقط بعد ما نتأكد إنه سحب حقيقي (تجاوز حد ٨ بكسل)
    // — استدعاؤه على أي حركة بسيطة (حتى رعشة إصبع طبيعية أثناء الضغط) يخلي
    // متصفحات الجوال تُلغي حدث click التالي بالكامل، فتفتح المشكلة نفسها اللي
    // واجهناها (سحب يشتغل لكن الضغط العادي ما يفتح شيء)
    if(dragInfo.current.moved && e.cancelable) e.preventDefault();
  };
  const endQuizDrag = (e) => {
    e.stopPropagation();
    setDragging(false);
    // نفتح مباشرة هنا (وقت رفع الإصبع) بدل الانتظار لحدث click من المتصفح —
    // click بعد اللمس مو مضمون يجي دايماً (رعشة إصبع بسيطة كفيلة يلغيه ببعض
    // المتصفحات)، فنعتمد فقط على touchend لمسارات اللمس، ونمنع onClick التالي
    // (لو المتصفح أطلقه بالإضافة لذلك) من يفتح مرة ثانية بالخطأ.
    if(!dragInfo.current.moved){
      setShowQuiz(true);
      quizSuppressNextClick.current = true;
    }
  };
  ...
  const handleQuizIconClick = (e) => {
    e.stopPropagation();
    if(quizSuppressNextClick.current){ quizSuppressNextClick.current=false; return; } // فتحناها أصلاً من touchend
  ...
  // سحب جانبي لتغيير الشرائح — يتحقق إن الحركة أفقية فعليًا (لا رأسية) قبل ما
  // يتدخل، وإلا يترك السحب الرأسي يمر بدون تدخل للتنقل الطبيعي بين المقاطع
  ...
  const handleTouchEnd = (e) => {
    ...
    if(Math.abs(diffX)>50 && Math.abs(diffX)>Math.abs(diffY)){
      if(diffX>0) goTo(slideIdx+1); // سحب يسار → التالي
    ...
    <div ref={containerRef} style={{position:"absolute",inset:0,zIndex:6,background:ts.bg,overflow:"hidden",cursor:"pointer"}}
      ...
      onTouchEnd={e=>{
        if(touchX.current!==null){
          const diffX=touchStart.current.x-e.changedTouches[0].clientX;
          const diffY=touchStart.current.y-e.changedTouches[0].clientY;
          // نوقف انتشار الحدث فقط لو كان سحباً أفقياً فعلياً (تغيير شريحة) —
          // وإلا نتركه يمر طبيعياً للأعلى، عشان السحب الرأسي (تغيير المقطع)
          // يستمر يشتغل بدون أي تدخل منّا
          if(Math.abs(diffX)>50 && Math.abs(diffX)>Math.abs(diffY)) e.stopPropagation();
        }
        handleTouchEnd(e);
      }}
    >
      ...
      {/* ─── رمز "اشرحلي بكلامك" (أسلوب فاينمان) — يظهر فقط بآخر شريحة بالمقطع،
          وقابل للسحب لأي مكان بالشاشة تمامًا مثل رمز السؤال ─── */}
      {slideIdx===total-1 && total>0 && (
        <button
          onClick={handleBrainIconClick}
          onTouchStart={beginBrainDrag}
          onTouchMove={handleBrainTouchMove}
          onTouchEnd={endBrainDrag}
          onTouchCancel={endBrainDrag}
          onMouseDown={beginBrainDrag}
          title="اشرحلي بكلامك"
          style={{
            position:"absolute",
            ...(brainPos?{left:brainPos.x+"px",top:brainPos.y+"px"}:{right:"10px",bottom:"150px"}),
            zIndex:15,width:BRAIN_ICON_SIZE+"px",height:BRAIN_ICON_SIZE+"px",borderRadius:"50%",border:"none",
            background:"linear-gradient(135deg,#4f46e5,#818cf8)",
            display:"flex",alignItems:"center",justifyContent:"center",
            fontSize:"16px",cursor:"grab",touchAction:"none",userSelect:"none",padding:0,
            animation: brainDragging?"none":"explainIconEnter 0.5s ease-out, explainIconGlow 2.4s ease-in-out 0.5s infinite",
          }}
        >🧠</button>
      )}
      {showExplain && (
        <ExplainBackModal video={video} onClose={()=>setShowExplain(false)}/>
      )}
    </div>
  );
};
