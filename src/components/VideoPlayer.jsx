...
const AnimatedSlides = ({video, playing, onClick, ts, slideIdx, setSlideIdx, fontSize="medium"}) => {
  ...
  const beginQuizDrag = (e) => {
    e.stopPropagation();
    setDebugTouch(c=>c+1); // ⚠️ سطر تشخيص مؤقت
  ...
  const [debugTouch,setDebugTouch]=useState(0); // ⚠️ عداد تشخيص مؤقت
  const [debugClick,setDebugClick]=useState(0); // ⚠️ عداد تشخيص مؤقت
  const handleQuizIconClick = (e) => {
    e.stopPropagation();
    setDebugClick(c=>c+1); // ⚠️ سطر تشخيص مؤقت
    ...
    >
      ...
      {sl.question&&(
        <button
          ...
          style={{
            position:"absolute",
            ...(quizPos?{left:quizPos.x+"px",top:quizPos.y+"px"}:{right:"14px",top:"220px"}),
            ...
            fontSize:"14px",cursor:"grab",touchAction:"none",userSelect:"none",padding:0,
            animation: dragging?"none":"quizIconPulse 2.2s ease-in-out infinite",
          }}
        >❓</button>
      )}

      {/* ⚠️ شارة تشخيص مؤقتة — تُظهر عدد اللمسات والضغطات المُستلمة فعليًا،
          نحذفها بعد ما نحدد السبب */}
      {sl.question&&(
        <div style={{position:"absolute",left:"10px",bottom:"10px",zIndex:50,background:"rgba(220,38,38,0.9)",color:"#fff",fontSize:"11px",fontWeight:"bold",padding:"4px 10px",borderRadius:"8px"}}>
          لمس: {debugTouch} — ضغط: {debugClick}
        </div>
      )}
