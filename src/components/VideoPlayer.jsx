...
@keyframes quizIconPulse{0%,100%{transform:scale(1)}50%{transform:scale(1.1)}}
...
const AnimatedSlides = ({video, playing, onClick, ts, slideIdx, setSlideIdx, fontSize="medium"}) => {
  ...
  const beginQuizDrag = (e, clientX, clientY) => {
    e.stopPropagation();
  ...
  useEffect(()=>{
    ...
    const handleMove=(e)=>{
      ...
      const size = 46;
      let x = t.clientX - rect.left - dragInfo.current.offsetX;
      let y = t.clientY - rect.top - dragInfo.current.offsetY;
      x = Math.max(4, Math.min(x, rect.width - size - 4));
      y = Math.max(4, Math.min(y, rect.height - size - 4));
      setQuizPos({x,y});
    };
    const handleUp=()=>{
      setDragging(false);
      if(!dragInfo.current.moved) setShowQuiz(true);
    };
  ...
  return (
    <div ref={containerRef} style={{position:"absolute",inset:0,zIndex:2,background:ts.bg,overflow:"hidden",cursor:"pointer"}}
      onClick={onClick}
    ...
    >
      ...
      {sl.question&&(
        <button
          ...
          onTouchStart={e=>{const t=e.touches[0];beginQuizDrag(e,t.clientX,t.clientY);}}
          onMouseDown={e=>beginQuizDrag(e,e.clientX,e.clientY)}
          title="سؤال سريع"
          style={{
            position:"absolute",
            ...(quizPos?{left:quizPos.x+"px",top:quizPos.y+"px"}:{right:"14px",bottom:"96px"}),
            zIndex:15,width:"46px",height:"46px",borderRadius:"50%",border:"none",
            background:"linear-gradient(135deg,#f59e0b,#facc15)",
            boxShadow:"0 4px 16px rgba(250,204,21,0.55)",
            display:"flex",alignItems:"center",justifyContent:"center",
            fontSize:"20px",cursor:"grab",touchAction:"none",userSelect:"none",
            animation: dragging?"none":"quizIconPulse 2.2s ease-in-out infinite",
          }}
        >❓</button>
      )}
