...
const AnimatedSlides = ({video, playing, onClick, ts, slideIdx, setSlideIdx, fontSize="medium"}) => {
  ...
  const QUIZ_ICON_SIZE = 30;
  // نستخدم أحداث اللمس مباشرة (مو Pointer Events) ونوقف انتشارها من نفس عنصر
  // الرمز، لأن حاوية الشريحة (سحب بين الشرائح) والحاوية الرئيسية بالتطبيق (سحب
  // بين المقاطع) عندهم مستمعي touch خاصين بيهم يتفعّلون تلقائياً لأي لمسة على
  // الشاشة ما لم نوقفها من مصدرها بالضبط — إيقاف الانتشار وقت اللمس الأول
  // يمنعهم نهائياً من رؤية هذه اللمسة (target اللمس يبقى ثابت على الرمز طول
  // السحبة بغض النظر عن مكان الإصبع).
  ...
  useEffect(()=>{
    ...
    const onUp=(e)=>{ setDragging(false); if(!dragInfo.current.moved) setShowQuiz(true); };
      ...
      {sl.question&&(
        <button
          ...
          style={{
            ...
            fontSize:"13px",cursor:"grab",touchAction:"none",userSelect:"none",padding:0,
