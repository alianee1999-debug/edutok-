...
import { getFirestore, collection, addDoc, onSnapshot, serverTimestamp, deleteDoc, updateDoc, setDoc, getDoc, doc } from "firebase/firestore";
...
const C = {
  ...
  floatingNav:{position:"absolute",bottom:0,left:0,right:0,display:"flex",justifyContent:"space-around",alignItems:"center",padding:"10px 0 16px",zIndex:25,background:"linear-gradient(0deg,rgba(0,0,0,0.55),rgba(0,0,0,0))"},
};
...
const AnimatedSlides = ({video, playing, onClick, ts, slideIdx, setSlideIdx}) => {
  ...
  return (
    <div style={{position:"absolute",inset:0,zIndex:2,background:ts.bg,overflow:"hidden",cursor:"pointer"}} onClick={onClick}>
      ...
      <div key={animKey} style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",justifyContent:"center",padding:"20px 18px",animation:`${visible?"slideFadeIn":"slideFadeOut"} ${TRANSITION}ms ease forwards`}}>
      ...
      {/* أزرار التنقل */}
      <div style={{position:"absolute",bottom:"12px",left:"12px",right:"12px",display:"flex",gap:"8px"}} onClick={e=>e.stopPropagation()}>
        <button disabled={slideIdx===0} onClick={()=>goTo(slideIdx-1)} style={{flex:1,padding:"9px",borderRadius:"10px",border:"none",backgroundColor:slideIdx===0?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.12)",color:slideIdx===0?"rgba(255,255,255,0.2)":"#fff",cursor:slideIdx===0?"not-allowed":"pointer",fontWeight:"bold",fontSize:"12px",backdropFilter:"blur(4px)"}}>◀ السابق</button>
        <button disabled={slideIdx===total-1} onClick={()=>goTo(slideIdx+1)} style={{flex:1,padding:"9px",borderRadius:"10px",border:"none",backgroundColor:slideIdx===total-1?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.12)",color:slideIdx===total-1?"rgba(255,255,255,0.2)":"#fff",cursor:slideIdx===total-1?"not-allowed":"pointer",fontWeight:"bold",fontSize:"12px",backdropFilter:"blur(4px)"}}>التالي ▶</button>
      </div>

      {/* نقاط التنقل */}
      <div style={{position:"absolute",top:"10px",left:"50%",transform:"translateX(-50%)",display:"flex",gap:"4px"}} onClick={e=>e.stopPropagation()}>
...
function VideoDescriptionModal({onClose,video,role,mySubscriptions,globalPrices,onOpenWallet,onSelectSubject}) {
...
function SlidesStudio({slidesTheme,setSlidesTheme,onSaveClip}) {
  ...
  if(mode==="json") return <div>{back}
    ...
    <div style={{backgroundColor:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.25)",borderRadius:"12px",padding:"10px 14px",marginBottom:"12px",fontSize:"12px",color:"#4ade80"}}>
      الصق هنا كود JSON الذي أعطاك إياه Claude، ثم اضغط "استيراد"
    </div>
    <textarea
      rows={10}
      value={jsonText}
      onChange={e=>{setJsonText(e.target.value);setJsonErr("");}}
      placeholder={'{"title":"عنوان الدرس","slides":[{"title":"عنوان الشريحة","points":["نقطة 1","نقطة 2"]}]}'}
      ...
      <label style={{display:"block",width:"100%",padding:"14px",backgroundColor:"rgba(139,92,246,0.08)",border:"2px dashed rgba(139,92,246,0.35)",borderRadius:"14px",textAlign:"center",cursor:"pointer",boxSizing:"border-box"}}>
        ...
        <input type="file" accept="image/*" style={{display:"none"}} onChange={e=>{
          ...
          reader.onload=(ev)=>{
            const img=new Image();
            img.onload=()=>{
              // ضغط تلقائي: أقصى عرض/ارتفاع 1024px، جودة 0.75
              const MAX=1024;
            ...
            <div key={clip.id} style={{ ...C.card, border: "1px solid rgba(139,92,246,0.2)", cursor: "pointer" }} onClick={() => openClip(clip)}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  ...
                  <div style={{ fontSize: "12px", color: "#71717a" }}>{clip.subject} - {clip.stage} - {clip.slides.length} شريحة</div>
                </div>
      ...
      {regularClips.length > 0 && (
        <div>
          <div style={{ fontSize: "13px", fontWeight: "bold", color: "#38bdf8", marginBottom: "8px", display: "flex", alignItems: "center", gap: "6px" }}>
            <Film size={14} /> مقاطع الفيديو ({regularClips.length})
          </div>
...
function AdminPricesTab() {
  ...
  const savePrices=async()=>{
    setSaving(true);
    try{
      await setDoc(doc(db,"settings","prices"),{values:prices,updatedAt:serverTimestamp()});
...
export default function App() {
  ...
  useEffect(()=>{
    const unsub=onSnapshot(doc(db,"settings","prices"),snap=>{
      setGlobalPrices(snap.exists()?(snap.data().values||{}):{});
    });
    return ()=>unsub();
  },[]);
  ...
  const allVideos = React.useMemo(()=>{
    ...
    // مدير: يرى الكل بدون قيود
    if(role==="admin"){
      return [...firebaseClips, ...SAMPLE_VIDEOS];
    }

    // طالب غير مشترك أو مشترك (وضع عشوائي):
    // 10 مقاطع عشوائية لكل مادة+مرحلة، تختلط كلها معاً
    const grouped = {};
    ...
    Object.values(grouped).forEach(group=>{
      // خلط عشوائي ثم أخذ 10 فقط من كل مجموعة
      const shuffled = [...group].sort(()=>Math.random()-0.5);
      mixed.push(...shuffled.slice(0,10));
    });

    // خلط النتيجة الكاملة عشوائياً
    const finalMix = mixed.sort(()=>Math.random()-0.5);
    return finalMix.length>0 ? [...finalMix,...SAMPLE_VIDEOS] : SAMPLE_VIDEOS;
  ...
  return (
    <div style={C.app}>
      ...
      {screen==="home"&&(
        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove} style={{...C.fullScreenWrap,userSelect:"none",overscrollBehavior:"none"}}>
          ...
          <div style={{position:"absolute",inset:0,background:video.bg||"linear-gradient(180deg,#0f172a,#1e1b4b)"}}>
            ...
            {videoIdx>0&&<div style={{position:"absolute",top:"56px",left:"50%",transform:"translateX(-50%)",color:"rgba(255,255,255,0.4)",zIndex:6}}><ChevronUp size={22}/></div>}
            {videoIdx<allVideos.length-1&&<div style={{position:"absolute",bottom:"90px",left:"50%",transform:"translateX(-50%)",color:"rgba(255,255,255,0.4)",zIndex:6}}><ChevronDown size={22}/></div>}
          ...
          {/* نقاط التنقل العمودية */}
          <div style={{position:"absolute",top:"60px",right:"16px",display:"flex",flexDirection:"column",gap:"4px",zIndex:6}}>
      ...
      {modal==="description" &&<VideoDescriptionModal onClose={closeModal} video={video} role={role} mySubscriptions={mySubscriptions} globalPrices={globalPrices} onOpenWallet={()=>setModal("wallet")} onSelectSubject={(subject,stage)=>{setSelectedSubject({subject,stage});setVideoIdx(0);}}/>}
