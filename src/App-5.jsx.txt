...
// ─── PDF MODAL ───────────────────────────────────────────
// ✅ التعديل: إضافة onWallet prop وتغيير أزرار الاشتراك لتفتح نافذة زين كاش
function PDFModal({onClose, isSubbed, onWallet}) {
  ...
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(249,115,22,0.2)"}}>
    ...
    {!isSubbed&&(
      <div style={{backgroundColor:"rgba(234,179,8,0.1)",border:"1px solid rgba(234,179,8,0.3)",borderRadius:"12px",padding:"12px",marginBottom:"14px",textAlign:"center"}}>
        ...
        {/* ✅ زر الاشتراك الرئيسي يفتح نافذة زين كاش */}
    ...
    :files.map(f=>(
      <div key={f.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",...C.card}}>
        ...
        {isSubbed
          ...
          /* ✅ زر اشترك بجانب كل ملف يفتح نافذة زين كاش */
...
function VideoDescriptionModal({onClose,video,role,mySubscriptions,onOpenWallet}) {
  const d=role==="student"&&video.subject?daysLeft(mySubscriptions,video.subject,video.stage):0;
  const sub=role==="student"&&video.subject?isSubscribed(mySubscriptions,video.subject,video.stage):true;
  return <div style={C.overlay}><div style={{...C.modalBox,border:"1px solid rgba(255,255,255,0.1)"}}>
    ...
    <p style={{fontSize:"13px",color:"#cbd5e1",margin:"0 0 14px"}}>‍ {video.teacher} • {video.subject} • {video.stage}</p>
    {role==="student"&&video.subject&&(
      sub&&d<=3?(
        <div style={{backgroundColor:"rgba(234,179,8,0.1)",border:"1px solid rgba(234,179,8,0.3)",borderRadius:"10px",padding:"10px 12px",fontSize:"13px",color:"#fbbf24",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span>اشتراك {video.subject} ينتهي خلال {d} يوم</span>
          <button onClick={()=>{onClose();onOpenWallet();}} style={{backgroundColor:"#f97316",border:"none",borderRadius:"6px",padding:"5px 12px",color:"#fff",fontSize:"12px",cursor:"pointer",fontWeight:"bold"}}>جدد</button>
        </div>
      ):!sub?(
        <div style={{backgroundColor:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:"10px",padding:"10px 12px",fontSize:"13px",color:"#f87171",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span>اشترك للوصول لمحتوى {video.subject}</span>
          <button onClick={()=>{onClose();onOpenWallet();}} style={{backgroundColor:"#ef4444",border:"none",borderRadius:"6px",padding:"5px 12px",color:"#fff",fontSize:"12px",cursor:"pointer",fontWeight:"bold"}}>اشترك</button>
        </div>
      ):null
    )}
  </div></div>;
...
export default function App() {
  ...
  const doRegister=async()=>{
    ...
    try{
      ...
      const stored = {name:regName.trim(),phone:phoneKey,account:regAccount.trim(),passHash:hash,passSalt:salt,createdAt:serverTimestamp()};
      await setDoc(doc(db,"students",phoneKey),stored);
      const s = {name:stored.name,phone:stored.phone,account:stored.account};
  ...
  const doLogin=async()=>{
    ...
    try{
      ...
      if(ok){
        const s = {name:ex.name,phone:ex.phone,account:ex.account};
        setCurrentStudent(s);setRole("student");setScreen("home");saveSession(s,"student");
  ...
  return (
    <div style={C.app}>
      ...
      {modal==="pdf"    &&<PDFModal     onClose={closeModal} isSubbed={isSubbed} onWallet={()=>setModal("wallet")}/>}
      ...
      {modal==="description" &&<VideoDescriptionModal onClose={closeModal} video={video} role={role} mySubscriptions={mySubscriptions} onOpenWallet={()=>setModal("wallet")}/>}
