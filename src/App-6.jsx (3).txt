...
export default function App() {
  ...
  const allVideos = React.useMemo(()=>{
    ...
    // لو اختار الطالب مادة → يرى مقاطعها بالتسلسل (مرتبة برقم المقطع)
    if(selectedSubject){
    ...
    // وضع عادي: للجميع (مع حد 10 مقاطع لغير المشترك)
    if(role==="student"){
      // 10 عشوائية من Firebase + عينات
      const shuffled = [...firebaseClips].sort(()=>Math.random()-0.5).slice(0,10);
      return [...shuffled, ...SAMPLE_VIDEOS];
    }

    // مدير أو زائر: يرى الكل
