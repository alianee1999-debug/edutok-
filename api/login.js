...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // بيانات الدخول (hash/salt) محفوظة بمستند فرعي منفصل مغلق بالكامل بقواعد Firestore
    // (students/{phone}/private/auth) — بهذا الشكل حتى لو انقرأ مستند الطالب الرئيسي بالخطأ
    // من مكان ثاني مستقبلاً، ما فيه أي بيانات حساسة بداخله أصلاً
    ...
    if (!authData.passHash || !authData.passSalt) {
      res.status(200).json({ ok: false, error: "الحساب غير مكتمل، تواصل مع المدير" });
      return;
    }

    const candidate = sha256Hex(authData.passSalt + ":" + String(password).trim());
    if (candidate !== authData.passHash) {
